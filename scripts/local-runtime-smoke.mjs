...
// Mirrors localRuntimeStatus()'s capabilities.videoLtx check in
// electron-main.js — kept as an informational flag only (not a hard
// failure below) since LTX is optional, not a required local capability
// the way FLUX image and Wan video are.
const ltxReady = optionsFor("CheckpointLoaderSimple", "ckpt_name").includes("ltxv-2b-0.9.8-distilled-fp8.safetensors")
  && optionsFor("CLIPLoader", "clip_name").includes("t5xxl_fp8_e4m3fn.safetensors")
  && Boolean(objectInfo?.LTXVImgToVideo);
...
console.log(JSON.stringify({
  ...
  comfyui: { baseUrl: comfy.baseUrl, device: cuda.name, vramTotal: cuda.vram_total, imageReady, videoReady, ltxReady }
}, null, 2));
