...
Once a Lab has an approved reference image, a **✨ Describe from reference**
button appears under the design-brief field. It reads that image with a
connected vision model (Gemini or OpenAI — whichever is connected in Model
Hub) and writes a description of what it sees straight into the design
brief, so you only have to add the action, pose, or moment you actually want
— not the whole visual description by hand. It appends rather than replaces,
so anything you already typed stays; run it again any time you swap the
reference image.
...
### Director-inspired presets

The **Director-inspired** chip filters to 22 craft presets, each modeled on
a music-video director's recognizable visual signature — camera language,
lighting, color, texture, and pacing — so you can browse and apply by a
name you already recognize. What actually reaches generation is
craft-only: camera movement, lighting approach, color palette, texture, and
pacing, never the director's name itself. The name is for your own
browsing and recognition; it never appears in a prompt.

A **My own look** card sits at the front of this scope — not a preset, just
a one-click revert to this project's own unstyled visual direction. It
shows **Active** whenever no style is currently locked, and **Use** the
moment any style is applied.
...
A newly added scene, character, location, or prop briefly highlights the
moment it appears, and a freshly generated take does the same in its
history list — a quick visual confirmation that something changed, not
just a silent re-render. The very first frame you ever generate in a
project, and the very first time you save one, get a slightly larger
one-time note instead of the usual quick confirmation. Neither is a
dialog to dismiss — both clear on their own in a few seconds.
...
When two or more scenes still need a frame, Storyboard's toolbar shows
**Generate everything (N)** next to Populate all shots. It generates a
still for every scene without one yet, one at a time — never in
parallel — using whatever model each scene is already set to (or the
default model, for a scene that's never been directed). Before it starts,
it shows an estimated cost and confirms; a scene with no connected
provider is skipped and named in that confirmation rather than silently
failing partway through. You can stop it between scenes with the ✕ on the
render status card, and every generated frame lands unreviewed in
Storyboard exactly as if you'd generated it by hand — nothing here skips
your review pass.

This step is stills only — animating them is the next one, below.

### Animate everything at once

Once two or more scenes have a frame but no video yet, the toolbar also
shows **Animate everything (N)**. Unlike Generate everything, this runs a
few scenes **at a time** (not strictly one at a time): video generation is
already queued and polled per provider, so each scene mostly waits on the
provider rather than on Storymaker, and running only one at a time would
leave that wait idle for no safety benefit. The concurrency stays capped
well short of hammering any provider.

Video costs meaningfully more per clip than a still, so the cost estimate
and confirmation matter more here — read it before continuing. The ✕ on
the render status card can only **stop new scenes from starting**: a video
already sent to a provider is already billed and can't be recalled, so
anything in flight finishes on its own regardless. Every finished video
lands unreviewed, exactly like a hand-generated one.

### Locking a shot against bulk actions

Every shot has a **Lock** toggle in Shot Director, next to its title.
Locking a shot is a promise to yourself as much as to Storymaker: **Generate
everything**, **Animate everything**, and **Populate all shots** will never
touch it, no matter what — even if the shot's current output later gets
cleared some other way. A locked scene shows a 🔒 **Locked** chip on its
Storyboard card.

Approving a shot — from Delivery's preflight, or using a video as a scene's
motion master — locks it automatically. That's the guarantee this exists
for: once you've called a shot finished, no bulk pass can ever quietly
redo it.

**Locking never limits your own hand.** It only ever gates the three bulk
actions above — Shot Director's own **Generate** button stays fully live on
a locked shot, exactly as if it weren't locked at all. A director can lock
a hand-tuned brief before it's even been generated, to protect it through
a later bulk pass, and can unlock an approved shot at any time to
deliberately let a future bulk action touch it again. Full manual, per-shot
control — model, prompt, references, camera, performance, every Director
Controls field — is always there regardless of lock state; locking adds a
safety rail on the automated path, it never removes one from the manual
path.

### Re-rolling every section of one kind

For a Music Video project, Storyboard's toolbar also carries a section-kind
picker and a **Re-roll section** button — "redo every Chorus" in one pass.
Unlike Generate/Animate everything (which only ever fill an *empty*
scene), this deliberately replaces what a scene already has: whichever
kind of output it currently carries gets a fresh one — a scene with only
a still gets a new still, a scene that already has a video gets a new
video (from its existing still, not the old video), matching exactly what
that scene's own "Generate another take" / "Generate another video"
button would do by hand.

Stills in the batch run one at a time; videos run concurrently, same as
the two single-purpose actions above. A locked or approved shot is never
included, whatever section kind it belongs to — this is the feature the
shot lock exists to make safe. The confirmation shows a cost estimate and
the still/video split before anything runs, and stopping mid-batch behaves
exactly as it does for Animate everything: no new item starts, but a
video already sent to a provider always finishes.

The old take a re-roll replaces isn't deleted — it stays in that shot's
generation history, it just stops being the one shown, the same review
gate every other generation in Storymaker already goes through.

### Re-rolling for a newly chosen feeling

Feeling isn't a per-section property the way a section's kind is — it's
one setting for the whole song, so there's no subset of sections to scope
a re-roll to by feeling. What changing Feeling actually needs is simpler:
everything already generated was made under whichever feeling was active
at the time, and picking a new one updates every section's text brief
immediately but doesn't repaint any pixel already rendered under the old
one.

That's what the **Re-roll all scenes for [Feeling]** button next to the
Feeling picker in the Song Map panel is for — it re-rolls every unlocked
scene with an existing take, all at once, using the exact same mechanism
as re-rolling by section kind (cost estimate first, stills sequential,
videos concurrent, locked and approved shots never touched). It only
appears once a feeling is chosen and at least one eligible scene exists.
...
Removing a character, location, prop, shot, or Style DNA no longer blocks on
a confirmation dialog — the action happens immediately and an **Undo** toast
appears in the corner for about eight seconds. Click it to put everything
back exactly as it was. Undo is single-level: it restores the whole project
to the moment just before that one action, so treat the toast as your window
to catch a misclick rather than a full history you can step back through.

### Running spend

A small **~$total** badge in the top bar (next to Open/Save) shows an
estimated running total for every AI generation in the current project —
hover it for a breakdown by provider and model, or click it to open
Delivery's full cost panel. It only appears once the project has at least
one generated asset, and it's the same estimate Delivery already shows, not
a second number that can drift from it. These are rough per-model
estimates, not real billing — check each provider's own dashboard for exact
charges.
