...
import { defaultStyleDna, layeredPaperEditorial, twoDStyleDnas, directorStyleDnas, legacyStyleToDna, migrateVisualDirection, resolveVisualDirection, stylePromptBlocks, styleVersion, blendStyles, STYLE_REFERENCE_ROLES, STYLE_STRENGTHS, styleHistoryFor, appendStyleHistory } from "./visual-direction.js";
import { analyzeAudioArrayBuffer, sectionShotPlan, sectionColor, distributeLyrics, formatTime, snapToBeat, splitSection, mergeSectionWithNext, setSectionBoundary, editSectionMeta, validateSections, SECTION_KINDS } from "./song-brain.mjs";
import { t, initLocale, setLocale, getLocale, SUPPORTED_LOCALES, formatCurrencyUSD } from "./i18n/index.mjs";

const asset = (name) => new URL(`../assets/styles/${name}.png`, import.meta.url).href;
// Two splash pieces that alternate launch-to-launch — not random, a genuine
// toggle, so two sessions in a row never repeat the same one. Picked once
// here at module load (not inside mountLaunchSplash, which reruns on every
// render() while the splash is still up — recomputing per-render would make
// the art flicker between the two mid-session). localStorage read/write is
// wrapped in try/catch since it can throw in a locked-down environment;
// falls back to the first piece rather than crashing the app over a
// cosmetic toggle.
const SPLASH_ARTWORKS = [
  new URL("../assets/storymaker-splash-1.png", import.meta.url).href,
  new URL("../assets/storymaker-splash-2.png", import.meta.url).href
];
const splashArtwork = (() => {
  const key = "storymaker-splash-last-index";
  try {
    const next = Number(localStorage.getItem(key)) === 0 ? 1 : 0;
    localStorage.setItem(key, String(next));
    return SPLASH_ARTWORKS[next];
  } catch {
    return SPLASH_ARTWORKS[0];
  }
})();
const styles = [
...
const systemStyleDnas = () => [...styles.map(legacyStyleToDna).filter((style) => style.id !== "system-layered-paper-editorial"), layeredPaperEditorial(), ...twoDStyleDnas(), ...directorStyleDnas()];
...
const modelCatalog = [
  ...
  // Added 2026-09-08, the day OpenAI released GPT Image 2.5 — Flare (fast,
  // everyday quality) and Sunburst (premium, edit-precision) — as two new
  // API models, plus their fal mirrors. Both confirmed live via direct
  // fetches of OpenAI's and fal's own docs, not inferred. Kie.ai does not
  // yet list either model as of this date, so it's intentionally left out;
  // add a kie-gpt-image-2.5-* row here the same way once they do.
  { provider: "openai", model: "gpt-image-2.5-flare", label: "GPT Image 2.5 Flare", modes: ["text-to-image", "image-edit", "reference-image"], status: "live" },
  { provider: "openai", model: "gpt-image-2.5-sunburst", label: "GPT Image 2.5 Sunburst", modes: ["text-to-image", "image-edit", "reference-image"], status: "live" },
  { provider: "fal", model: "fal-gpt-image-2.5-flare", label: "GPT Image 2.5 Flare · fal", modes: ["text-to-image", "image-edit", "reference-image"], status: "live" },
  { provider: "fal", model: "fal-gpt-image-2.5-sunburst", label: "GPT Image 2.5 Sunburst · fal", modes: ["text-to-image", "image-edit", "reference-image"], status: "live" },
...
const registeredModelCapability = (model) => ({
  ...
  // gpt-image-2.5-flare/sunburst (OpenAI direct + fal, added 2026-09-08):
  // the only models with a "background" control (transparent/opaque/auto —
  // see the shotBackground select in enhanceShotDirector) and a richer
  // qualityTiers set (low/medium/high/xhigh/max, surfaced as Fast/Balanced/
  // High/Very High/Maximum) instead of the app-wide Fast/Balanced/High.
  // Every other model's capability entry omits qualityTiers on purpose —
  // updateCapabilities only repopulates #shotQuality when it's present.
  "gpt-image-2.5-flare": { ratios: ["16:9", "9:16", "1:1", "3:2", "2:3", "21:9"], references: ["image"], controls: ["prompt", "negative prompt", "quality", "reference fidelity", "background"], output: "Image", qualityTiers: ["Fast", "Balanced", "High", "Very High", "Maximum"] },
  "gpt-image-2.5-sunburst": { ratios: ["16:9", "9:16", "1:1", "3:2", "2:3", "21:9"], references: ["image"], controls: ["prompt", "negative prompt", "quality", "reference fidelity", "background"], output: "Image", qualityTiers: ["Fast", "Balanced", "High", "Very High", "Maximum"] },
  "fal-gpt-image-2.5-flare": { ratios: ["16:9", "9:16", "1:1", "3:2", "2:3", "21:9"], references: ["image"], controls: ["prompt", "negative prompt", "quality", "reference fidelity", "background"], output: "Image", qualityTiers: ["Fast", "Balanced", "High", "Very High", "Maximum"] },
  "fal-gpt-image-2.5-sunburst": { ratios: ["16:9", "9:16", "1:1", "3:2", "2:3", "21:9"], references: ["image"], controls: ["prompt", "negative prompt", "quality", "reference fidelity", "background"], output: "Image", qualityTiers: ["Fast", "Balanced", "High", "Very High", "Maximum"] },
...
const blankProject = () => ({ id: `project-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, name: "Untitled Film", logline: "", premise: "", world: "", themes: [], rules: "", relationships: "", source: { name: "", text: "", importedAt: "" }, ingestion: { status: "empty", analysis: null, importedAt: "", warnings: [] }, directorReview: null, scriptSuggestions: [], storyVersions: [], lockedFields: [], style: "", visualDirection: null, characters: [], locations: [], sets: [], props: [], scenes: [], assets: [], audioTracks: [], styleDnas: [], song: null, songLyrics: "", songFeeling: "", songDirection: null, updatedAt: new Date().toISOString() });
...
// Stored preference wins; unset falls back to the browser's own language
// when Storymaker has it, English otherwise (initLocale, not a raw
// localStorage read — it's the one place that fallback logic lives).
let locale = initLocale(localStorage.getItem("storymaker-locale") || "");
// New installs now default to Simple Mode instead of Studio Mode. Studio's
// thirteen nav tabs are a wall for someone's first five minutes — Simple
// Mode's guided, single-path flow already exists to solve exactly that
// (simpleModeGuide below), it just was never anyone's actual starting
// point. Can't tell "brand new profile" from "existing user who simply
// never touched this specific setting" from storymaker-experience-mode
// alone — both read as unset. storymaker-generation-view is a reliable
// proxy instead: it's written unconditionally the first time ANY Shot
// Director panel opens (setExperience() below), which any real production
// use would have done at least once — so an existing installation's next
// launch after this update still lands in Studio, not Simple, even though
// it never explicitly recorded a preference either.
const hasExistingUsage = localStorage.getItem("storymaker-generation-view") !== null;
let experienceMode = localStorage.getItem("storymaker-experience-mode") || (hasExistingUsage ? "studio" : "simple");
...
let localEngineState = { runtimeFound: false, running: false, managed: false, device: null, download: null, catalog: [] };
let deliveryState = { previewRender: false };
let updateState = { status: "unknown", version: "", percent: 0, message: "" };
let telemetryState = { configured: false, enabled: false };
let licenseState = { configured: false, licensed: true, hasKey: false, status: "none", tier: "pro", proTierConfigured: false };
...
// Micro-interaction: a newly created or generated item gets a brief CSS
// flash the next time it renders, then never again. render() rebuilds
// affected markup from scratch via innerHTML every call, so there's no
// persisting DOM node for a CSS *transition* to animate between — but a
// CSS *animation* plays fine on a freshly created node, as long as it's
// applied exactly once. A stale flag would otherwise replay the flash on
// that same card during the next unrelated render (anything that calls
// render() re-creates every card's DOM node from scratch). Holding one id
// at a time and clearing it the moment a matching card checks for it gives
// it that single-shot behavior — set right before the render() that should
// show it, consumed by the one card whose id matches.
let pendingHighlight = "";
const flashHighlight = (id) => { pendingHighlight = id || ""; };
const consumeHighlight = (id) => { if (id && pendingHighlight === id) { pendingHighlight = ""; return true; } return false; };
...
// `locked` (disabled, undeletable, force-checked) is kept only as a latent
// capability for a future case that genuinely needs it — nothing currently
// passes it true. `suggested` is the non-blocking version actually in use:
// a plain, always-togglable checkbox with a "SCENE CAST" badge so the user
// can see *why* a tile is called out, without it being pre-checked or
// impossible to leave unchecked.
const referenceChoiceTile = (asset, checked, inputName = "shotReferences", removable = false, locked = false, suggested = false) => `<label class="reference-tile${checked ? " picked" : ""}${locked ? " continuity-locked" : ""}${suggested ? " continuity-suggested" : ""}" title="${esc(locked ? `${asset.name} is automatically attached for scene continuity` : suggested ? `${asset.name} belongs to this scene's cast, sets, or props — check it to attach it to this shot` : asset.name)}"><input type="checkbox" name="${inputName}" value="${esc(asset.id)}" ${checked ? "checked" : ""} ${locked ? "disabled" : ""}/>${asset.kind === "image" ? `<span class="reference-tile-art" style="background-image:url('${esc(fileUrl(asset))}')"></span>` : `<span class="reference-tile-art reference-tile-icon">${mediaIcon(asset.kind)}</span>`}<small>${esc(asset.name)}</small>${locked ? `<em>CONTINUITY</em>` : suggested ? `<em>SCENE CAST</em>` : ""}${asset.kind === "image" ? `<button type="button" class="reference-tile-zoom" data-lightbox-asset="${esc(asset.id)}" title="View ${esc(asset.name)} full size">⤢</button>` : ""}${removable ? `<button type="button" class="reference-tile-remove" data-remove-reference-asset="${esc(asset.id)}" title="Remove ${esc(asset.name)} from the project">×</button>` : ""}</label>`;
...
function mountReferenceLibrary({ roles, getRoles, ownedAssetIds, onImported, headTitle, headDescription, importButtonId, onRemoved }) {
  ...
  // The describe-from-reference button (Character/Prop/Set Lab) starts
  // disabled until an entity has a reference image, computed once when the
  // Lab modal first opens — but a successful generate or import updates the
  // gallery in place afterward (see the comment above every such call site)
  // rather than reopening the whole modal, so that initial disabled state
  // otherwise goes stale the moment the first reference actually lands.
  // mountReferenceLibrary is the one place both paths already call, so it's
  // the single point that can keep the button's state current for both.
  if (images.length) ["characterLabDescribeReference", "propLabDescribeReference", "setLabDescribeReference"].forEach((id) => { const button = document.getElementById(id); if (button) { button.disabled = false; button.removeAttribute("title"); } });
...
function visualTakeMarkup({ variation, asset }, index, total, scene) {
  ...
  return `<article class="visual-take ${approved || motionMaster ? "approved" : ""}${consumeHighlight(asset.id) ? " just-added" : ""}"><button data-open-variation="${esc(asset.id)}" class="visual-take-image"${style}>${media}<span>TAKE ${String(total - index).padStart(2, "0")} · ${asset.kind.toUpperCase()}</span></button><div><small>${esc(variation.generation?.model || asset.generation?.model || scene.generationSize || "Project media")}</small><h3>${title}</h3>${action}</div></article>`;
}
...
// shot has no generated output yet, and it isn't locked.
...
// locked: an explicit, durable "bulk actions must never touch this shot"
// flag — separate from outputReview/outputAssetId so it survives even if
// the shot's current output later gets cleared (e.g. its asset removed via
// Media Library cleanup), and settable by hand before a shot has any
// output at all. Auto-set true at every approval point (approveDeliveryShot,
// the motion-master handler); toggled by hand from Shot Director for a
// pro director who wants a hand-tuned brief protected before generating,
// or who deliberately wants to unlock an approved shot to let a future
// bulk pass touch it again.
const ensureShotShape = (scene, shot, index) => ({ id: shot?.id || `shot-${scene?.id || "scene"}-${index + 1}`, title: shot?.title || `${scene?.title || "Scene"} Shot ${String(index + 1).padStart(2, "0")}`, purpose: shot?.purpose || scene?.note || "Advance the scene objective.", framing: shot?.framing || "Medium shot", lens: shot?.lens || "35mm", movement: shot?.movement || "Static", duration: shot?.duration || "4", audio: shot?.audio || "", outputAssetId: shot?.outputAssetId || "", outputReview: shot?.outputReview || "unreviewed", locked: Boolean(shot?.locked), blueprint: { ...defaultShotBlueprint(), ...(shot?.blueprint || {}), narrative: shot?.blueprint?.narrative || shot?.purpose || scene?.note || "" }, modelSettings: { ...defaultShotModelSettings(), ...normalizeReferenceRoles(normalizeModelSettings(shot?.modelSettings || {})), referenceSelectionVersion: Number(shot?.modelSettings?.referenceSelectionVersion || 0), referenceUrls: Array.isArray(shot?.modelSettings?.referenceUrls) ? shot.modelSettings.referenceUrls : [], outputHistory: Array.isArray(shot?.modelSettings?.outputHistory) ? shot.modelSettings.outputHistory : [] } });
...
const ensureSceneShape = () => {
  project.scenes = Array.isArray(project.scenes) ? project.scenes.map((scene, index) => {
    const normalized = {
      ...
      songSectionId: scene?.songSectionId || "", // Music Video mode: ties this scene to a Song Map section (timing source)
      songSectionCreative: (scene?.songSectionCreative && typeof scene.songSectionCreative === "object") ? scene.songSectionCreative : null, // AI Director's per-section treatment (replaced wholesale on every re-direct)
      // Phase 8 — performance & choreography layer. These are the user's hand
      // edits and survive an AI re-direct (songSectionCreative does not).
      // sectionCast.{leadId,backupIds,dancerIds} reference project.characters ids;
      // choreography is a free-text movement note (falls back to the AI's
      // songSectionCreative.movement when blank).
      choreography: typeof scene?.choreography === "string" ? scene.choreography : "",
      sectionCast: (scene?.sectionCast && typeof scene.sectionCast === "object")
        ? { leadId: scene.sectionCast.leadId || "", backupIds: Array.isArray(scene.sectionCast.backupIds) ? scene.sectionCast.backupIds.filter(Boolean) : [], dancerIds: Array.isArray(scene.sectionCast.dancerIds) ? scene.sectionCast.dancerIds.filter(Boolean) : [] }
        : emptySectionCast()
    };
...
// Generic vision-capable provider pick, shared by style drift checking and
// the Lab reference-description helper below — not drift-specific despite
// the two current callers both being vision features.
...
async function checkStyleDrift(assetId, styleDnaId, sceneIndex = selectedSceneIndex) {
  ...
  try {
    ...
    // Re-fetch: ensureSceneShape() rebuilds project.scenes into new object
    // references on every render(), which can happen while the await above
    // is in flight — mutating the captured `scene` after that would write
    // to a detached object and silently lose the drift history.
    const liveScene = project.scenes[sceneIndex] || scene;
    liveScene.driftHistory = Array.isArray(liveScene.driftHistory) ? liveScene.driftHistory : [];
    liveScene.driftHistory.unshift(validated);
    if (liveScene.driftHistory.length > 5) liveScene.driftHistory.pop();
...
// Character/Prop/Set Lab prompt helper: reads a reference image and fills
// the design-brief field with a generated description, so a user who
// already uploaded a reference only has to add the action/pose they want
// it to carry out instead of writing the whole visual description by
// hand. Shared by all three Labs — same button, same handler, per-kind
// instruction text lives server-side (see REFERENCE_DESCRIPTION_PROMPTS in
// electron-main.js). Appends rather than replaces so a user's own edits to
// the field are never silently discarded.
async function describeReferenceForPrompt(kind, referenceAssetId, textareaId, button) {
  const asset = assetById(referenceAssetId);
  if (!asset?.path && !asset?.previewUrl) return notify("Add a reference image first, then describe it.");
  if (asset.kind !== "image") return notify("Only an image reference can be described.");
  const provider = selectVisionProvider();
  if (!provider) return notify("No vision provider configured. Add Gemini or OpenAI in Model Hub.");
  const field = $(`#${textareaId}`);
  if (!field) return;
  const original = button.textContent;
  button.disabled = true; button.textContent = "Reading reference…";
  try {
    const result = await window.storyMakerDesktop?.describeReference?.({ provider, kind, assetPath: asset.path || "", assetUrl: asset.previewUrl || "" });
    const description = (result?.description || "").trim();
    if (!description) throw new Error("No description was returned for this reference.");
    field.value = field.value.trim() ? `${field.value.trim()}\n\n${description}` : description;
    field.focus();
    notify("Description added from the reference — now add the action you want it to perform.");
  } catch (error) {
    notify(error?.message || "Could not describe this reference image.");
  } finally {
...
// All three toast variants (plain, celebrate, undo) share one #toast element
// and one pending timer — each must clear the OTHER variants' classes before
// claiming it, or a fast-following plain notify() after a celebrate/undo
// toast inherits its leftover styling instead of the plain look.
const resetToastVariant = (toast) => toast.classList.remove("celebrate", "undo");
function notify(message) {
  const toast = $("#toast");
  if (!toast) return;
  // Electron wraps every rejected ipcRenderer.invoke as
  // "Error invoking remote method 'name': Error: <the real message>".
  // Strip that wrapper so a user sees the actual message, not the plumbing.
  message = String(message ?? "").replace(/^Error invoking remote method '[^']+':\s*(?:Error:\s*)?/, "");
  resetToastVariant(toast);
  toast.textContent = message;
  toast.classList.add("show");
  clearTimeout(notify.timer);
  notify.timer = setTimeout(() => toast.classList.remove("show"), 2600);
  // Every gated call (generation, delivery, export) rejects with this exact
  // phrase from the main process when the copy isn't activated. Rather than
  // thread a handler through ~15 separate catch blocks, catch it centrally
  // here and surface the activation dialog — debounced so a burst of failed
  // calls opens it once.
  if (/isn't activated on this device/i.test(String(message || "")) && !$("#modalRoot")?.querySelector(".license-modal")) {
    clearTimeout(notify.licenseTimer);
    notify.licenseTimer = setTimeout(() => openLicenseDialog(), 400);
  }
}
// A richer, once-per-project celebration for a handful of real milestones
// (first frame generated, first save) — same #toast element as notify(),
// a distinct look and a longer read time, never a blocking dialog. Kept
// deliberately small and skippable: a milestone the user can't help but
// see for a couple seconds, not one they have to act on.
function notifyCelebrate(title, detail) {
  const toast = $("#toast");
  if (!toast) return;
  resetToastVariant(toast);
  toast.innerHTML = `<strong>${esc(title)}</strong><span>${esc(detail)}</span>`;
  toast.classList.add("show", "celebrate");
  clearTimeout(notify.timer);
  notify.timer = setTimeout(() => { toast.classList.remove("show", "celebrate"); toast.innerHTML = ""; }, 4200);
}
// A destructive action's safety net: instead of a blocking confirm() dialog
// (this app used to be inconsistent about even having one — some of these
// five actions did, most didn't, none were reversible either way), the
// action runs immediately and a dismissible "Undo" toast stands in for
// confirmation. Single-slot, not a history stack: `pendingUndo` holds at
// most one snapshot, matching the Gmail-style "Archive · Undo" pattern
// rather than true multi-level undo, since layering restores of a data
// model this interconnected (a removed character also touches every
// scene's castIds, a removed style touches scene/shot overrides and
// favorites — see deleteStyleDna) is where multi-level undo stops being
// trustworthy. The snapshot is the whole `project` object, taken right
// before the mutation and restored verbatim — simpler and more reliable
// than hand-reconstructing each action's cascading side effects, at the
// honest cost that any OTHER edit made during the ~8s undo window is also
// rolled back if Undo is pressed. Short window, prominent toast: judged an
// acceptable trade for how rarely that overlap will actually happen.
...
function withUndo(label, mutate) {
  ...
  notifyUndo(label, () => {
    if (pendingUndo !== snapshot) return;
    project = snapshot;
    pendingUndo = null;
    setDirty();
    render();
...
function notifyUndo(label, restore) {
  const toast = $("#toast");
  if (!toast) return;
  resetToastVariant(toast);
  toast.innerHTML = `<span>${esc(label)}</span><button type="button" id="toastUndo">Undo</button>`;
  toast.classList.add("show", "undo");
  clearTimeout(notify.timer);
  notify.timer = setTimeout(() => { toast.classList.remove("show", "undo"); toast.innerHTML = ""; pendingUndo = null; }, 8000);
  const undoButton = $("#toastUndo");
  // restore() (withUndo's closure) ends with its own notify("Undone.") —
  // that already resets and redisplays #toast with a fresh timer, so
  // nothing else needs to touch the toast here. It used to: clearing the
  // undo toast's chrome AFTER restore() ran erased the "Undone." message
  // restore() had just shown, and clearing `pendingUndo` before restore()
  // could read it made restore()'s own guard see null and silently no-op
  // — Undo visually dismissed the toast but never restored anything.
  if (undoButton) undoButton.onclick = () => { clearTimeout(notify.timer); restore(); };
}
// Each check-and-set happens in one call so every generation success path
// can call these unconditionally and safely — the flag on project.milestones
// (persisted with the project, so it fires once ever, not once per app
// session) makes every call after the first a no-op.
function celebrateFirstGeneration() {
  ensureProductionShape();
  if (project.milestones.firstGeneration) return;
  project.milestones.firstGeneration = true;
  notifyCelebrate("First frame generated!", "Your story just became visible. Keep going — every scene builds the film.");
}
function celebrateFirstSave() {
  ensureProductionShape();
  if (project.milestones.firstSave) return;
  project.milestones.firstSave = true;
  notifyCelebrate("First save — safe and sound.", "This production now lives on disk. Save often; it only takes a second.");
}
const FILM_TRIVIA = [
...
// Read by the Storyboard template to keep "Generate everything" disabled
// across the render() calls bulkGenerateAllScenes triggers after every
// scene (each one rebuilds the button from scratch, so a plain
// button.disabled set once would be wiped by the very next render).
...
// onSettled (optional) fires exactly once, at whichever terminal branch this
// job reaches — success, provider failure, or a check-failed error — so a
// caller (bulkGenerateAllVideos) can await a job to completion without
// duplicating this function's polling/upscale/asset-creation logic. Threaded
// through every recursive retry call below; a caller that doesn't pass it
// gets the exact prior fire-and-forget behavior.
async function monitorQueuedVideoJob(scene, shot, job, modelLabel, attempt = 0, onSettled = null) {
  const dockId = job?.renderDockId || job?.id || "foreground";
  const settle = (outcome) => { if (onSettled) { const fn = onSettled; onSettled = null; fn(outcome); } };
  if (!job?.providerTaskId || !window.storyMakerDesktop?.pollShotVideo) { closeGenerationOverlay(dockId); return settle({ ok: false, error: "No provider task to monitor." }); }
  // Re-resolve scene/shot fresh by id on every call, including every
  // recursive retry — never trust the objects passed in beyond this point.
  // render() (called below on completion, and by anything else re-rendering
  // Storyboard while this job polls) runs ensureSceneShape(), which rebuilds
  // project.scenes into brand-new objects every time. With two or more video
  // jobs polling concurrently — exactly what bulk generation does — an
  // earlier job's completion silently orphans every other job's held
  // reference, and that job's eventual write never reaches the real project.
  // Same failure mode confirmed live in bulkGenerateAllScenes before it was
  // fixed there; fixing it at the source here covers every caller, including
  // two ordinary manual "Generate video" clicks on different scenes.
  const liveScene = project.scenes.find((candidate) => candidate.id === scene?.id);
  if (liveScene) { scene = liveScene; shot = liveScene.shots.find((candidate) => candidate.id === shot?.id) || shot; }
  const monitorKey = `${job.provider || "provider"}:${job.model || "model"}:${job.providerTaskId}`;
  if (!attempt) {
    if (activeVideoMonitorIds.has(monitorKey)) return settle({ ok: false, error: "Already being monitored." });
    activeVideoMonitorIds.add(monitorKey);
  }
  const progress = Math.min(88, 16 + attempt * 4);
  updateGenerationOverlay({ id: dockId, progress, kind: "video", stage: attempt ? "Rendering at " + providerLabel(job.provider) + " · checking take " + (attempt + 1) : "Queued with " + providerLabel(job.provider) + " · preparing render" });
  try {
    ...
    if (result.asset) {
      ...
      if (upscaleFactor > 1) { if (!providerReady("fal")) throw new Error("Connect fal to create the requested " + job.deliveryResolution.toUpperCase() + " delivery file."); updateGenerationOverlay({ id: dockId, progress: 92, stage: "Creating the requested " + job.deliveryResolution.toUpperCase() + " delivery video" }); const upscaled = await window.storyMakerDesktop.upscaleMedia({ path: result.asset.path, kind: "video", factor: upscaleFactor }); if (!upscaled?.asset) throw new Error("The upscaler did not return the requested delivery video."); finalAsset = upscaled.asset; }
      const generated = { ...finalAsset, id: `shot-video-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, importedAt: new Date().toISOString(), source: upscaleFactor > 1 ? "fal" : job.provider, generation: { ...(upscaleFactor > 1 ? { ...(result.generation || {}), upscale: { provider: "fal", factor: upscaleFactor, target: job.deliveryResolution } } : result.generation), sourceFrameAssetId: job.sourceFrameAssetId || "", visualDirection: job.visualDirection || visualDirectionMetadata(scene, shot) } };
      project.assets = projectAssets(); project.assets.unshift(generated); job.assetId = generated.id; job.completedAt = new Date().toISOString(); job.status = "completed"; shot.outputAssetId = generated.id; syncAutomaticMotionMaster(scene, shot, generated.id); setDirty(); updateGenerationOverlay({ id: dockId, progress: 100, stage: "Video is saved to this project." }); setTimeout(() => closeGenerationOverlay(dockId), 900); render(); notify(modelLabel + " video is ready to review.");
      activeVideoMonitorIds.delete(monitorKey); return settle({ ok: true, assetId: generated.id });
    }
    if (result.status === "failed" || result.status === "cancelled") { setDirty(); updateGenerationOverlay({ id: dockId, progress: 100, stage: result.error || "The provider could not complete this video." }); setTimeout(() => closeGenerationOverlay(dockId), 1800); notify(result.error || "This video could not be completed."); activeVideoMonitorIds.delete(monitorKey); return settle({ ok: false, error: result.error || "The provider could not complete this video." }); }
    setDirty(); setTimeout(() => monitorQueuedVideoJob(scene, shot, job, modelLabel, attempt + 1, onSettled), Math.max(4000, Number(result.pollAfterSeconds || 6) * 1000));
  } catch (error) {
    Object.assign(job, { status: "check-failed", checkedAt: new Date().toISOString(), error: error?.message || "Video status could not be checked." }); setDirty(); updateGenerationOverlay({ id: dockId, stage: job.error }); setTimeout(() => closeGenerationOverlay(dockId), 1800); notify(job.error); activeVideoMonitorIds.delete(monitorKey); settle({ ok: false, error: job.error });
  }
}

// Room names double as internal routing identifiers (data-nav, `active ===`
// comparisons, the command palette's page targets) throughout this file —
// they stay English/stable. Only the RENDERED label translates, looked up
// here by that stable identifier.
const NAV_ROOM_LABEL_KEYS = {
  "Home": "nav.room.home", "Projects": "nav.room.projects", "Media Library": "nav.room.mediaLibrary", "AI Director": "nav.room.aiDirector",
  "Model Hub": "nav.room.modelHub", "Help": "nav.room.help", "Settings": "nav.room.settings"
};
const NAV_GROUP_LABEL_KEYS = { "PROJECT": "nav.group.project", "LIBRARY": "nav.group.library", "SYSTEM": "nav.group.system" };
function nav() {
  ...
  return navGroups.map((group) => ({ ...group, items: experienceMode === "simple" ? group.items.filter(([name]) => guidedRooms.has(name)) : group.items })).filter((group) => group.items.length).map((group) => `<div class="nav-group"><p>${esc(t(NAV_GROUP_LABEL_KEYS[group.label] || group.label))}</p>${group.items.map(([name, mark]) => `<button class="nav-item ${active === name ? "active" : ""}" data-nav="${name}" title="${esc(t(NAV_ROOM_LABEL_KEYS[name] || name))}"><i>${mark}</i><span>${esc(t(NAV_ROOM_LABEL_KEYS[name] || name))}</span></button>`).join("")}</div>`).join("");
}
function licenseBanner() {
  if (!licenseState.configured || licenseState.licensed) return "";
  return `<div class="license-banner"><span><b>${esc(t("shell.licenseBannerLead"))}</b> ${esc(t("shell.licenseBannerRest"))}</span><button type="button" id="licenseBannerActivate" class="save-button">${esc(t("shell.licenseBannerButton"))}</button></div>`;
}
function simpleModeGuide() {
  if (experienceMode !== "simple") return "";
  const steps = [["01", t("nav.simple.story"), "Story Bible"], ["02", t("nav.simple.look"), "Style Library"], ["03", t("nav.simple.board"), "Storyboard"], ["04", t("nav.simple.make"), "Motion Graphics"], ["05", t("nav.simple.deliver"), "Delivery"]];
  return `<section class="simple-mode-guide"><div><small>${esc(t("nav.simple.title"))}</small><strong>${esc(t("nav.simple.subtitle"))}</strong></div><nav>${steps.map(([number, label, destination]) => `<button data-nav="${destination}" class="${active === destination ? "active" : ""}"><span>${number}</span>${esc(label)}</button>`).join("")}</nav></section>`;
}
function shell(content) {
  // Persistent, unobtrusive running spend — projectCostSummary() already
  // powers Delivery's cost panel, but that's a page most people only visit
  // near the end of a project; this is BYO-API-key, real money per click,
  // and someone actively generating in Character Lab or Storyboard had no
  // way to see it without navigating away. Reuses the same summary rather
  // than a second tally, so the two numbers can never drift apart.
  const spend = projectCostSummary();
  const spendCount = spend.trackedCount + spend.untrackedCount;
  const spendBadge = spendCount ? `<button class="quiet-button spend-badge" data-nav="Delivery" title="${esc(t("shell.spend.tooltip", { total: formatCurrencyUSD(spend.total), count: spendCount }) + (spend.untrackedCount ? t("shell.spend.tooltipUntracked", { count: spend.untrackedCount }) : ""))}">${esc(t("shell.spend.badge", { total: formatCurrencyUSD(spend.total) }))}${spend.untrackedCount ? "+" : ""}</button>` : "";
  return `<div class="studio-shell nav-${navMode} experience-${experienceMode}" data-theme="${theme}">
    <aside class="studio-sidebar">
      ...
      <button id="newProject" class="create-button"><b>+</b><span>${esc(t("shell.newProject"))}</span></button>
      <button id="experienceMode" class="experience-mode-button" title="Switch workspace depth"><span>${experienceMode === "simple" ? "S" : "ST"}</span><b>${experienceMode === "simple" ? esc(t("shell.experienceSimpleLabel")) : esc(t("shell.experienceStudioLabel"))}</b><small>${experienceMode === "simple" ? esc(t("shell.experienceSimpleDetail")) : esc(t("shell.experienceStudioDetail"))}</small></button>
      <nav>${nav()}</nav>
      <div class="sidebar-foot"><button class="workspace-chip" data-nav="Model Hub"><i>${providerCatalog.filter((provider) => providerReady(provider.id)).length ? "●" : "○"}</i><span><b>${providerCatalog.filter((provider) => providerReady(provider.id)).length ? esc(t("shell.studioConnected")) : esc(t("shell.localWorkspace"))}</b><small>${providerCatalog.filter((provider) => providerReady(provider.id)).length ? esc(t("shell.providerAccessReady")) : esc(t("shell.privateOnThisPc"))}</small></span></button></div>
    </aside>
    <main class="studio-main">
      <header class="studio-bar">
        <div class="project-path"><span>WHEELBARROW STUDIOS</span><b>/</b><strong>${esc(project.name)}</strong></div>
        <div class="production-nav"><button class="${active === "Story Bible" ? "selected" : ""}" data-nav="Story Bible">${esc(t("nav.topbar.storyBible"))}</button><button class="${active === "Style Library" ? "selected" : ""}" data-nav="Style Library">${esc(t("nav.topbar.styleLibrary"))}</button><button class="${active === "Character Bible" ? "selected" : ""}" data-nav="Character Bible">${esc(t("nav.topbar.characterBible"))}</button><button class="${active === "Locations" ? "selected" : ""}" data-nav="Locations">${esc(t("nav.topbar.locations"))}</button><button class="${active === "Production Assets" ? "selected" : ""}" data-nav="Production Assets">${esc(t("nav.topbar.productionAssets"))}</button><button class="${active === "Storyboard" ? "selected" : ""}" data-nav="Storyboard">${esc(t("nav.topbar.storyboard"))}</button><button class="${active === "Continuity" ? "selected" : ""}" data-nav="Continuity">${esc(t("nav.topbar.continuity"))}</button><button class="${active === "Shot Planner" ? "selected" : ""}" data-nav="Shot Planner">${esc(t("nav.topbar.shotPlanner"))}</button><button class="${active === "Timeline" ? "selected" : ""}" data-nav="Timeline">${esc(t("nav.topbar.timeline"))}</button><button class="${active === "Motion Graphics" ? "selected" : ""}" data-nav="Motion Graphics">${esc(t("nav.topbar.motionGraphics"))}</button><button class="${active === "Audio Studio" ? "selected" : ""}" data-nav="Audio Studio">${esc(t("nav.topbar.audioStudio"))}</button><button class="${active === "Delivery" ? "selected" : ""}" data-nav="Delivery">${esc(t("nav.topbar.delivery"))}</button></div>
        <div class="bar-actions">${spendBadge}<button class="quiet-button" id="quickOpen">${esc(t("shell.open"))}</button><button class="save-button" id="saveProject">${esc(t("shell.save"))}<span>${dirty ? "•" : ""}</span></button></div>
      </header>
      <section class="studio-content">${licenseBanner()}${simpleModeGuide()}${content}</section>
    </main>
...
function homeProductionStatus() {
  ...
  const next = !hasStory
    ? { room: "Story Bible", eyebrow: t("home.next.eyebrowDecision"), title: t("home.next.story.title"), copy: t("home.next.story.copy"), action: t("home.next.story.action") }
    : !project.style
      ? { room: "Style Library", eyebrow: t("home.next.eyebrowDecision"), title: t("home.next.style.title"), copy: t("home.next.style.copy"), action: t("home.next.style.action") }
      : !characters.length
        ? { room: "Character Bible", eyebrow: t("home.next.eyebrowDecision"), title: t("home.next.cast.title"), copy: t("home.next.cast.copy"), action: t("home.next.cast.action") }
        : !scenes.length
          ? { room: "Storyboard", eyebrow: t("home.next.eyebrowDecision"), title: t("home.next.storyboard.title"), copy: t("home.next.storyboard.copy"), action: t("home.next.storyboard.action") }
          : !shots
            ? { room: "Shot Planner", eyebrow: t("home.next.eyebrowDecision"), title: t("home.next.shots.title"), copy: t("home.next.shots.copy"), action: t("home.next.shots.action") }
            : { room: "Motion Graphics", eyebrow: t("home.next.eyebrowReady"), title: t("home.next.ready.title"), copy: t("home.next.ready.copy"), action: t("home.next.ready.action") };
...
function visualDirectionSummary(scene = null, shot = null) {
  ...
  if (!style) return { name: t("home.visualDirection.notSet"), detail: t("home.visualDirection.notSetDetail"), source: t("home.visualDirection.waiting"), notSet: true };
  ...
  return {
    ...
    source: t("home.visualDirection.sourceLine", { source, strength: strengthLabel })
  };
}
...
function home() {
  ...
  return `<div class="production-home">
    <header class="production-home-heading">
      <div><p>${esc(t("home.greetingLine", { greeting: homeGreeting() }))}</p><h1>${esc(t("home.heading.title"))}</h1><span>${project.logline ? esc(project.logline) : esc(t("home.heading.defaultSpan"))}</span></div>
      <div class="home-command-row">${hasStory ? `<button id="newProjectHero" class="save-button">${esc(t("home.newProject"))}</button><button id="openProjectHero" class="quiet-button">${esc(t("home.openProject"))}</button>` : `<button id="openProjectHero" class="quiet-button">${esc(t("home.openProject"))}</button>`}</div>
    </header>
    <section class="production-resume-card">
      <div class="production-resume-art" style="background-image:linear-gradient(90deg,#090c15e8 0%,#090c158c 54%,#090c151c),url('${styleImage(selected)}')"><span>${esc(t("home.activeProduction"))}</span><strong>${esc(projectName)}</strong><small>${esc(project.style || t("home.visualLanguageNotChosen"))}</small></div>
      ${!hasStory ? `<div class="production-resume-copy production-resume-start"><div><p>${esc(t("home.startHere.eyebrow"))}</p><h2>${esc(t("home.startHere.title"))}</h2><span>${esc(t("home.startHere.body"))}</span></div><div class="production-resume-actions"><button id="pasteScriptHero" class="save-button">${esc(t("home.startHere.pasteButton"))}</button><button id="importFileHero" class="line-action">${esc(t("home.startHere.importLink"))}</button></div></div>`
        : `<div class="production-resume-copy"><div><p>${esc(status.next.eyebrow)}</p><h2>${esc(status.next.title)}</h2><span>${esc(status.next.copy)}</span></div><div class="production-progress"><div><span>${esc(t("home.readiness.label"))}</span><b>${status.progress}%</b></div><i><em style="width:${status.progress}%"></em></i><small>${esc(t("home.readiness.detail", { complete: status.complete }))}</small></div><div class="production-resume-actions"><button data-nav="${status.next.room}" class="save-button">${esc(status.next.action)}</button><button data-nav="Projects" class="line-action">${esc(t("home.projectLibrary"))}</button></div></div>`}
    </section>
    ${hasStory ? `<section class="production-home-section"><div class="production-section-heading"><div><p>${esc(t("home.pulse.eyebrow"))}</p><h2>${esc(t("home.pulse.title"))}</h2></div><span>${connectedProviders ? esc(t("home.pulse.providersConnected", { count: connectedProviders })) : esc(t("home.pulse.localWorkspace"))}</span></div><div class="production-pulse-grid">
      <button class="production-pulse-card" data-nav="Story Bible"><i>01</i><span>${esc(t("home.pulse.story.title"))}</span><strong>${project.logline || project.source?.text ? esc(t("home.pulse.story.inProgress")) : esc(t("home.pulse.story.waiting"))}</strong><small>${project.source?.name ? esc(project.source.name) : esc(t("home.pulse.story.importPrompt"))}</small></button>
      <button class="production-pulse-card" data-nav="Character Bible"><i>02</i><span>${esc(t("home.pulse.cast.title"))}</span><strong>${esc(t("home.pulse.cast.count", { count: status.characters.length }))}</strong><small>${status.characters.length ? esc(t("home.pulse.cast.available")) : esc(t("home.pulse.cast.create"))}</small></button>
      <button class="production-pulse-card" data-nav="Storyboard"><i>03</i><span>${esc(t("home.pulse.scenes.title"))}</span><strong>${esc(t("home.pulse.scenes.sceneCount", { count: status.scenes.length }))} · ${esc(t("home.pulse.scenes.shotCount", { count: status.shots }))}</strong><small>${status.scenes.length ? esc(t("home.pulse.scenes.openBoard")) : esc(t("home.pulse.scenes.mapStory"))}</small></button>
      <button class="production-pulse-card" data-nav="Model Hub"><i>04</i><span>${esc(t("home.pulse.access.title"))}</span><strong>${connectedProviders ? esc(t("home.pulse.access.connected")) : esc(t("home.pulse.access.connect"))}</strong><small>${connectedProviders ? esc(t("home.pulse.access.review")) : esc(t("home.pulse.access.keysPrivate"))}</small></button>
    </div></section>
    <section class="visual-direction-home-card ${visualDirection.notSet ? "waiting" : "ready"}">
      <div class="visual-direction-home-mark">✦</div>
      <div><p>${esc(t("home.visualDirection.eyebrow"))}</p><h2>${esc(visualDirection.name)}</h2><span>${esc(visualDirection.detail)}</span></div>
      <div class="visual-direction-home-state"><small>${esc(visualDirection.source)}</small><button data-nav="Style Library" class="${visualDirection.notSet ? "save-button" : "quiet-button"}">${visualDirection.notSet ? esc(t("home.visualDirection.setLook")) : esc(t("home.visualDirection.open"))}</button></div>
    </section>
    <section class="production-home-lower"><div class="production-start-grid"><div class="production-section-heading"><div><p>${esc(t("home.create.eyebrow"))}</p><h2>${esc(t("home.create.title"))}</h2></div></div><div class="production-entry-grid"><button id="pasteScriptHome" class="production-entry"><i>✎</i><strong>${esc(t("home.create.pasteScript.title"))}</strong><small>${esc(t("home.create.pasteScript.body"))}</small></button><button id="directMusicVideoHome" class="production-entry"><i>♪</i><strong>${esc(t("home.create.musicVideo.title"))}</strong><small>${esc(t("home.create.musicVideo.body"))}</small></button><button id="openProjectHome" class="production-entry"><i>↥</i><strong>${esc(t("home.create.importOpen.title"))}</strong><small>${esc(t("home.create.importOpen.body"))}</small></button><button data-nav="Style Library" class="production-entry"><i>◈</i><strong>${esc(t("home.create.visualDirection.title"))}</strong><small>${esc(t("home.create.visualDirection.body"))}</small></button></div></div>
      <aside class="production-style-panel"><div class="production-section-heading"><div><p>${esc(t("home.style.eyebrow"))}</p><h2>${esc(project.style || t("home.style.chooseDirection"))}</h2></div><button data-nav="Style Library" class="line-action">${esc(t("home.style.openLink"))}</button></div><p>${project.style ? esc(selected.tone) : esc(t("home.style.defaultBody"))}</p><div class="production-style-strip">${availableStyles.map((style) => `<button class="production-style-card ${project.style === style.name ? "selected" : ""}" data-style="${esc(style.name)}" title="${esc(t("home.style.chooseTitle", { name: style.name }))}"><img src="${styleImage(style)}" alt="${esc(style.name)}"/><span>${esc(style.name)}</span></button>`).join("")}</div></aside>
    </section>` : ""}
  </div>`;
}
...
function productionPromptFor(scene, shot, kind = "image") {
  ...
  const style = resolvedStyle.style ? `${resolvedStyle.style.promptLabel || resolvedStyle.style.name} (${resolvedStyle.sources.map((source) => source.scope).join(" → ")}): ${styleBlocks.positive.join(". ")}` : "Visual language: cinematic, coherent, production-ready.";
...
// A song section can legitimately run longer than the 99s the shot-planning
// field caps at, so in a song-timed project the section's real length wins —
// for the Timeline sums, the clip widths, AND the render (visualPreviewEntries
// reads the same section float via scene.songSectionId).
function songSectionSeconds(scene) {
  const section = (project.song?.sections || []).find((s) => s.id === scene?.songSectionId);
  return section ? Math.max(0.5, Number(section.end) - Number(section.start)) : null;
}
function clipSeconds(scene, shot) { return songSectionSeconds(scene) ?? numericDuration(shot?.duration); }
function ensureTimelineShape() {
  ...
  project.scenes.forEach((scene) => {
    ...
    // Don't clamp a song-section shot's stored duration to 99s.
    scene.shots.forEach((shot) => { if (!scene.songSectionId) shot.duration = String(numericDuration(shot.duration)); });
  });
...
function productionTimelineWorkspace() {
  ...
  const total = clips.reduce((sum, item) => sum + clipSeconds(item.scene, item.shot), 0);
...
function openAudioCueModal(cueId = "") {
  ...
  // The TYPE options' own text IS their stored value (no separate value=
  // attribute) — cue.type is compared/displayed verbatim elsewhere (CSV
  // export, FCPXML audio-role mapping), so these 4 words stay English on
  // purpose, the same way room identifiers do, rather than corrupting
  // stored project data the moment a non-English locale is active.
  $("#modalRoot").innerHTML = `<div class="modal-backdrop"><form id="audioCueForm" class="project-modal audio-cue-modal"><button type="button" id="closeModal">×</button><p>${esc(t("audio.cueModal.eyebrow"))} / ${existing ? esc(t("audio.cueModal.editCue")) : esc(t("audio.cueModal.newCue"))}</p><h2>${existing ? esc(t("audio.cueModal.editTitle")) : esc(t("audio.cueModal.newTitle"))}</h2><label>${esc(t("audio.cueModal.asset"))}<select id="cueAsset">${audioAssets.map((asset) => `<option value="${esc(asset.id)}" ${existing?.assetId === asset.id ? "selected" : ""}>${esc(asset.name)}</option>`).join("")}</select></label><div class="audio-cue-grid"><label>${esc(t("audio.cueModal.type"))}<select id="cueType">${["Dialogue", "Music", "Ambience", "Sound effect"].map((value) => `<option ${(existing?.type || "Music") === value ? "selected" : ""}>${value}</option>`).join("")}</select></label><label>${esc(t("audio.cueModal.scene"))}<select id="cueScene">${sceneOptions}</select></label><label>${esc(t("audio.cueModal.startSec"))}<input id="cueStart" value="${esc(existing?.start ?? "0")}" /></label><label>${esc(t("audio.cueModal.durationSec"))}<input id="cueDuration" value="${esc(existing?.duration ?? "5")}" /></label><label>${esc(t("audio.cue.level"))}<input id="cueLevel" value="${esc(existing?.level ?? "0 dB")}" /></label><label>${esc(t("audio.cueModal.notes"))}<input id="cueNotes" value="${esc(existing?.notes || "")}" placeholder="${esc(t("audio.cueModal.notesPlaceholder"))}" /></label></div><div><button type="button" id="cancelModal" class="quiet-button">${esc(t("common.cancel"))}</button><button class="save-button">${existing ? esc(t("audio.cueModal.saveCue")) : esc(t("audio.cueModal.addCue"))}</button></div></form></div>`;
...
// Music Video mode — Phases 2-4.
// A Song Map becomes the cut: one scene per detected section, each scene's
// single shot timed to that section's exact length; the track sits as a
// full-length Music cue at zero. visualPreviewEntries() (electron-main)
// times the rough cut and the FCPXML/Resolve export to the song via
// scene.songSectionId. Each section scene is seeded with a real creative
// brief from its kind + energy + the chosen "feeling", and its lyric lines
// — so the storyboard reads as a directed music video, not blank scenes.
const MUSIC_VIDEO_FEELINGS = ["performance", "narrative", "dance", "visualworld", "cinematic", "afrobeats"].map((id) => ({ id }));
function feelingLabel(id) { return MUSIC_VIDEO_FEELINGS.some((f) => f.id === id) ? t(`feeling.${id}.label`) : ""; }
function feelingHint(id) { return t(`feeling.${id}.hint`); }
function sectionBrief(section, songMap, feeling) {
  const kind = section.kind;
  const energyPct = Math.round((section.energy || 0) * 100);
  const range = `${formatTime(section.start)}–${formatTime(section.end)}`;
  const base = {
    Intro: "Establish the world and the mood. Wider, unhurried, few cuts. Hold the artist back — this is the room before the song.",
    Verse: "Carry the story. Closer framing, grounded blocking, medium energy. Detail and character over spectacle.",
    "Pre-Chorus": "Lift. Tighten in, build motion and cutting pace toward the hook.",
    Chorus: "The hook. Wide, performance-forward, the highest energy in the cut — the shot people remember. Repeat visual motifs so each chorus rhymes.",
    Bridge: "Break the pattern. Contrast the choruses — different location, light, or energy. A held moment.",
    Instrumental: "Let the visuals lead. Movement, landscape, or a performance beat with no vocal to serve.",
    Drop: "Peak impact. Fast cuts, bold motion, maximum contrast against what came before.",
    Outro: "Resolve and release. Pull back, slow the cutting, let the last image land and breathe."
  }[kind] || "Serve the section's energy.";
  const feel = feeling ? ` ${{
    performance: "Keep the artist in nearly every shot; favour direct address.",
    narrative: "Advance the story thread here; keep a recurring character or place.",
    dance: "Movement is the subject — formations and group energy over coverage.",
    visualworld: "Mood and place over plot; let atmosphere do the work.",
    cinematic: "Film-language framing: wide lenses, deliberate camera moves, strong contrast.",
    afrobeats: "Warm colour, sunlight, texture, confident movement, celebration."
  }[feeling] || ""}` : "";
  return `${kind} · ${range} · energy ${energyPct}%. ${base}${feel}`;
}
// Re-seed each section scene's note (and shot brief) from the current
// feeling + lyrics, without re-analyzing the audio. Called when the feeling
// or lyrics change.
// Compose the visible section brief. When the AI Director has run
// (scene.songSectionCreative), its shootable treatment leads; otherwise the
// deterministic template (kind + energy + feeling). Lyrics for the section
// are appended either way. Re-run on any feeling / lyrics / direction change.
// The empty per-section cast-role shape — one factory instead of the
// literal hand-written at every site that needs a default/blank value.
function emptySectionCast() { return { leadId: "", backupIds: [], dancerIds: [] }; }
// Phase 8 — resolve who performs this section. Manual role picks
// (scene.sectionCast) win; the AI Director's `lead`/`backup` names seed the
// roles by name-match when the user hasn't set them. Returns the character
// objects plus a de-duped id list for likeness attachment.
function sectionPerformers(scene) {
  const cast = scene.sectionCast || {};
  const byId = (id) => project.characters.find((ch) => ch.id === id) || null;
  const byName = (name) => name ? project.characters.find((ch) => ch.name && ch.name.toLowerCase() === String(name).toLowerCase()) || null : null;
  const c = scene.songSectionCreative;
  const lead = byId(cast.leadId) || byName(c?.lead) || null;
  const manualBackup = (cast.backupIds || []).map(byId).filter(Boolean);
  const aiBackup = manualBackup.length ? [] : (Array.isArray(c?.backup) ? c.backup.map(byName).filter(Boolean) : []);
  const backup = [...manualBackup, ...aiBackup].filter((ch) => ch.id !== lead?.id);
  const dancers = (cast.dancerIds || []).map(byId).filter(Boolean).filter((ch) => ch.id !== lead?.id && !backup.some((b) => b.id === ch.id));
  const ids = [...new Set([lead?.id, ...backup.map((ch) => ch.id), ...dancers.map((ch) => ch.id)].filter(Boolean))];
  return { lead, backup, dancers, ids };
}
// The movement note that actually drives generation: the user's hand-edited
// choreography if present, else the AI Director's movement line.
function effectiveChoreography(scene) {
  return (scene.choreography || "").trim() || (scene.songSectionCreative?.movement || "").trim();
}
function performerSummary(perf) {
  return [
    perf.lead && `Lead ${perf.lead.name}`,
    perf.backup.length && `backup ${perf.backup.map((ch) => ch.name).join(", ")}`,
    perf.dancers.length && `dancers ${perf.dancers.map((ch) => ch.name).join(", ")}`
  ].filter(Boolean).join("; ");
}
function composeSectionNote(scene, section, sectionLines) {
  const c = scene.songSectionCreative;
  const header = `${section.kind} · ${formatTime(section.start)}–${formatTime(section.end)} · energy ${Math.round((section.energy || 0) * 100)}%`;
  const castLine = performerSummary(sectionPerformers(scene));
  const choreo = effectiveChoreography(scene);
  let body;
  if (c && (c.shot || c.performance)) {
    body = [
      c.shot && `Shot: ${c.shot}`,
      c.camera && `Camera: ${c.camera}`,
      c.blocking && `Blocking: ${c.blocking}`,
      c.performance && `Performance: ${c.performance}`,
      choreo && `Choreography: ${choreo}`,
      c.wardrobe && `Wardrobe: ${c.wardrobe}`,
      castLine && `Cast: ${castLine}`
    ].filter(Boolean).join("\n");
  } else {
    body = [
      sectionBrief(section, project.song, project.songFeeling).replace(/^[^.]*\.\s*/, ""), // drop the header sentence the template repeats
      castLine && `Cast: ${castLine}`,
      choreo && `Choreography: ${choreo}`
    ].filter(Boolean).join("\n");
  }
  const note = `${header}\n${body}`;
  return sectionLines.length ? `${note}\n\n♪ ${sectionLines.join(" / ")}` : note;
}
function reseedSongScenes() {
  if (!project.song) return;
  const lines = distributeLyrics(project.songLyrics || "", project.song.sections);
  project.song.lyrics = lines;
  project.scenes.forEach((scene) => {
    const section = project.song.sections.find((s) => s.id === scene.songSectionId);
    if (!section) return;
    const sectionLines = lines.filter((l) => l.sectionId === section.id).map((l) => l.text);
    scene.note = composeSectionNote(scene, section, sectionLines);
    const c = scene.songSectionCreative;
    const perf = sectionPerformers(scene);
    const castLine = performerSummary(perf);
    const choreo = effectiveChoreography(scene);
    if (scene.shots.length) {
      const brief = c && c.shot
        ? [c.shot, c.camera, c.blocking, c.performance, choreo].filter(Boolean).join(" ")
        : [sectionBrief(section, project.song, project.songFeeling), castLine && `Performers: ${castLine}.`, choreo && `Choreography: ${choreo}`].filter(Boolean).join(" ");
      // A section scene normally holds exactly one shot, but Shot Planner's
      // "Add shot" works on any scene — refresh EVERY shot here (not just
      // shots[0]) so a second shot never keeps a stale prompt from whenever
      // it was added, and split the section's exact length evenly across
      // however many shots there are, so the export/render timing this
      // feeds (visualPreviewEntries) and this displayed duration always agree.
      const perShotDuration = String(Math.max(0.5, Math.round(((section.end - section.start) / scene.shots.length) * 10) / 10));
      scene.shots.forEach((shot) => {
        shot.purpose = brief;
        shot.duration = perShotDuration;
        const bp = shot.blueprint || {};
        // Fold the section's performing cast (by name, so their likeness
        // references attach) + choreography into the blueprint fields
        // shotImagePrompt already reads — "Blocking and composition", "Motion
        // plan", "Performance direction". Lyrics ride along via scene.note.
        shot.blueprint = {
          ...bp,
          narrative: brief,
          camera: c?.camera || bp.camera || "",
          performance: c?.performance || bp.performance || "",
          blocking: [castLine && `Performers — ${castLine}.`, c?.blocking || bp.blocking || ""].filter(Boolean).join(" ").trim(),
          motion: choreo || bp.motion || ""
        };
      });
    }
    // Auto-attach every performing cast member so their Character Bible
    // references flow into this section's generation — the likeness lock,
    // for free, for the lead AND the ensemble.
    if (perf.ids.length) {
      const merged = new Set([...(scene.castIds || []), ...perf.ids]);
      scene.castIds = [...merged];
    }
    if (!scene.sectionCast) scene.sectionCast = emptySectionCast();
  });
}
// One-time seeding of the editable role pickers from an AI direction pass —
// name-matched to Character Bible entries. Only fills blanks, so a later
// re-direct never clobbers the user's hand-picked cast.
function seedSectionCastFromAI() {
  const byName = (name) => name ? project.characters.find((ch) => ch.name && ch.name.toLowerCase() === String(name).toLowerCase()) : null;
  project.scenes.forEach((scene) => {
    if (!scene.songSectionId || !scene.songSectionCreative) return;
    const c = scene.songSectionCreative;
    if (!scene.sectionCast) scene.sectionCast = emptySectionCast();
    if (!scene.sectionCast.leadId) { const lead = byName(c.lead); if (lead) scene.sectionCast.leadId = lead.id; }
    if (!scene.sectionCast.backupIds.length && Array.isArray(c.backup)) {
      const ids = c.backup.map(byName).filter(Boolean).map((ch) => ch.id).filter((id) => id !== scene.sectionCast.leadId);
      if (ids.length) scene.sectionCast.backupIds = [...new Set(ids)];
    }
  });
}
// Delivery polish — does the assembled cut actually cover the whole song?
// Every section should have exactly one scene tied to it (scene.songSectionId);
// a section with none produces a silent gap in the picture edit (the audio
// keeps playing under nothing). Surfaced in the Deliver workspace so that
// gap is caught before a handoff, not discovered by an editor after.
function songSyncReport(project) {
  const song = project.song;
  if (!song) return null;
  const sections = song.sections || [];
  const sceneBySection = new Map(project.scenes.filter((scene) => scene.songSectionId).map((scene) => [scene.songSectionId, scene]));
  const missing = sections.filter((section) => !sceneBySection.has(section.id));
  const coveredSec = sections.filter((section) => sceneBySection.has(section.id)).reduce((sum, section) => sum + Math.max(0, section.end - section.start), 0);
  return { name: song.name, bpm: song.bpm, durationSec: song.durationSec, sectionCount: sections.length, missing, coveredSec, lyricsCount: (song.lyrics || []).length, ok: missing.length === 0 };
}
async function directMusicVideoWithAI() {
  if (!project.song) return notify("Direct a song first.");
  if (!window.storyMakerDesktop?.directMusicVideoTreatment) return notify("The AI director is available in the packaged Windows app.");
  const hasOutput = project.scenes.some((s) => s.shots.some((sh) => sh.outputAssetId));
  if ((project.songDirection || hasOutput) && !confirm("Re-direct every section with AI? This rewrites the section briefs (generated frames and takes are untouched).")) return;
  const button = $("#directMusicVideoAI"); const label = button?.textContent;
  if (button) { button.disabled = true; button.textContent = "Directing…"; }
  try {
    const sections = project.song.sections.map((s) => ({
      id: s.id, label: s.label, kind: s.kind, startSec: s.start, endSec: s.end, energy: s.energy,
      lyrics: (project.song.lyrics || []).filter((l) => l.sectionId === s.id).map((l) => l.text)
    }));
    const result = await window.storyMakerDesktop.directMusicVideoTreatment({
      song: { name: project.song.name, bpm: project.song.bpm, durationSec: project.song.durationSec },
      feeling: project.songFeeling || "",
      treatment: [project.logline, project.premise, project.source?.text].filter(Boolean).join("\n\n").slice(0, 8000),
      cast: project.characters.map((c) => ({ name: c.name, description: [c.role, c.appearance, c.wardrobe].filter(Boolean).join(". ") })),
      sections
    });
    const byId = new Map(result.sections.map((s) => [s.sectionId, s]));
    project.scenes.forEach((scene) => { if (byId.has(scene.songSectionId)) scene.songSectionCreative = byId.get(scene.songSectionId); });
    project.songDirection = { logline: result.logline, energyArc: result.energyArc, visualApproach: result.visualApproach, provider: result.provider, at: new Date().toISOString() };
    if (result.logline && (!project.logline || project.logline === "")) project.logline = result.logline;
    seedSectionCastFromAI();
    reseedSongScenes();
    setDirty();
    render();
    notify(`AI directed ${result.sections.length} sections${result.provider ? ` · ${result.provider}` : ""}. Every section now has a shot, camera, performance and movement note.`);
  } catch (error) {
    notify(error?.message?.replace(/^Error invoking remote method '[^']+':\s*(?:Error:\s*)?/, "") || "The AI director could not complete.");
  } finally {
    if (button?.isConnected) { button.disabled = false; button.textContent = label || "Direct with AI"; }
  }
}
// A scene shell for a section that didn't exist before (a split). ensureSceneShape
// fills the rest; reseedSongScenes writes the real note + shot brief.
function makeSongScene(section) {
  return { id: `mv-${section.id}`.slice(0, 60), title: section.label, note: "", songSectionId: section.id, songSectionCreative: null,
    shots: [{ title: `${section.label} — shot`, purpose: "", duration: String(Math.max(1, Math.round((section.end - section.start) * 10) / 10)) }] };
}
// Apply a pure section-array transform and reconcile the scene list without
// rebuilding it — generated frames, hand edits and AI direction on the
// sections that survive are kept.
function commitSectionEdit(op, sectionId, result) {
  const err = validateSections(result.sections, project.song.durationSec);
  if (err) return notify(err);
  if (op === "merge") {
    const gone = project.scenes.find((s) => s.songSectionId === result.removedSectionId);
    if (gone?.shots?.some((sh) => sh.outputAssetId) && !confirm(`Merging discards "${gone.title}" and its generated output. Continue?`)) return;
    project.song.sections = result.sections;
    project.scenes = project.scenes.filter((s) => s.songSectionId !== result.removedSectionId);
  } else if (op === "split") {
    project.song.sections = result.sections;
    const idx = project.scenes.findIndex((s) => s.songSectionId === sectionId);
    const newScene = makeSongScene(result.sections.find((s) => s.id === result.newSectionId));
    project.scenes.splice(idx < 0 ? project.scenes.length : idx + 1, 0, newScene);
  } else {
    project.song.sections = result.sections;
  }
  ensureSceneShape();
  ...
  setDirty();
  render();
  if (active === "Audio Studio") openSectionEditor();
}
function openSectionEditor() {
  const song = project.song;
  if (!song) return notify(t("audio.sectionEditor.needSong"));
  const sections = song.sections;
  // SECTION_KINDS values (Verse/Chorus/...) are stored data — the AI
  // Director schema, section-color mapping, and CSV/FCPXML exports all key
  // off these exact words — so they stay English, same reasoning as the
  // audio-cue TYPE options above.
  const rows = sections.map((s, i) => {
    const dur = Math.max(0, s.end - s.start);
    return `<div class="section-editor-row" style="--sec:${sectionColor(s.kind)}">
      <input data-section-label="${esc(s.id)}" value="${esc(s.label)}" title="${esc(t("audio.sectionEditor.labelTitle"))}" />
      <select data-section-kind="${esc(s.id)}" title="${esc(t("audio.sectionEditor.kindTitle"))}">${SECTION_KINDS.map((k) => `<option ${s.kind === k ? "selected" : ""}>${k}</option>`).join("")}</select>
      <span class="section-editor-time">${formatTime(s.start)}–${formatTime(s.end)} <em>${formatTime(dur)}</em></span>
      <span class="section-editor-split"><input data-section-split-at="${esc(s.id)}" value="${Math.round((s.start + s.end) / 2)}" inputmode="decimal" title="${esc(t("audio.sectionEditor.splitAtTitle"))}" ${dur < 5 ? "disabled" : ""} /><button type="button" class="quiet-button" data-section-split="${esc(s.id)}" ${dur < 5 ? "disabled" : ""}>${esc(t("audio.sectionEditor.split"))}</button></span>
      <button type="button" class="quiet-button" data-section-merge="${esc(s.id)}" ${i >= sections.length - 1 ? "disabled" : ""}>${esc(t("audio.sectionEditor.merge"))}</button>
      ${i < sections.length - 1 ? `<label class="section-editor-boundary">${esc(t("audio.sectionEditor.boundaryLabel"))}<input data-section-boundary="${esc(s.id)}" value="${dur ? s.end.toFixed(1) : ""}" inputmode="decimal" title="${esc(t("audio.sectionEditor.boundaryTitle"))}" /></label>` : "<span></span>"}
    </div>`;
  }).join("");
  $("#modalRoot").innerHTML = `<div class="modal-backdrop"><section class="project-modal wide-modal section-editor"><button type="button" id="closeModal">×</button><p>${esc(t("audio.sectionEditor.eyebrow"))}</p><h2>${esc(t("audio.sectionEditor.title"))}</h2><p class="scene-modal-note">${esc(t("audio.sectionEditor.body"))}</p><div class="section-editor-rows">${rows}</div><div><button type="button" id="cancelModal" class="save-button">${esc(t("audio.sectionEditor.done"))}</button></div></section></div>`;
  $("#closeModal").onclick = closeModal; $("#cancelModal").onclick = closeModal;
  const snap = (t) => snapToBeat(t, song, "bar");
  $("#modalRoot").querySelectorAll("[data-section-label]").forEach((el) => el.addEventListener("change", () => {
    commitSectionEdit("meta", el.dataset.sectionLabel, editSectionMeta(song.sections, el.dataset.sectionLabel, { label: el.value }));
  }));
  $("#modalRoot").querySelectorAll("[data-section-kind]").forEach((el) => el.addEventListener("change", () => {
    commitSectionEdit("meta", el.dataset.sectionKind, editSectionMeta(song.sections, el.dataset.sectionKind, { kind: el.value }));
  }));
  $("#modalRoot").querySelectorAll("[data-section-boundary]").forEach((el) => el.addEventListener("change", () => {
    const time = Number(el.value);
    if (!Number.isFinite(time)) return notify(t("audio.sectionEditor.enterTimeError"));
    try { commitSectionEdit("boundary", el.dataset.sectionBoundary, setSectionBoundary(song.sections, el.dataset.sectionBoundary, snap(time))); }
    catch (error) { notify(error.message); }
  }));
  $("#modalRoot").querySelectorAll("[data-section-merge]").forEach((el) => el.addEventListener("click", () => {
    try { commitSectionEdit("merge", el.dataset.sectionMerge, mergeSectionWithNext(song.sections, el.dataset.sectionMerge)); }
    catch (error) { notify(error.message); }
  }));
  $("#modalRoot").querySelectorAll("[data-section-split]").forEach((el) => el.addEventListener("click", () => {
    const section = song.sections.find((s) => s.id === el.dataset.sectionSplit);
    const raw = Number($(`[data-section-split-at="${CSS.escape(section.id)}"]`)?.value);
    if (!Number.isFinite(raw)) return notify(t("audio.sectionEditor.enterSplitTimeError"));
    const time = snap(raw);
    try { commitSectionEdit("split", section.id, splitSection(song.sections, section.id, time)); notify(t("audio.sectionEditor.splitConfirm", { label: section.label, time: formatTime(time) })); }
    catch (error) { notify(error.message); }
  }));
}
function applySongToProject(songMap, audioAsset, feeling) {
  const plan = sectionShotPlan(songMap);
  project.song = songMap;
  project.songFeeling = feeling || project.songFeeling || "";
  if (!project.name || project.name === "Untitled Film") project.name = songMap.name;
  project.scenes = plan.map((p) => ({
    id: `mv-${p.sectionId}`.slice(0, 60),
    title: p.label,
    note: sectionBrief(songMap.sections.find((s) => s.id === p.sectionId), songMap, project.songFeeling),
    songSectionId: p.sectionId,
    shots: [{
      title: `${p.label} — shot`,
      purpose: `${p.kind} section, ${p.durationSec.toFixed(1)}s at ${songMap.bpm} BPM.`,
      // One decimal so the Timeline's duration sum (which reads this string
      // field) stays close to real; the render is exact via songSectionId.
      duration: String(Math.max(1, Math.round(p.durationSec * 10) / 10))
    }]
  }));
  ensureSceneShape();
  reseedSongScenes();
  project.audioTracks = (project.audioTracks || []).filter((cue) => cue.assetId !== audioAsset.id);
  project.audioTracks.unshift({
    id: `cue-song-${Date.now()}`,
    assetId: audioAsset.id,
    type: "Music",
    sceneId: "",
    shotTitle: "",
    start: "0",
    duration: String(Math.round(songMap.durationSec)),
    level: "0 dB",
    notes: `Master track · ${songMap.bpm} BPM · ${songMap.sections.length} sections`
  });
  ensureProductionShape();
  setDirty();
}
// The "arrival" — analysis lands as a moment, not a toast. Shows what the
// Director heard, lets the user pick a feeling, then commits and drops them
// into the storyboard.
function openSongDirectedReveal(songMap, audioAsset) {
  const strip = songMap.sections.map((s) => `<span class="song-map-chip" style="--sec:${sectionColor(s.kind)}"><b>${esc(s.label)}</b><i>${formatTime(Math.max(0, s.end - s.start))}</i></span>`).join("");
  const feelings = MUSIC_VIDEO_FEELINGS.map((f) => `<label class="feeling-choice"><input type="radio" name="mvFeeling" value="${f.id}" ${project.songFeeling === f.id ? "checked" : ""} /><span><b>${esc(feelingLabel(f.id))}</b><small>${esc(feelingHint(f.id))}</small></span></label>`).join("");
  const meta = `${t("audio.reveal.metaPrefix", { bpm: songMap.bpm, duration: formatTime(songMap.durationSec) })}${t("audio.sectionsCount", { count: songMap.sections.length })}${t("audio.reveal.metaSuffix")}`;
  $("#modalRoot").innerHTML = `<div class="modal-backdrop"><section class="project-modal wide-modal song-reveal"><button type="button" id="closeModal">×</button><p>${esc(t("audio.reveal.eyebrow"))}</p><h2>${esc(songMap.name)}</h2><p class="song-reveal-meta">${esc(meta)}</p><div class="song-map-sections">${strip}</div><div class="feeling-picker"><small>${esc(t("audio.feelingPicker.label"))} <em>${esc(t("audio.feelingPicker.hint"))}</em></small><div class="feeling-choices">${feelings}</div></div><div><button type="button" id="cancelModal" class="quiet-button">${esc(t("audio.reveal.notNow"))}</button><button type="button" id="songRevealDirect" class="save-button">${esc(t("audio.reveal.directButton"))}</button></div></section></div>`;
  $("#closeModal").onclick = closeModal;
  $("#cancelModal").onclick = closeModal;
  $("#songRevealDirect").addEventListener("click", () => {
    const feeling = $("input[name='mvFeeling']:checked")?.value || "";
    applySongToProject(songMap, audioAsset, feeling);
    closeModal();
    active = "Storyboard";
    render();
    notify(`${t("audio.reveal.confirmPrefix", { name: songMap.name })}${feeling ? t("audio.reveal.confirmFeelingSuffix", { feeling: feelingLabel(feeling) }) : ""}${t("audio.reveal.confirmSuffix", { count: songMap.sections.length })}`);
  });
}
async function directAsMusicVideo() {
  if (!window.storyMakerDesktop?.readMediaBytes) return notify("Directing to a song is available in the packaged Windows app.");
  const directed = Boolean(project.song);
  const hasScenes = project.scenes.length > 0;
  if (directed) {
    // Rebuilding the section list wholesale replaces project.scenes
    // (applySongToProject) — a new song's sections don't overlap the old
    // ones, so nothing carries over. Spell out what's actually at stake
    // instead of a generic "rebuild the section list?", so this isn't
    // discovered after the fact.
    const outputCount = project.scenes.filter((scene) => scene.shots.some((shot) => shot.outputAssetId)).length;
    const castCount = project.scenes.filter((scene) => scene.sectionCast?.leadId || scene.sectionCast?.backupIds?.length || scene.sectionCast?.dancerIds?.length || scene.choreography).length;
    const risks = [
      outputCount && `${outputCount} generated shot${outputCount === 1 ? "" : "s"}`,
      castCount && `${castCount} section${castCount === 1 ? "" : "s"} with cast or choreography assigned`,
      project.songDirection && "the AI director's treatment"
    ].filter(Boolean);
    const warning = risks.length ? ` This discards ${risks.join(", ")} — none of it carries over to the new song.` : "";
    if (!confirm(`This project is already directed to "${project.song.name}". Import a new song and rebuild the section list?${warning}`)) return;
  }
  if (!directed && hasScenes && !confirm(`Directing to a song replaces the current ${project.scenes.length} scene${project.scenes.length === 1 ? "" : "s"} with one scene per song section. Continue?`)) return;
  try {
    const additions = await importAssetsIntoProject();
    const audio = additions.find((asset) => asset.kind === "audio");
    if (!audio) return notify(additions.length ? "That file isn't a supported audio format." : "No audio file was selected.");
    if (!audio.path) return notify("The imported song has no local path to analyze.");
    notify("Analysing the song…");
    const raw = await window.storyMakerDesktop.readMediaBytes(audio.path);
    const bytes = raw.buffer.slice(raw.byteOffset, raw.byteOffset + raw.byteLength);
    const songMap = await analyzeAudioArrayBuffer(bytes, { name: audio.name.replace(/\.[^.]+$/, ""), fileName: audio.name });
    openSongDirectedReveal(songMap, audio);
  } catch (error) {
    notify(error?.message?.replace(/^Error invoking remote method '[^']+':\s*(?:Error:\s*)?/, "") || "The song could not be analysed.");
  }
}
function audioStudioWorkspace() {
  ...
  const song = project.song;
  const lyricCount = (song?.lyrics || []).length;
  const dir = project.songDirection;
  const directionBlock = dir ? `<div class="song-direction"><small>${esc(t("audio.panel.aiDirectionEyebrow"))}${dir.provider ? ` · ${esc(dir.provider)}` : ""}</small>${dir.logline ? `<p class="song-direction-logline">${esc(dir.logline)}</p>` : ""}${dir.energyArc ? `<p><b>${esc(t("audio.panel.energyArcLabel"))}</b> ${esc(dir.energyArc)}</p>` : ""}${dir.visualApproach ? `<p><b>${esc(t("audio.panel.visualApproachLabel"))}</b> ${esc(dir.visualApproach)}</p>` : ""}</div>` : "";
  const panelSummary = `${t("audio.reveal.metaPrefix", { bpm: song?.bpm, duration: formatTime(song?.durationSec || 0) })}${song ? t("audio.sectionsCount", { count: song.sections.length }) : ""}${project.songFeeling ? t("audio.panel.feelingSuffix", { feeling: feelingLabel(project.songFeeling) }) : ""}${lyricCount ? t("audio.panel.lyricsSuffix", { count: lyricCount }) : ""}${t("audio.panel.summarySuffix")}`;
  // Every generated frame/video that already exists was rendered under
  // whichever feeling was active at the time — changing the dropdown below
  // updates every section's text brief immediately (reseedSongScenes, in
  // its own change handler) but never repaints pixels already made under
  // the old one. This is the companion action for that gap.
  const rerollFeelingCount = project.songFeeling ? rerollCandidates(null).length : 0;
  const songPanel = song ? `<section class="song-map-panel"><header><div><small>${esc(t("audio.panel.eyebrow"))}</small><h2>${esc(song.name)}</h2><p>${esc(panelSummary)}</p></div><div class="song-map-actions"><button id="directMusicVideoAI" class="save-button" title="${esc(t("audio.panel.directAITitle"))}">${dir ? esc(t("audio.panel.redirectAI")) : esc(t("audio.panel.directAI"))}</button><button id="directMusicVideo" class="quiet-button">${esc(t("audio.panel.reanalyze"))}</button></div></header><div class="song-map-sections">${song.sections.map((s) => `<span class="song-map-chip" style="--sec:${sectionColor(s.kind)}" title="${esc(s.label)} · ${formatTime(s.start)}–${formatTime(s.end)}"><b>${esc(s.label)}</b><i>${formatTime(Math.max(0, s.end - s.start))}</i></span>`).join("")}<button id="editSections" class="song-map-chip song-map-chip-edit" title="${esc(t("audio.panel.editSectionsTitle"))}">✎ ${esc(t("audio.panel.editSections"))}</button></div>${directionBlock}<div class="song-map-controls"><div class="song-feeling-row"><label>${esc(t("audio.panel.feelingLabel"))}<select id="songFeelingSelect"><option value="">${esc(t("audio.panel.feelingNone"))}</option>${MUSIC_VIDEO_FEELINGS.map((f) => `<option value="${f.id}" ${project.songFeeling === f.id ? "selected" : ""}>${esc(feelingLabel(f.id))}</option>`).join("")}</select></label>${rerollFeelingCount ? `<button type="button" id="rerollForFeeling" class="quiet-button" ${bulkGenerationActive ? "disabled" : ""} title="Regenerate every unlocked scene's current take with ${esc(feelingLabel(project.songFeeling))}'s direction — a new still or video, replacing what's there now">${bulkGenerationActive ? "Rolling…" : `Re-roll all scenes for ${esc(feelingLabel(project.songFeeling))} (${rerollFeelingCount})`}</button>` : ""}</div><label class="song-lyrics-field">${esc(t("audio.panel.lyricsLabel"))}<textarea id="songLyricsInput" rows="4" placeholder="${esc(t("audio.panel.lyricsPlaceholder"))}">${esc(project.songLyrics || "")}</textarea></label></div></section>` : "";
  return `<div class="audio-workspace"><div class="workspace-heading"><div><p>${esc(t("audio.workspace.eyebrow"))}</p><h1>${esc(t("audio.workspace.title"))}</h1></div><div class="story-actions"><button id="importAudioDirect" class="quiet-button">${esc(t("audio.workspace.importAudio"))}</button>${song ? "" : `<button id="directMusicVideo" class="quiet-button" title="${esc(t("audio.workspace.directMusicVideoTitle"))}">${esc(t("audio.workspace.directMusicVideo"))}</button>`}<button id="addAudioCue" class="save-button" ${assets.length ? "" : "disabled"}>${esc(t("audio.workspace.addCue"))}</button></div></div>${songPanel}<section class="audio-intro"><div><small>${esc(t("audio.intro.eyebrow"))}</small><h2>${esc(t("audio.intro.title"))}</h2><p>${esc(t("audio.intro.body"))}</p></div><div><b>${assets.length}</b><span>${esc(t("audio.assetCount", { count: assets.length }))}</span><b>${cues.length}</b><span>${esc(t("audio.cueCount", { count: cues.length }))}</span></div></section><section class="audio-workflow"><span><b>1</b> ${esc(t("audio.workflow.step1"))}</span><i>→</i><span><b>2</b> ${esc(t("audio.workflow.step2"))}</span><i>→</i><span><b>3</b> ${esc(t("audio.workflow.step3"))}</span><i>→</i><span><b>4</b> ${esc(t("audio.workflow.step4"))}</span></section><section class="audio-cue-list">${cues.length ? cues.map((cue, index) => { const asset = assetById(cue.assetId); const scene = project.scenes.find((item) => item.id === cue.sceneId); return `<article><i>${String(index + 1).padStart(2, "0")}</i><div><small>${esc(cue.type)} · ${esc(scene?.title || t("audio.cue.globalScene"))}</small><h3>${esc(asset?.name || t("audio.cue.missingAsset"))}</h3><p>${esc(cue.notes || t("audio.cue.noNote"))}</p></div><div class="audio-cue-time"><span>${esc(t("audio.cue.start"))} <b>${esc(cue.start)}s</b></span><span>${esc(t("audio.cue.duration"))} <b>${esc(cue.duration)}s</b></span><span>${esc(t("audio.cue.level"))} <b>${esc(cue.level)}</b></span></div><div class="audio-preview">${asset ? `<audio controls preload="metadata" src="${esc(fileUrl(asset))}"></audio>` : ""}<button data-edit-audio-cue="${esc(cue.id)}" class="quiet-button">${esc(t("audio.cue.editTiming"))}</button><button data-remove-audio-cue="${esc(cue.id)}" class="remove-button">${esc(t("audio.cue.remove"))}</button></div></article>`; }).join("") : `<div class="audio-empty"><i>♫</i><h2>${assets.length ? esc(t("audio.empty.readyTitle")) : esc(t("audio.empty.buildTitle"))}</h2><p>${assets.length ? esc(t("audio.empty.readyBody")) : esc(t("audio.empty.buildBody"))}</p><div class="audio-empty-actions"><button ${assets.length ? 'id="addFirstAudioCue"' : 'id="importFirstAudioDirect"'} class="save-button">${assets.length ? esc(t("audio.empty.addFirstCue")) : esc(t("audio.empty.importFile"))}</button><button data-nav="Timeline" class="quiet-button">${esc(t("audio.empty.openTimeline"))}</button></div></div>`}</section></div>`;
}
async function exportProductionPackage() {
  if (!window.storyMakerDesktop?.exportProductionPackage) return notify(t("deliver.notify.packageWebOnly"));
  const button = $("#exportProductionPackage"); if (button) { button.disabled = true; button.textContent = t("deliver.notify.exporting"); }
  try { const result = await window.storyMakerDesktop.exportProductionPackage({ project }); if (!result || result.canceled) return; notify(t("deliver.notify.packageExported", { path: result.folderPath })); }
  catch (error) { notify(error?.message || t("deliver.notify.packageFailed")); }
  finally { if (button) { button.disabled = false; button.textContent = t("deliver.exportPackage"); } }
}
async function exportLyrics() {
  if (!window.storyMakerDesktop?.exportLyrics) return notify(t("deliver.notify.lyricsWebOnly"));
  const button = $("#exportLyrics"); const label = button?.textContent;
  if (button) { button.disabled = true; button.textContent = t("deliver.notify.exporting"); }
  try {
    const result = await window.storyMakerDesktop.exportLyrics({ project });
    if (!result || result.canceled) return;
    notify(t("deliver.notify.lyricsExported", { count: result.lines, path: result.filePath }));
  } catch (error) { notify(error?.message || t("deliver.notify.lyricsFailed")); }
  finally { if (button) { button.disabled = false; button.textContent = label || t("deliver.mv.exportLyricsButton"); } }
}
async function exportEditTimeline() {
  if (!window.storyMakerDesktop?.exportEditTimeline) return notify(t("deliver.notify.editWebOnly"));
  const button = $("#exportEditTimeline"); if (button) { button.disabled = true; button.textContent = t("deliver.notify.exporting"); }
  try {
    const result = await window.storyMakerDesktop.exportEditTimeline({ project });
    if (!result || result.canceled) return;
    notify(`${t("deliver.notify.editExported", { count: result.clips })}${result.audioCues ? t("deliver.notify.andAudioCues", { count: result.audioCues }) : ""}${result.sectionMarkers ? t("deliver.notify.andSectionMarkers", { count: result.sectionMarkers }) : ""}: ${result.filePath}${result.proLocked ? t("deliver.notify.proLockedMarkers") : ""}`);
  } catch (error) { notify(error?.message || t("deliver.notify.editFailed")); }
  finally { if (button) { button.disabled = false; button.textContent = t("deliver.editor.exportButton"); } }
}
async function sendShotToResolve(sceneIndex, shotIndex, button) {
  if (!window.storyMakerDesktop?.sendShotToResolve) return notify(t("deliver.notify.resolveWebOnly"));
  const scene = project.scenes[sceneIndex]; const shot = scene?.shots?.[shotIndex];
  const output = shot ? assetById(shot.outputAssetId) : null;
  if (!output?.path) return notify(t("deliver.notify.noOutputToSend"));
  const originalText = button?.textContent;
  if (button) { button.disabled = true; button.textContent = t("deliver.notify.sending"); }
  // A still holds for its planned length; a real clip always plays its own
  // length regardless, so duration only ever applies to an image output —
  // same clamp as visualPreviewEntries() on the Node side, so a shot sent
  // individually gets the exact same duration it would in the bulk edit.
  const duration = output.kind === "image" ? Math.max(0.5, Math.min(60, Number(shot?.duration) || 4)) : null;
  try {
    const result = await window.storyMakerDesktop.sendShotToResolve({ filePath: output.path, binName: project.name, appendToTimeline: true, duration });
    const where = result.appendedToTimeline ? t("deliver.notify.binAndTimeline", { bin: result.binName }) : t("deliver.notify.binOnly", { bin: result.binName });
    notify(`${result.alreadyInBin ? t("deliver.notify.alreadyIn") : t("deliver.notify.sentTo")} ${where}${result.note ? ` ${result.note}` : ""}`);
  } catch (error) { notify(error?.message || t("deliver.notify.resolveShotFailed")); }
  finally { if (button) { button.disabled = false; button.textContent = originalText || t("deliver.preflight.sendToResolve"); } }
}
async function sendEditToResolve() {
  if (!window.storyMakerDesktop?.sendEditToResolve) return notify(t("deliver.notify.resolveWebOnly"));
  const button = $("#sendEditToResolve"); if (button) { button.disabled = true; button.textContent = t("deliver.notify.sending"); }
  try {
    const result = await window.storyMakerDesktop.sendEditToResolve({ project, appendToTimeline: true });
    const sentCount = (result.imported || 0) + (result.reused || 0);
    const where = result.appendedToTimeline ? t("deliver.notify.binAndAppended", { bin: result.binName }) : t("deliver.notify.binOnly", { bin: result.binName });
    notify(`${t("deliver.notify.sentShots", { count: sentCount })} ${where}${result.note ? ` ${result.note}` : ""}`);
  } catch (error) { notify(error?.message || t("deliver.notify.resolveEditFailed")); }
  finally { if (button) { button.disabled = false; button.textContent = t("deliver.editor.sendToResolveButton"); } }
}
...
async function renderVisualPreview() {
  if (!window.storyMakerDesktop?.renderVisualPreview) return notify(t("deliver.notify.previewWebOnly"));
  const button = $("#renderVisualPreview"); if (button) { button.disabled = true; button.textContent = t("deliver.notify.assembling"); }
  try {
    const result = await window.storyMakerDesktop.renderVisualPreview({ project });
    if (!result || result.canceled) return;
    notify(`${t("deliver.notify.roughCutSaved", { count: result.clips || t("deliver.notify.selectedFallback") })}${result.audioCues ? t("deliver.notify.andAudioCues", { count: result.audioCues }) : ""}${result.chapters ? t("deliver.notify.andChapters", { count: result.chapters }) : ""}: ${result.filePath}${result.proLocked ? t("deliver.notify.proLockedChapters") : ""}`);
  } catch (error) { notify(error?.message || t("deliver.notify.previewFailed")); }
  finally { if (button) { button.disabled = false; button.textContent = t("deliver.assemble.button"); } }
}
function shotDeliveryState(shot) {
  ...
  if (output) return { tone: shot.outputReview === "approved" ? "approved" : shot.outputReview === "revision" ? "revision" : "review", label: shot.outputReview === "approved" ? t("deliver.state.approved") : shot.outputReview === "revision" ? t("deliver.state.needsRevision") : t("deliver.state.readyToReview"), detail: t("deliver.state.outputDetail", { kind: output.kind || t("deliver.state.mediaFallback"), name: output.name }) };
  if (latest?.status === "failed") return { tone: "failed", label: t("deliver.state.renderFailed"), detail: latest.error || t("deliver.state.retryDetail") };
  if (["submitting", "queued", "generating", "processing", "running"].includes(latest?.status)) return { tone: "queued", label: t("deliver.state.inProduction"), detail: `${latest.provider || t("deliver.state.providerFallback")} · ${latest.model || t("deliver.state.modelFallback")}` };
  if (!shot.modelSettings?.prompt?.trim()) return { tone: "needs", label: t("deliver.state.needsPrompt"), detail: t("deliver.state.addPromptDetail") };
  return { tone: "planned", label: t("deliver.state.readyToRender"), detail: `${shot.modelSettings?.model || t("deliver.state.chooseModel")} · ${shot.modelSettings?.aspectRatio || t("deliver.state.frameUnset")}` };
}
...
// Rough per-generation USD estimates, not exact billing. Real provider
// pricing varies by resolution, quality tier, and duration, and shifts
// often (GPT Image 2.5 alone launched mid-session at roughly 2x GPT Image
// 2's per-token rate) — tracking every model's exact current price here
// would be stale within weeks. This exists so a project carries SOME
// running cost signal instead of none, clearly labeled as an estimate
// everywhere it's shown, with a per-model breakdown so a user can sanity
// check or mentally correct it rather than trust one blended number.
// ComfyUI is the one case with a real, not estimated, answer: it's your
// own GPU, so it's genuinely $0 regardless of model.
const GENERATION_COST_ESTIMATES = {
  // OpenAI — verified against developers.openai.com pricing this session.
  // gpt-image-1: official range ~$0.02-$0.19/image by quality+size; using a
  // medium-quality 1024x1024 point. gpt-image-2.5 (Flare/Sunburst): token
  // pricing is $8/1M image-input, $30/1M image-output — a single output
  // image is roughly 1000-1500 tokens, so ~$0.03-$0.045; both share a rate
  // per OpenAI's own docs page for these two models.
  "openai:image": 0.04,
  "openai:gpt-image-1:image": 0.03,
  "openai:gpt-image-2.5-flare:image": 0.04,
  "openai:gpt-image-2.5-sunburst:image": 0.04,
  // Google's Gemini image models are token-billed at a lower rate than
  // OpenAI's per public pricing pages; kept as one rough figure rather than
  // a false-precision per-model split.
  "google:image": 0.02,
  // fal/Kie/WaveSpeed are gateways in front of many underlying vendor
  // models with their own individual pricing (30+ WaveSpeed image/video
  // slugs alone) — one flat per-provider, per-kind figure rather than
  // chasing exact numbers for each that would drift immediately.
  "fal:image": 0.03,
  "fal:video": 1.5,
  "kie:image": 0.04,
  "kie:video": 1.2,
  "wavespeed:image": 0.03,
  "wavespeed:video": 1.2
};
function estimateGenerationCost(generation, kind) {
  const provider = String(generation?.provider || "");
  if (!provider || provider === "comfyui" || generation?.local === true) return 0;
  const model = String(generation?.model || "");
  const upscaleFactor = Number(generation?.upscale?.factor || 1);
  const base = GENERATION_COST_ESTIMATES[`${provider}:${model}:${kind}`] ?? GENERATION_COST_ESTIMATES[`${provider}:${kind}`];
  if (base == null) return null; // No estimate for this provider/kind combination — shown as "not tracked", never silently as $0.
  // A delivery upscale (fal Topaz pass) is itself a second billed request;
  // approximate its added cost as proportional to the upscale factor rather
  // than ignoring it.
  return base * (upscaleFactor > 1 ? 1 + upscaleFactor * 0.15 : 1);
}
// Only assets with a `generation` object were actually AI-generated —
// imported media (references, uploaded footage) carries no generation
// metadata and correctly costs nothing here.
function projectCostSummary(sourceProject = project) {
  const rows = new Map(); // "provider:model" -> { provider, model, count, total, untracked }
  let total = 0; let untrackedCount = 0; let trackedCount = 0;
  (sourceProject.assets || []).forEach((asset) => {
    const generation = asset?.generation;
    if (!generation?.provider) return;
    const kind = asset.kind === "video" ? "video" : "image";
    const cost = estimateGenerationCost(generation, kind);
    const key = `${generation.provider}:${generation.model || "unknown"}`;
    const row = rows.get(key) || { provider: generation.provider, model: generation.model || "", count: 0, total: 0, untracked: false };
    row.count += 1;
    if (cost == null) { row.untracked = true; untrackedCount += 1; } else { row.total += cost; total += cost; trackedCount += 1; }
    rows.set(key, row);
  });
  return { total, trackedCount, untrackedCount, rows: [...rows.values()].sort((a, b) => b.total - a.total || b.count - a.count) };
}
function deliveryWorkspace() {
  ...
  const mv = songSyncReport(project);
  // Chapter markers, embedded chapters, lyric captions, and the section CSV
  // are Pro-tier delivery polish (see electron-main.js's assertProTier).
  // licenseState.tier defaults to "pro" whenever licensing is unconfigured
  // or no Pro product exists yet, so this reads as fully unlocked until the
  // owner deliberately sets one up — same "wired but inert" posture as
  // every other not-yet-configured integration in this app.
  const isPro = licenseState.tier === "pro";
  const mvSyncLine = mv ? (mv.ok
    ? `${t("deliver.mv.synced", { count: mv.sectionCount })}${t("deliver.mv.coverageSuffix", { duration: formatTime(mv.durationSec) })}${isPro ? t("deliver.mv.chapterPro") : t("deliver.mv.chapterLocked")}`
    : t("deliver.mv.missing", { count: mv.missing.length, names: mv.missing.map((section) => esc(section.label || section.kind)).join(", ") })) : "";
  const mvPanel = mv ? `<section class="delivery-mv-panel${isPro ? "" : " delivery-mv-panel-locked"}"><div class="delivery-mv-head"><small>${esc(t("deliver.mv.eyebrow"))}${isPro ? "" : esc(t("deliver.mv.proLockedSuffix"))}</small><h2>${esc(mv.name || t("deliver.mv.untitledSong"))} · ${mv.bpm ? esc(t("deliver.mv.bpmPrefix", { bpm: Math.round(mv.bpm) })) : ""}${formatTime(mv.durationSec)}</h2><p>${mvSyncLine}</p></div><div class="delivery-mv-actions"><span>${esc(t("deliver.mv.lyricLineCount", { count: mv.lyricsCount }))}</span><button id="exportLyrics" class="quiet-button" ${(mv.lyricsCount && isPro) ? "" : "disabled"} title="${!isPro ? esc(t("deliver.mv.exportLyricsTitleLocked")) : mv.lyricsCount ? esc(t("deliver.mv.exportLyricsTitleReady")) : esc(t("deliver.mv.exportLyricsTitleEmpty"))}">${isPro ? esc(t("deliver.mv.exportLyricsButton")) : esc(t("deliver.mv.exportLyricsButtonLocked"))}</button></div></section>` : "";
  const costPanel = cost.trackedCount || cost.untrackedCount ? `<section class="delivery-cost"><div class="delivery-cost-head"><div><small>${esc(t("deliver.cost.eyebrow"))}</small><h2>${esc(t("deliver.cost.title", { total: formatCurrencyUSD(cost.total), count: cost.trackedCount + cost.untrackedCount }))}</h2><p>${esc(t("deliver.cost.body"))}${cost.untrackedCount ? esc(t("deliver.cost.untrackedSuffix", { count: cost.untrackedCount })) : ""}</p></div><div class="delivery-cost-total"><b>${esc(formatCurrencyUSD(cost.total))}</b><span>${esc(t("deliver.cost.estimatedLabel"))}</span></div></div><div class="delivery-cost-rows">${cost.rows.map((row) => `<div class="delivery-cost-row"><span>${esc(providerLabel(row.provider))}${row.model ? ` · ${esc(row.model)}` : ""}</span><b>${row.count}×</b><em>${row.untracked ? esc(t("deliver.cost.notTracked")) : esc(formatCurrencyUSD(row.total))}</em></div>`).join("")}</div></section>` : "";
  return `<div class="delivery-workspace"><div class="workspace-heading"><div><p>${esc(t("deliver.heading.eyebrow"))}</p><h1>${esc(t("deliver.heading.title"))}</h1></div><button id="exportProductionPackage" class="save-button">${esc(t("deliver.exportPackage"))}</button></div>${mvPanel}<section class="delivery-hero"><div><small>${esc(t("deliver.hero.eyebrow"))}</small><h2>${project.name === "Untitled Film" ? esc(t("deliver.hero.nameProduction")) : esc(project.name)}</h2><p>${esc(t("deliver.hero.body"))}</p></div><div class="delivery-counts"><span><b>${project.scenes.length}</b> ${esc(t("deliver.counts.scenes", { count: project.scenes.length }))}</span><span><b>${shotCount}</b> ${esc(t("deliver.counts.shots", { count: shotCount }))}</span><span><b>${finished}</b> ${esc(t("deliver.counts.outputs", { count: finished }))}</span><span><b>${project.audioTracks.length}</b> ${esc(t("deliver.counts.audioCues", { count: project.audioTracks.length }))}</span></div></section><section class="delivery-contents"><article><i>01</i><h3>${esc(t("deliver.contents.projectRecord.title"))}</h3><p>${t("deliver.contents.projectRecord.body")}</p></article><article><i>02</i><h3>${esc(t("deliver.contents.shotList.title"))}</h3><p>${t("deliver.contents.shotList.body")}</p></article><article><i>03</i><h3>${esc(t("deliver.contents.audioCueSheet.title"))}</h3><p>${t("deliver.contents.audioCueSheet.body")}</p></article><article><i>04</i><h3>${esc(t("deliver.contents.notes.title"))}</h3><p>${t("deliver.contents.notes.body")}</p></article>${mv && isPro ? `<article><i>05</i><h3>${esc(t("deliver.contents.sectionCsv.title"))}</h3><p>${t("deliver.contents.sectionCsv.body")}</p></article><article><i>06</i><h3>${esc(t("deliver.contents.lyricsSrt.title"))}</h3><p>${t("deliver.contents.lyricsSrt.body")}</p></article>` : mv ? `<article class="delivery-contents-locked"><i>05</i><h3>${esc(t("deliver.contents.lockedTitle"))}</h3><p>${t("deliver.contents.lockedBody")}</p></article>` : ""}</section><section class="delivery-preflight"><div class="delivery-preflight-head"><div><small>${esc(t("deliver.preflight.eyebrow"))}</small><h2>${esc(t("deliver.preflight.title"))}</h2><p>${esc(t("deliver.preflight.body"))}</p></div><div class="delivery-preflight-metrics"><span><b>${reviewed}</b> ${esc(t("deliver.preflight.approved"))}</span><span><b>${activeJobs}</b> ${esc(t("deliver.preflight.rendering"))}</span><span class="${blockers ? "attention" : ""}"><b>${blockers}</b> ${esc(t("deliver.preflight.attention"))}</span></div></div>${audit.length ? `<div class="delivery-audit-list">${audit.map(({ scene, sceneIndex, shot, shotIndex, state }, index) => `<article class="delivery-audit-row delivery-${state.tone}"><i>${String(index + 1).padStart(2, "0")}</i><div class="delivery-audit-copy"><small>${esc(scene.title)} · ${esc(shot.modelSettings?.model || t("deliver.preflight.noModel"))}</small><h3>${esc(shot.title)}</h3><p>${esc(state.detail)}</p></div><div class="delivery-audit-status"><b>${esc(state.label)}</b><span>${esc(shot.modelSettings?.aspectRatio || t("deliver.preflight.frameUnset"))} · ${shot.modelSettings?.referenceAssetIds?.length || 0} ${esc(t("deliver.preflight.localRef", { count: shot.modelSettings?.referenceAssetIds?.length || 0 }))}</span></div><div class="delivery-audit-actions"><button class="quiet-button" data-direct-shot="${sceneIndex}:${shotIndex}">${esc(t("deliver.preflight.direct"))}</button>${shot.outputAssetId ? `<button class="line-action" data-review-delivery-shot="${sceneIndex}:${shotIndex}">${esc(t("deliver.preflight.review"))}</button>${state.tone !== "approved" ? `<button class="line-action" data-approve-delivery-shot="${sceneIndex}:${shotIndex}">${esc(t("deliver.preflight.approve"))}</button>` : ""}<button class="quiet-button" data-send-to-resolve="${sceneIndex}:${shotIndex}" title="${esc(t("deliver.preflight.sendToResolveTitle"))}">${esc(t("deliver.preflight.sendToResolve"))}</button>` : ""}</div></article>`).join("")}</div>` : `<div class="delivery-audit-empty"><h3>${esc(t("deliver.preflight.emptyTitle"))}</h3><p>${esc(t("deliver.preflight.emptyBody"))}</p><button data-nav="Shot Planner" class="quiet-button">${esc(t("deliver.preflight.openShotPlanner"))}</button></div>`}</section>${costPanel}<section class="delivery-render"><div><small>${esc(t("deliver.assemble.eyebrow"))}</small><h2>${esc(t("deliver.assemble.title"))}</h2><p>${deliveryState.previewRender ? (approved ? `${esc(t("deliver.assemble.readyBody"))}${mv ? esc(isPro ? t("deliver.assemble.chapterPro") : t("deliver.assemble.chapterLocked")) : ""}` : esc(t("deliver.assemble.notReadyBody"))) : esc(t("deliver.assemble.noFfmpeg"))}</p></div><div><span class="delivery-render-status ${deliveryState.previewRender ? "ready" : ""}">${deliveryState.previewRender ? esc(t("deliver.assemble.statusReady")) : esc(t("deliver.assemble.statusUnavailable"))}</span><button id="renderVisualPreview" class="quiet-button" ${canRender ? "" : "disabled"}>${esc(t("deliver.assemble.button"))}</button></div></section><section class="delivery-render"><div><small>${esc(t("deliver.editor.eyebrow"))}</small><h2>${esc(t("deliver.editor.title"))}</h2><p>${approved ? `${esc(t("deliver.editor.readyBody"))}${mv ? esc(isPro ? t("deliver.editor.markerPro") : t("deliver.editor.markerLocked")) : ""}` : esc(t("deliver.editor.notReadyBody"))}</p></div><div><span class="delivery-render-status ${approved ? "ready" : ""}">${approved ? esc(t("deliver.editor.statusReady")) : esc(t("deliver.editor.statusNotReady"))}</span><button id="exportEditTimeline" class="quiet-button" ${approved ? "" : "disabled"}>${esc(t("deliver.editor.exportButton"))}</button><button id="sendEditToResolve" class="quiet-button" ${approved ? "" : "disabled"} title="${esc(t("deliver.editor.sendToResolveTitle"))}">${esc(t("deliver.editor.sendToResolveButton"))}</button></div></section><section class="delivery-next"><div><small>${esc(t("deliver.next.eyebrow"))}</small><h2>${esc(t("deliver.next.title"))}</h2><p>${esc(t("deliver.next.body"))}</p></div><button data-nav="Model Hub" class="quiet-button">${esc(t("deliver.next.reviewProviders"))}</button></section></div>`;
}
...
function approveDeliveryShot(sceneIndex, shotIndex) {
  ...
  shot.locked = true; // approving a shot is a director's final word on it — bulk actions must never touch it again
...
function aiWorkspace() {
  ...
  return `<div class="director-workspace"><div class="workspace-heading"><div><p>AI DIRECTOR / STORY REVIEW</p><h1>Make the next decision the right one.</h1></div><span class="ai-online">${openAiReady ? "● LIVE OPENAI READY" : "○ LOCAL DIRECTOR"}</span></div><section class="director-context"><div><small>CONNECTED STORY CONTEXT</small><h2>${project.logline ? esc(project.logline) : "The director is waiting for the first story signal."}</h2><div class="director-context-tags"><span>${project.style || "No visual language"}</span><span>${project.themes?.length ? project.themes.join(" · ") : "No themes"}</span><span>${project.locations?.length || 0} location${project.locations?.length === 1 ? "" : "s"}</span><span>${project.characters?.length || 0} character${project.characters?.length === 1 ? "" : "s"}</span></div></div><aside><strong>${openAiReady ? "A live Director review is ready." : "Private offline analysis is active."}</strong><p>${openAiReady ? "When you run a review, Storymaker sends this current project record to OpenAI and saves the structured direction inside your project." : providers.length ? "Other provider connections are saved. The first live Director adapter currently uses OpenAI; local suggestions below remain available." : "Suggestions below are generated locally from your current project record."}</p>${openAiReady ? `<button id="requestLiveDirectorReview" class="save-button">Run live director review</button>` : `<button data-nav="Model Hub" class="line-action">Connect OpenAI in Model Hub →</button>`}</aside></section>${reviewPanel}${influencePanel}${suggestionsPanel}<section class="director-decision-list">${suggestions.map((suggestion, index) => `<article><i>${String(index + 1).padStart(2, "0")}</i><div><small>${suggestion.signal}</small><h3>${suggestion.title}</h3><p>${suggestion.copy}</p></div>${suggestion.target === "build" ? `<button id="buildStoryMap" class="save-button">Build story map</button>` : `<button data-nav="${suggestion.target}" class="quiet-button">Open</button>`}</article>`).join("")}</section></div>`;
}
...
function locationsWorkspace() {
  ...
  const cards = project.sets.length ? project.sets.map((set, index) => {
    ...
    return entityCardMarkup({ index, name: set.name, eyebrow: set.mood || "Environment", description: set.description, emptyDescription: "Define this place once, then its approved reference follows it into every assigned scene.", referenceCount: entityReferenceRoles(set).size, sceneCount, image, editAttr: "edit-set", editLabel: "Edit location profile →", labAttr: "open-set-lab", labLabel: "Open Environment Lab", removeAttr: "remove-set", cardClass: "world-asset-card location-card", assignAttr: "assign-set", justAdded: consumeHighlight(set.id) });
...
function storyboardProductionWorkspace() {
  ...
  const pendingGeneration = bulkGenerationCandidates().length;
  const pendingVideo = bulkVideoCandidates().length;
  const rerollKindOptions = sectionKindsPresent().map((kind) => ({ kind, count: rerollCandidates(kind).length })).filter((item) => item.count > 0);
  ...
  ensureSetShape();
  const setCards = project.sets.length ? project.sets.map((set, index) => { const reference = assetById(set.referenceAssetId); const image = reference?.kind === "image" ? fileUrl(reference) : ""; const sceneCount = project.scenes.filter((scene) => (scene.setIds || []).includes(set.id)).length; const referenceCount = entityReferenceRoles(set).size; return entityCardMarkup({ index, name: set.name, eyebrow: set.mood || "", description: set.description, emptyDescription: "Give this location a description so continuity and generation stay grounded.", referenceCount, sceneCount, image, editAttr: "edit-set", editLabel: "Edit location profile →", labAttr: "open-set-lab", labLabel: "Open Set Lab", removeAttr: "remove-set", justAdded: consumeHighlight(set.id) }); }).join("") : `<div class="empty-stage wide"><span>✦</span><h2>No locations yet.</h2><p>Add the first location this story returns to.</p><button id="addFirstSet" class="line-action">Add the first location →</button></div>`;
  ...
  return `<div class="board-workspace"><div class="workspace-heading"><div><p>STORYBOARD / GENERATED PRODUCTION BOARD</p><h1>Put the generated film on the wall.</h1></div><div class="story-actions">${unexpandedScenes ? `<button id="populateAllShots" class="quiet-button" title="Break ${unexpandedScenes} scene${unexpandedScenes === 1 ? "" : "s"} into a full shot list with camera, lighting, performance, and audio for every shot">Populate all shots (${unexpandedScenes})</button>` : ""}${pendingGeneration ? `<button id="bulkGenerateScenes" class="quiet-button" ${bulkGenerationActive ? "disabled" : ""} title="Generate a still frame for every scene that doesn't have one yet — one at a time, with a cost estimate first">${bulkGenerationActive ? "Rolling…" : `Generate everything (${pendingGeneration})`}</button>` : ""}${pendingVideo ? `<button id="bulkGenerateVideos" class="quiet-button" ${bulkGenerationActive ? "disabled" : ""} title="Animate every scene that has a frame but no video yet — a few at a time, with a cost estimate first">${bulkGenerationActive ? "Rolling…" : `Animate everything (${pendingVideo})`}</button>` : ""}${rerollKindOptions.length ? `<span class="reroll-control"><select id="rerollKindSelect" title="Section kind to re-roll">${rerollKindOptions.map(({ kind, count }) => `<option value="${esc(kind)}">${esc(kind)} (${count})</option>`).join("")}</select><button id="rerollSectionKind" class="quiet-button" ${bulkGenerationActive ? "disabled" : ""} title="Regenerate every unlocked scene of this section kind — a new still or video, replacing what's current. Locked and approved shots are always skipped.">${bulkGenerationActive ? "Rolling…" : "Re-roll section"}</button></span>` : ""}<button data-nav="Locations" class="quiet-button">Locations</button><button data-nav="Production Assets" class="quiet-button">Props & accessories</button><button data-nav="Shot Planner" class="quiet-button">Plan shots</button><button data-nav="Media Library" class="quiet-button">Open media library</button><button id="addScene" class="save-button">Add scene</button></div></div><div class="board-canvas">${project.song && !project.songDirection ? `<section class="storyboard-notice mv-direct-notice"><strong>Directed to "${esc(project.song.name)}" — briefs are from a template.</strong><span>Run one AI pass to give every section a real shot, camera, blocking, performance and movement note, cut to the song's structure.</span><button type="button" id="directMusicVideoStoryboard" class="save-button">Direct with AI</button></section>` : ""}<section class="storyboard-notice"><strong>Generated outputs only</strong><span>${isStudio ? "Studio mode: every scene groups its own shots beneath it. Generate a quick scene take, or direct any shot individually — camera, lighting, model, and references, all yours to command." : "Simple mode: every shot is here, grouped under its scene. Hit Generate image or Generate video right on the card — no dials to turn, just direction."}</span></section><div class="beatline"><span>ACT I</span><i></i><span>ACT II</span><i></i><span>ACT III</span></div><div class="scene-board">${project.scenes.length ? sceneBoardContent || `<div class="empty-stage wide"><span>▤</span><h2>No scenes yet. Just possibility.</h2><p>Build the first beat and let the rhythm emerge.</p><button id="addFirstScene" class="line-action">Add first scene →</button></div>` : `<div class="empty-stage wide"><span>▤</span><h2>No scenes yet. Just possibility.</h2><p>Build the first beat and let the rhythm emerge.</p><button id="addFirstScene" class="line-action">Add first scene →</button></div>`}</div></div></div>`;
}
function formatGB(bytes) { return `${(Number(bytes || 0) / 1e9).toFixed(1)} GB`; }
function managedEngineSection() {
  const s = localEngineState;
  const dl = s.download;
  const runtimeLine = s.runtimeFound
    ? `Storymaker will run your ${esc(s.runtimeKind === "comfy-desktop" ? "ComfyUI Desktop" : s.runtimeKind === "portable" ? "portable ComfyUI" : "local ComfyUI")} engine headless — no window, started only when a local shot needs it, stopped when idle.`
    : "No local ComfyUI engine found yet. Install ComfyUI Desktop once (or unzip a portable ComfyUI) and Storymaker will run it for you from then on — you won't need to open it.";
  const engineBadge = s.managed ? "RUNNING (MANAGED)" : s.running ? "RUNNING" : s.runtimeFound ? "READY TO START" : "NOT FOUND";
  const deviceLine = s.device ? `${esc(s.device.name)} · ${formatGB(s.device.vramTotal)} VRAM` : "";
  const cards = (s.catalog || []).map((entry) => {
    const busy = dl && dl.entryId === entry.id;
    const pct = busy && dl.totalBytes ? Math.min(100, Math.round((dl.receivedBytes / dl.totalBytes) * 100)) : 0;
    const state = busy ? `${dl.phase === "verifying" ? "Verifying" : dl.phase === "done" ? "Finishing" : "Downloading"} · ${pct}% (${formatGB(dl.receivedBytes)} / ${formatGB(dl.totalBytes)})${dl.file ? ` · ${esc(dl.file)}` : ""}`
      : entry.installed ? "Installed and ready."
      : !entry.vramOk ? `Needs about ${entry.minVramGB} GB VRAM${entry.vramGB ? ` — this GPU has ${entry.vramGB} GB` : ""}.`
      : `${entry.missingFiles.length} file${entry.missingFiles.length === 1 ? "" : "s"} to download · ${formatGB(entry.totalBytes)} total`;
    const action = busy
      ? `<div class="engine-progress"><div style="width:${pct}%"></div></div><button class="quiet-button" data-engine-cancel>Cancel</button>`
      : entry.installed ? `<span class="engine-ok">✓ Installed</span>`
      : !entry.vramOk ? `<button class="quiet-button" data-engine-download="${entry.id}">Download anyway</button>`
      : `<button class="save-button" data-engine-download="${entry.id}">Download (${formatGB(entry.totalBytes)})</button>`;
    return `<article class="engine-model ${entry.installed ? "installed" : ""}"><div><small>${esc(entry.engine.toUpperCase())} · min ${entry.minVramGB} GB VRAM</small><h3>${esc(entry.label)}</h3><p>${esc(entry.blurb)}</p><span class="engine-model-state">${state}</span></div><div class="engine-model-action">${action}</div></article>`;
  }).join("");
  return `<section class="local-runtime-section managed-engine-section"><header><div><small>MANAGED LOCAL VIDEO ENGINE</small><h2>Local models, without the ComfyUI setup.</h2><p>${esc(runtimeLine)}</p></div><div class="managed-engine-controls"><span class="engine-badge ${s.managed || s.running ? "on" : ""}">${engineBadge}</span>${s.runtimeFound ? (s.managed ? `<button id="stopLocalEngine" class="quiet-button">Stop engine</button>` : `<button id="startLocalEngine" class="quiet-button">Start engine</button>`) : ""}<button id="refreshLocalEngine" class="quiet-button">Refresh</button></div></header>${deviceLine ? `<p class="engine-device">${deviceLine}</p>` : ""}<div class="engine-model-grid">${cards || `<p class="context-empty">Model catalog unavailable.</p>`}</div><p class="engine-fineprint">Weights download from Hugging Face into a folder Storymaker owns, and are shared with any ComfyUI models you already have. Nothing downloads or runs without you choosing it here.</p></section>`;
}
...
function helpWorkspace() {
  ...
  const topics = [
    ...
    ["format", "02", "Format a story"],
    ["cloud", "03", "Connect cloud AI"],
    ["local", "04", "Set up local AI"],
    ["workflow", "05", "Make your first shot"],
    ["styles", "06", "Create custom styles"],
  ...
  const format = `<section class="help-chapter"><header><small>IMPORT · FORMAT</small><h2>You don't need Final Draft.</h2><p>Storymaker's importer reads formatted screenplays, but it also reads treatments, numbered outlines, Fountain and Final Draft files, and plain prose. Nothing you paste is committed until you review what was detected — so the fastest way to learn the format is to paste something and look.</p></header><div class="help-callout"><strong>What the built-in parser looks for</strong><span>Scene headings that start with <code>INT.</code> or <code>EXT.</code>, then a location, then <code>- DAY</code> / <code>- NIGHT</code>. A character's name in CAPITALS on its own line, with their dialogue on the line directly below. A blank line before and after every heading, name, and paragraph. Each short action paragraph becomes one shot.</span></div><div class="help-callout"><strong>No screenplay formatting? Still fine.</strong><span>A treatment, a numbered outline (<code>1. The setup</code>), or plain prose parses too — choose <strong>Improve with AI</strong> on the review screen and it builds a full scene breakdown from any format. The <strong>Format a story</strong> requirements only apply to the offline parser.</span></div><div class="help-format-example"><div><p>A KNOWN-GOOD SAMPLE</p><pre>${esc(SAMPLE_SCRIPT)}</pre><span>Paste this and you get 3 scenes, one character (NADIA), and the lighthouse locations — with each action paragraph lined up as a shot.</span><button type="button" id="helpTrySample" class="save-button">Try this sample in the paste box →</button></div><div><p>A BLANK SKELETON TO FILL IN</p><pre>${esc(SCRIPT_SKELETON)}</pre><span>Replace the CAPITALS and the paragraphs with your own. It's already valid — paste it as-is and edit.</span><button type="button" id="helpCopySkeleton" class="quiet-button">Copy skeleton</button></div></div></section>`;
  ...
  const chapters = { start, format, cloud, local, workflow, styles: stylesHelp, troubleshooting };
...
function updateStatusCopy() {
  switch (updateState.status) {
    ...
    case "downloading": return `Downloading version ${updateState.version}… ${updateState.percent}%`;
    case "downloaded": return `Version ${updateState.version} downloaded — restart to install it.`;
    case "error": return updateState.message || "The update check failed.";
    default: return "Not checked yet this session.";
  }
}
function settingsWorkspace() {
  const licenseBody = !licenseState.configured
    ? esc(t("settings.license.bodyNotRequired"))
    : licenseState.hasKey && licenseState.licensed
      ? `${esc(t("settings.license.activatedLead", { name: licenseState.customerName || t("settings.license.you") }))}${licenseState.activationLimit ? esc(t("settings.license.deviceInfo", { usage: licenseState.activationUsage || 1, limit: licenseState.activationLimit })) : ""}.${licenseState.staleOffline ? esc(t("settings.license.staleNote")) : ""} ${esc(t("settings.license.activatedBody"))}${licenseState.proTierConfigured ? (licenseState.tier === "pro" ? esc(t("settings.license.proUnlocked")) : esc(t("settings.license.proLocked"))) : ""}`
      : esc(t("settings.license.bodyNotActivated"));
  return `<div class="settings-workspace"><div class="workspace-heading"><div><p>${esc(t("settings.heading.eyebrow"))}</p><h1>${esc(t("settings.heading.title"))}</h1></div></div><section class="settings-surface"><div><small>${esc(t("settings.appearance.eyebrow"))}</small><h2>${esc(t("settings.appearance.title"))}</h2><p>${esc(t("settings.appearance.body"))}</p></div><div class="settings-controls"><label>${esc(t("settings.workspaceDepth.label"))}<select id="experienceSelect"><option value="simple" ${experienceMode === "simple" ? "selected" : ""}>${esc(t("settings.workspaceDepth.simple"))}</option><option value="studio" ${experienceMode === "studio" ? "selected" : ""}>${esc(t("settings.workspaceDepth.studio"))}</option></select></label><label>${esc(t("settings.colorMode.label"))}<select id="themeSelect"><option value="dark" ${theme === "dark" ? "selected" : ""}>${esc(t("settings.colorMode.dark"))}</option><option value="light" ${theme === "light" ? "selected" : ""}>${esc(t("settings.colorMode.light"))}</option></select></label><label>${esc(t("settings.navigation.label"))}<select id="navSelect"><option value="full" ${navMode === "full" ? "selected" : ""}>${esc(t("settings.navigation.full"))}</option><option value="compact" ${navMode === "compact" ? "selected" : ""}>${esc(t("settings.navigation.compact"))}</option><option value="icons" ${navMode === "icons" ? "selected" : ""}>${esc(t("settings.navigation.icons"))}</option></select></label><label>${esc(t("settings.language.label"))}<select id="localeSelect">${SUPPORTED_LOCALES.map((entry) => `<option value="${entry.id}" ${locale === entry.id ? "selected" : ""}>${esc(entry.label)}</option>`).join("")}</select></label></div></section><section class="settings-surface"><div><small>${esc(t("settings.updates.eyebrow"))}</small><h2>${appVersion ? esc(t("settings.updates.versionKnown", { version: appVersion })) : esc(t("settings.updates.versionUnknown"))}</h2><p>${esc(updateStatusCopy())}</p></div><div class="settings-controls"><button id="checkForUpdates" type="button" class="quiet-button" ${updateState.status === "checking" || updateState.status === "downloading" ? "disabled" : ""}>${esc(t("settings.updates.check"))}</button>${updateState.status === "available" ? `<button id="downloadUpdateBtn" type="button" class="save-button">${esc(t("settings.updates.download", { version: updateState.version }))}</button>` : ""}${updateState.status === "downloaded" ? `<button id="installUpdateBtn" type="button" class="save-button">${esc(t("settings.updates.install", { version: updateState.version }))}</button>` : ""}</div></section><section class="settings-surface"><div><small>${esc(t("settings.diagnostics.eyebrow"))}</small><h2>${esc(t("settings.diagnostics.title"))}</h2><p>${esc(t("settings.diagnostics.body"))}</p></div><div class="settings-controls"><button id="openDiagnosticsLog" type="button" class="quiet-button">${esc(t("settings.diagnostics.open"))}</button></div></section><section class="settings-surface"><div><small>${esc(t("settings.telemetry.eyebrow"))}</small><h2>${telemetryState.configured ? esc(t("settings.telemetry.titleConfigured")) : esc(t("settings.telemetry.titleUnconfigured"))}</h2><p>${telemetryState.configured ? esc(t("settings.telemetry.bodyConfigured")) : esc(t("settings.telemetry.bodyUnconfigured"))}</p></div><div class="settings-controls"><label class="prompt-enhancement"><input id="telemetryEnabled" type="checkbox" ${telemetryState.enabled ? "checked" : ""} ${telemetryState.configured ? "" : "disabled"} /> <span><b>${esc(t("settings.telemetry.checkboxLabel"))}</b><small>${telemetryState.configured ? esc(t("settings.telemetry.checkboxHintOn")) : esc(t("settings.telemetry.checkboxHintOff"))}</small></span></label></div></section><section class="settings-surface"><div><small>${esc(t("settings.license.eyebrow"))}</small><h2>${!licenseState.configured ? esc(t("settings.license.titleNotRequired")) : licenseState.hasKey && licenseState.licensed ? esc(t("settings.license.titleActivated")) : esc(t("settings.license.titleNotActivated"))}</h2><p>${licenseBody}</p></div><div class="settings-controls">${!licenseState.configured ? "" : licenseState.hasKey && licenseState.licensed ? `<button id="manageLicense" type="button" class="quiet-button">${esc(t("settings.license.manageButton"))}</button>` : `<button id="manageLicense" type="button" class="save-button">${esc(t("settings.license.enterButton"))}</button>`}</div></section></div>`;
}
...
async function generateStylePreview(dnaId, scenario, modelId) {
  ...
  const button = document.querySelector(`[data-generate-style-preview="${CSS.escape(dnaId)}:${scenario}"]`); if (button) { button.disabled = true; button.textContent = "Generating preview…"; }
...
function openMyStylesManager() {
  ...
  document.querySelectorAll("[data-manage-remove-style]").forEach((button) => button.addEventListener("click", async () => {
    ...
    if (!dna) return;
    if (location === "library") {
      // My Styles lives outside `project` (its own IPC-persisted file), so
      // a project-snapshot Undo can't cover it — keeps its confirm() dialog.
      if (!confirm(`Remove "${dna.name}" from My Styles?`)) return;
    ...
    } else withUndo(`"${dna.name}" removed from this project.`, () => deleteStyleDna(id));
...
function styleDnaWorkspace() {
  ...
  const visible = all.filter((dna) => visualStyleScope === "my" ? customIds.has(dna.id) : visualStyleScope === "favorites" ? favoriteIds.includes(dna.id) : visualStyleScope === "2d" ? (dna.categoryIds || []).includes("2d-animation") : visualStyleScope === "director" ? (dna.categoryIds || []).includes("director-inspired") : true).filter((dna) => !query || [dna.name, dna.description, ...(dna.tags || []), ...(dna.categoryIds || [])].join(" ").toLowerCase().includes(query));
  ...
  const cardImage = (dna) => {
    const previewUrl = stylePreviewUrl(dna);
    // Falls back to a deterministic two-color gradient only when no bundled
    // or generated art resolved for this style (e.g. an asset filename
    // mismatch) — keeps a director card visually distinguishable instead of
    // silently collapsing to the shared generic splash graphic.
    if (dna.cardGradient && previewUrl === splashArtwork) return `<div class="visual-style-card-image" style="background:linear-gradient(135deg, ${esc(dna.cardGradient[0])}, ${esc(dna.cardGradient[1])})"></div>`;
    return `<div class="visual-style-card-image"><img src="${esc(previewUrl)}" alt="" /></div>`;
  };
  const card = (dna) => `<article class="visual-style-card ${active?.id === dna.id ? "active" : ""}"><button class="visual-style-card-main" data-style-detail="${esc(dna.id)}">${cardImage(dna)}<span>${esc((dna.categoryIds || ["visual direction"])[0])}</span><h3>${esc(dna.name)}</h3><p>${esc(dna.shortDescription || styleSummary(dna))}</p><small>${esc(dna.source)} · v${dna.version}</small></button><div><button class="quiet-button" data-toggle-style-favorite="${esc(dna.id)}" aria-label="Toggle favorite">${favoriteIds.includes(dna.id) ? "★" : "☆"}</button><button class="save-button" data-apply-project-style="${esc(dna.id)}">Apply</button></div></article>`;
  // No-influence option, only shown alongside the director presets — not a
  // Style DNA, just a one-click revert to this project's own unstyled look.
  const ownLookCard = visualStyleScope === "director" ? `<article class="visual-style-card own-look-card ${active ? "" : "active"}"><button class="visual-style-card-main" data-clear-project-style="true"><div class="visual-style-card-image own-look-card-image">✦</div><span>no influence</span><h3>My own look</h3><p>Keep this project's own visual direction — no director preset applied.</p></button><div><button class="save-button" data-clear-project-style="true">${active ? "Use" : "Active"}</button></div></article>` : "";
...
function mountLaunchSplash() {
  ...
  // The visual itself gets its own leaf node with role="img"/aria-label —
  // it's the only place the product name and tagline exist as text
  // equivalents, since they're baked into the image as pixels. That role
  // must NOT wrap the dismiss button or status badge: role="img" tells
  // assistive tech to treat its whole subtree as one opaque image, which
  // would swallow real interactive/text content inside it. Both stay
  // siblings, positioned over the art via CSS instead.
  // The "Start Creating" pill is baked into the artwork as pixels, not a
  // real element — it was reported as dead-looking because it is: nothing
  // was ever listening there. This adds a real, invisible button in the
  // same spot (a sibling of .launch-splash-art-bg, never a child of it —
  // that div's role="img" would swallow a real interactive child from
  // assistive tech, per the note above). Percentage-positioned, not
  // pixel-positioned: .launch-splash-art is locked to the art's own
  // 16:9 aspect ratio, so a percentage box tracks the pixel button
  // exactly at any window size. Real text, transparent — screen readers
  // get an actual "Start Creating" button instead of a silent hotspot.
  // The two splash images do NOT share one pill size/position closely
  // enough for one shared box to look aligned on both — measured directly
  // against each 1672×941 PNG's actual pixels (canvas pixel-scan for the
  // dark rounded-rect band, not eyeballed): splash-1's pill is ~22% of the
  // image wide, splash-2's is only ~16%, and their vertical centers differ
  // by ~1.4% of the image height. `data-splash-variant` lets CSS give each
  // image its own precisely-fitted box instead of one padded union of both
  // that was visibly offset from either.
  const splashVariant = splashArtwork === SPLASH_ARTWORKS[0] ? "1" : "2";
  root.innerHTML = `<div class="launch-splash-wrap"><section class="launch-splash"><div class="launch-splash-art" data-splash-variant="${splashVariant}"><div class="launch-splash-art-bg" role="img" aria-label="Wheelbarrow Studios — Storymaker: ideas to cinematic reality" style="background-image:url('${splashArtwork}')"></div><button id="dismissSplash" class="launch-dismiss" title="Enter Storymaker">×</button><button id="splashStartCreating" class="launch-start-creating">Start Creating</button></div><div class="launch-content"><div><button id="splashNewProject" class="save-button">Create a project</button><button id="splashOpenProject" class="quiet-button">Open project</button></div><button id="splashGuide" class="launch-guide">Explore the studio guide →</button></div></section></div>`;
  $("#dismissSplash")?.addEventListener("click", dismissSplash);
  $("#splashStartCreating")?.addEventListener("click", () => { dismissSplash(); openProjectModal(); });
...
async function generateCharacterTurnaround(index) {
  let character = project.characters[index]; const portrait = assetById(character?.referenceAssetId);
  ...
  const stylePrefix = resolvedStyle.style ? `STYLE DNA — ${resolvedStyle.style.promptLabel || resolvedStyle.style.name}. ${styleBlocks.positive.join(". ")}. ` : "";
  ...
  try {
    for (const [direction, label] of views) {
      try {
        ...
        // Re-fetch by index: ensureCharacterShape() rebuilds project.characters
        // into new object references on every render(), which can happen
        // during the await above — mutating the originally-captured
        // `character` after that would write to a detached object and
        // silently lose this turnaround view.
        character = project.characters[index] || character;
...
async function generateCharacterReference(index) {
  let character = project.characters[index]; const model = modelCatalog.find((item) => item.model === $("#characterLabModel")?.value); if (!character || !model) return;
  if (!modelReady(model)) return notify(`${model.label} is not ready. Check its runtime or connection in Model Hub.`);
  ...
  try {
    ...
    if (!result?.asset) throw new Error("No character image was returned."); const generated = { ...result.asset, id: `character-generated-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, importedAt: new Date().toISOString(), source: result.generation?.provider || model.provider, generation: { ...(result.generation || {}), visualDirection: visualDirectionMetadata() } };
    // Re-fetch by index — see generateCharacterTurnaround for why: a
    // render() during the await above replaces project.characters with new
    // object references, and this project's own comments document that
    // exact bug class already fixed elsewhere (monitorQueuedVideoJob etc.).
    character = project.characters[index] || character;
    project.assets = projectAssets(); project.assets.unshift(generated); character.referenceAssetId = generated.id; character.turnaroundAssetIds = []; setDirty();
...
function openProjectModal() {
  ...
  // Lands on Home, not Story Bible — Home's own !hasStory branch (see
  // home()) is the deliberately-designed single-CTA first-time experience
  // ("the fastest route to something happening is pasting a script
  // directly"), and homeProductionStatus() already walks a first-time user
  // through story → style → cast → storyboard → shots one guided step at a
  // time once they have one. Landing on Story Bible instead skipped past
  // both — a genuinely built onboarding flow a new project just never saw.
  $("#newProjectForm").onsubmit = (event) => { event.preventDefault(); if (dirty && !confirm(`You have unsaved story work in "${project.name}". Starting a new project will lose it unless you save first. Continue anyway?`)) return; project = { ...blankProject(), name: $("#newTitle").value.trim() || "Untitled Film", logline: $("#newLogline").value.trim() }; filePath = ""; setDirty(); active = "Home"; closeModal(); render(); notify("Project workspace created."); };
}
function closeModal() { $("#modalRoot").innerHTML = ""; }
// Ctrl+K / Settings > Command Palette has sent a "palette" menu command
// since this menu item was first added, but nothing ever listened for it
// (src/main.js, an old pre-rewrite prototype, had a stub that showed a
// "coming into focus" toast — that file hasn't been the live renderer
// since 0.3.18 and index.html loads src/storymaker.js instead, which
// never had ANY case for "palette" at all, so the shortcut silently did
// nothing). This is the real implementation: reuses the exact same
// navigation state mutation as every data-nav button (active = X; render())
// and the exact same actions already wired to the app menu, so there is
// nothing here that behaves differently from clicking the equivalent
// button by hand.
let paletteQuery = "";
let paletteIndex = 0;
function commandPaletteEntries() {
  const workflowPages = ["Story Bible", "Style Library", "Character Bible", "Locations", "Production Assets", "Storyboard", "Continuity", "Shot Planner", "Timeline", "Motion Graphics", "Audio Studio", "Delivery"];
  const sidebarPages = navGroups.flatMap((group) => group.items.map(([name]) => name));
  const goTo = (page) => { closeModal(); active = page; render(); if (page === "Delivery") { refreshDeliveryCapabilities(); refreshLicenseState(); } if (page === "Settings") refreshLicenseState(); if (page === "Model Hub") refreshProviders(); };
  const navEntries = [...new Set([...sidebarPages, ...workflowPages])].map((page) => ({ label: page, group: "Go to", run: () => goTo(page) }));
  const actionEntries = [
    { label: "New project", group: "Actions", run: () => { closeModal(); dismissSplash(); openProjectModal(); } },
    { label: "Open project…", group: "Actions", run: () => { closeModal(); dismissSplash(); openProject(); } },
    { label: "Save", group: "Actions", run: () => { closeModal(); saveProject(); } },
    { label: "Save as…", group: "Actions", run: () => { closeModal(); saveProject(true); } },
    { label: theme === "light" ? "Switch to dark studio" : "Switch to light studio", group: "Actions", run: () => { closeModal(); theme = theme === "light" ? "dark" : "light"; localStorage.setItem("storymaker-theme", theme); render(); } },
    { label: experienceMode === "simple" ? "Switch to Studio mode" : "Switch to Simple mode", group: "Actions", run: () => { closeModal(); experienceMode = experienceMode === "simple" ? "studio" : "simple"; localStorage.setItem("storymaker-experience-mode", experienceMode); render(); notify(`${experienceMode === "simple" ? "Simple" : "Studio"} mode is active.`); } }
  ];
...
function renderCommandPalette() {
  ...
  const filtered = query ? entries.filter((entry) => entry.label.toLowerCase().includes(query) || entry.group.toLowerCase().includes(query)) : entries;
  ...
  let rowIndex = -1;
  const listMarkup = groups.map((group) => `<div class="command-palette-group"><p>${esc(group)}</p>${filtered.filter((entry) => entry.group === group).map((entry) => { rowIndex += 1; return `<button type="button" class="command-palette-row ${rowIndex === paletteIndex ? "active" : ""}" data-palette-index="${rowIndex}">${esc(entry.label)}</button>`; }).join("")}</div>`).join("");
  $("#modalRoot").innerHTML = `<div class="modal-backdrop command-palette-backdrop" id="paletteBackdrop"><div class="command-palette"><input id="paletteInput" type="text" placeholder="Jump to a page or run a command…" autocomplete="off" spellcheck="false" value="${esc(paletteQuery)}" /><div class="command-palette-list">${filtered.length ? listMarkup : `<p class="command-palette-empty">No matching commands.</p>`}</div></div></div>`;
  const input = $("#paletteInput");
  input?.addEventListener("input", (event) => { paletteQuery = event.target.value; paletteIndex = 0; renderCommandPalette(); });
  input?.addEventListener("keydown", (event) => {
    if (event.key === "ArrowDown") { event.preventDefault(); paletteIndex = Math.min(filtered.length - 1, paletteIndex + 1); renderCommandPalette(); }
    else if (event.key === "ArrowUp") { event.preventDefault(); paletteIndex = Math.max(0, paletteIndex - 1); renderCommandPalette(); }
    else if (event.key === "Enter") { event.preventDefault(); filtered[paletteIndex]?.run(); }
  });
  $("#paletteBackdrop")?.addEventListener("click", (event) => { if (event.target.id === "paletteBackdrop") closeModal(); });
  $("#modalRoot").querySelectorAll("[data-palette-index]").forEach((button) => button.addEventListener("click", () => filtered[Number(button.dataset.paletteIndex)]?.run()));
  focusPaletteInput();
}
...
async function refreshLicenseState() {
  if (!window.storyMakerDesktop?.licenseStatus) return;
  try {
    const status = await window.storyMakerDesktop.licenseStatus();
    licenseState = { ...licenseState, ...status };
    if (active === "Settings" || active === "Delivery") render();
  } catch { /* leave last-known state */ }
}
function openLicenseDialog() {
  dismissSplash();
  const s = licenseState;
  const activated = s.hasKey && s.licensed;
  $("#modalRoot").innerHTML = `<div class="modal-backdrop"><section class="project-modal license-modal"><button type="button" id="closeModal">×</button><p>STORYMAKER / ACTIVATION</p><h2>${activated ? "This copy is activated." : "Activate Storymaker"}</h2>${activated
    ? `<p class="scene-modal-note">Licensed to ${esc(s.customerName || "you")}${s.activationLimit ? ` · device ${s.activationUsage || 1} of ${s.activationLimit}` : ""}. Deactivating frees this device's seat so you can use it elsewhere.</p><div><button type="button" id="licenseDeactivate" class="quiet-button">Deactivate this device</button><button type="button" id="cancelModal" class="save-button">Done</button></div>`
    : `<p class="scene-modal-note">Writing, storyboards, and visual direction are always open. A license key unlocks AI generation and delivery. Your key came in your purchase email; you can move it between devices any time.</p><label class="license-key-field"><span>LICENSE KEY</span><input id="licenseKeyInput" type="text" autocomplete="off" spellcheck="false" placeholder="XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX" value="${esc(pendingLicenseKey)}" /></label>${s.message ? `<p class="license-note-warn">${esc(s.message)}</p>` : ""}<div><button type="button" id="licenseActivate" class="save-button">Activate</button><button type="button" id="cancelModal" class="quiet-button">Not now</button></div>`}</section></div>`;
  $("#closeModal").onclick = closeModal;
  $("#cancelModal").onclick = closeModal;
  const input = $("#licenseKeyInput");
  input?.addEventListener("input", (event) => { pendingLicenseKey = event.target.value; });
  input?.focus();
  $("#licenseActivate")?.addEventListener("click", async () => {
    const key = ($("#licenseKeyInput")?.value || "").trim();
    if (!key) return notify("Enter your license key.");
    const button = $("#licenseActivate"); button.disabled = true; button.textContent = "Activating…";
    try {
      const status = await window.storyMakerDesktop.activateLicense(key);
      licenseState = { ...licenseState, ...status };
      pendingLicenseKey = "";
      if (status.licensed) { closeModal(); render(); notify("Storymaker is activated. Generation and delivery are unlocked."); }
      else { render(); openLicenseDialog(); }
    } catch (error) {
      notify(error?.message?.replace(/^Error invoking remote method '[^']+':\s*Error:\s*/, "") || "Activation failed.");
      if (button?.isConnected) { button.disabled = false; button.textContent = "Activate"; }
    }
  });
  $("#licenseDeactivate")?.addEventListener("click", async () => {
    const button = $("#licenseDeactivate"); button.disabled = true; button.textContent = "Deactivating…";
    try {
      const status = await window.storyMakerDesktop.deactivateLicense();
      licenseState = { ...licenseState, ...status };
      closeModal(); render(); notify("This device has been deactivated.");
    } catch (error) { notify(error?.message || "Could not deactivate."); if (button?.isConnected) { button.disabled = false; button.textContent = "Deactivate this device"; } }
  });
}
function addCharacter() { ensureCharacterShape(); const count = project.characters.length + 1; const id = `character-${Date.now()}-${count}`; project.characters.push({ id, name: `Character ${count}`, role: "A role still taking shape.", appearance: "", wardrobe: "", voice: "", objective: "", movementStyle: "", continuityRules: "", references: [], referenceAssetId: "" }); flashHighlight(id); setDirty(); render(); openCharacterModal(count - 1); }
...
function addSet() { ensureSetShape(); const count = project.sets.length + 1; const id = `set-${Date.now()}-${count}`; project.sets.push({ id, name: `Set ${count}`, description: "", mood: "", referenceAssetId: "" }); flashHighlight(id); setDirty(); render(); openSetModal(count - 1); }
...
function addProductionAsset() {
  ...
  const id = `production-asset-${Date.now()}-${count}`;
  project.props.push({ id, name: `Prop ${count}`, category: "prop", description: "", referenceAssetId: "", references: [] });
  flashHighlight(id);
...
function openPropLab(index) {
  ...
  const hasReference = Boolean(assetById(item.referenceAssetId));
  $("#modalRoot").innerHTML = `<div class="modal-backdrop"><form id="propLabForm" class="project-modal character-lab-modal"><button type="button" id="closeModal">×</button><p>PROP LAB / GENERATION WORKBENCH</p><h2>Build ${/^[aeiou]/i.test(category) ? "an" : "a"} ${esc(category)} the camera can return to.</h2><section class="character-lab-hero"><div><small>CONTINUITY-FIRST DESIGN</small><h3>${esc(item.name)}</h3><p>Generate a reference image, keep the approved look in the project library, and reuse it in every scene where this asset appears.</p></div><div class="character-lab-hero-badges"><span class="autosave">${imageCount} IMAGE REFERENCE${imageCount === 1 ? "" : "S"}</span>${styleIndicatorMarkup()}</div></section><div class="character-lab-grid"><div><label>DESIGN BRIEF<textarea id="propLabPrompt">${esc(prompt)}</textarea></label><button type="button" id="propLabDescribeReference" class="quiet-button lab-describe-button" ${hasReference ? "" : 'disabled title="Add a reference image first."'}>✨ Describe from reference</button><label>NEGATIVE CONSTRAINTS<textarea id="propLabNegative" placeholder="No people, no text, no logos, no background clutter…"></textarea></label><div class="character-lab-actions"><button type="button" id="propLabGenerate" class="save-button">Generate reference</button></div></div><aside class="character-lab-side"><h4>Model direction</h4><label class="select-label">IMAGE MODEL<select id="propLabModel" class="model-select">${modelOptions}</select></label><label>OUTPUT RATIO<select id="propLabAspect"><option>1:1</option><option>3:2</option><option>16:9</option><option>9:16</option><option>21:9</option></select></label><label>QUALITY<select id="propLabQuality"><option>Balanced</option><option>Fast</option><option>High</option></select></label><div id="${REFERENCE_LIBRARY_ROOT_ID}"></div></aside></div><div class="character-lab-actions"><button type="button" id="cancelModal" class="quiet-button">Close</button><button class="save-button">Save prop profile</button></div></form></div>`;
  $("#closeModal").onclick = closeModal; $("#cancelModal").onclick = closeModal;
  $("#propLabDescribeReference")?.addEventListener("click", (event) => describeReferenceForPrompt("prop", item.referenceAssetId, "propLabPrompt", event.currentTarget));
...
async function generatePropReference(index) {
  let item = project.props[index]; const model = modelCatalog.find((entry) => entry.model === $("#propLabModel")?.value); if (!item || !model) return;
  if (!modelReady(model)) return notify(`${model.label} is not ready. Check its runtime or connection in Model Hub.`);
  ...
  const stylePrefix = resolvedStyle.style ? `STYLE DNA — ${resolvedStyle.style.promptLabel || resolvedStyle.style.name}. ${styleBlocks.positive.join(". ")}. ` : "";
  const button = $("#propLabGenerate"); if (button) { button.disabled = true; button.textContent = "Generating prop…"; }
  ...
  try {
    ...
    if (!result?.asset) throw new Error("No reference image was returned."); const generated = { ...result.asset, id: `prop-generated-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, importedAt: new Date().toISOString(), source: result.generation?.provider || model.provider, generation: { ...(result.generation || {}), visualDirection: visualDirectionMetadata() } };
    // Re-fetch by index — see generateCharacterTurnaround for why: a
    // render() during the await above replaces project.props with new
    // object references (ensureProductionShape rebuilds it every call).
    item = project.props[index] || item;
    project.assets = projectAssets(); project.assets.unshift(generated); item.referenceAssetId = generated.id; setDirty();
    // Refresh just the reference gallery in place instead of reopening the
    // whole Lab — a full reopen would silently wipe the design brief,
    // negative constraints, and model/aspect/quality choices right after
    // Generate, the exact moment someone is most likely to tweak and
...
async function generateSetReference(index) {
  let set = project.sets[index]; const model = modelCatalog.find((item) => item.model === $("#setLabModel")?.value); if (!set || !model) return;
  if (!modelReady(model)) return notify(`${model.label} is not ready. Check its runtime or connection in Model Hub.`);
  ...
  try {
    ...
    if (!result?.asset) throw new Error("No environment image was returned."); const generated = { ...result.asset, id: `set-generated-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, importedAt: new Date().toISOString(), source: result.generation?.provider || model.provider, generation: { ...(result.generation || {}), visualDirection: visualDirectionMetadata() } };
    // Re-fetch by index — see generateCharacterTurnaround for why: a
    // render() during the await above replaces project.sets with new
    // object references (ensureSetShape rebuilds it every call).
    set = project.sets[index] || set;
    project.assets = projectAssets(); project.assets.unshift(generated); set.referenceAssetId = generated.id; setDirty();
    // Refresh just the reference gallery in place instead of reopening the
    // whole Lab — a full reopen would silently wipe the design brief,
    // negative constraints, and model/aspect/quality choices right after
    // Generate, the exact moment someone is most likely to tweak and
...
function addScene() { const count = project.scenes.length + 1; const id = `scene-${Date.now()}-${count}`; project.scenes.push({ id, title: `Untitled sequence ${count}`, note: "A new beat waiting for its purpose.", referenceAssetId: "", variations: [], approvedVariationId: "", generationPrompt: "", generationSize: "1536x1024", castIds: [], shots: [] }); flashHighlight(id); setDirty(); render(); notify("Scene added to the board."); }
...
function removeShot(sceneIndex, shotIndex) {
  ...
  withUndo(`"${shot.title}" removed from the plan.`, () => { scene.shots.splice(shotIndex, 1); setDirty(); render(); });
}
...
function enhanceShotDirector(sceneIndex, shotIndex) {
 ...
 $("#shotDeliveryResolution").value = settings.deliveryResolution || "native";
  ...
  // 0.4.61: a search box that still defaulted to showing the ENTIRE
  // project Media Library read, in practice, like the same bug — reported
  // again with a screenshot after 0.4.60 shipped: a shot for Lila and
  // Chloe (per its own Shot Intent text) still opened onto a wall of
  // Teacher Wen's turnaround history, just now technically "unattached."
  // The fix isn't a better way to search past that wall — it's not
  // building the wall by default. With no search text, show only tiles
  // that are relevant to THIS shot (this scene's own cast/set/prop
  // references, plus anything already attached) and hide the rest of the
  // library entirely; a search narrows/expands into the full library on
  // demand. Called after every grid rebuild so an active search survives
  // a provider/model change, and on the search box's own input event.
  const applyReferenceSearchFilter = () => {
    const query = ($("#shotReferenceSearch")?.value || "").trim().toLowerCase();
    const tiles = [...form.querySelectorAll(".shot-reference-picker .reference-tile")];
    let visibleCount = 0;
    tiles.forEach((tile) => {
      const name = tile.querySelector("small")?.textContent?.toLowerCase() || "";
      const isRelevantByDefault = tile.classList.contains("continuity-suggested") || Boolean(tile.querySelector("input")?.checked);
      const visible = query ? name.includes(query) : isRelevantByDefault;
      tile.hidden = !visible;
      if (visible) visibleCount += 1;
    });
    const hint = form.querySelector("#shotReferenceSearchHint");
    if (hint) hint.hidden = query.length > 0 || visibleCount > 0 || tiles.length === 0;
  };
  $("#shotReferenceSearch")?.addEventListener("input", applyReferenceSearchFilter);
  const updateCapabilities = () => {
    ...
    // Every model gets the app-wide Fast/Balanced/High unless its capability
    // opts into a richer set (currently only gpt-image-2.5-flare/sunburst's
    // qualityTiers, mapped to low/medium/high/xhigh/max — see
    // IMAGE_QUALITY_TO_API in electron-main.js). Fast/Balanced/High are a
    // subset of that richer list, so switching between these models never
    // loses the user's existing choice.
    replaceOptions($("#shotQuality"), capability.qualityTiers || ["Fast", "Balanced", "High"], $("#shotQuality").value || settings.quality);
    ...
    // 0.4.59: this used to force-check AND disable ("lock") every reference
    // belonging to a character/set/prop merely assigned to the scene's cast
    // list — so, e.g., a teacher character cast once in a schoolyard scene
    // had her entire reference library (every turnaround view, every saved
    // variation — sometimes a dozen+ images) silently injected into every
    // shot in that scene, including shots she wasn't even in, with no way
    // to uncheck them. Reported directly: "an asset appears in every shot
    // by default irrespective of whatever character was supposed to be
    // there... each shot should not be populated from assets unless I add
    // it." Nothing here gets pre-checked or disabled anymore — the scene's
    // cast/set/prop references are only surfaced as a labeled, one-click
    // "SCENE CAST" suggestion; whether any of them end up attached is
    // exactly and only whatever the user has checked. Not sliced to the
    // model's input limit here — that limit only matters at the moment of
    // checking a box (enforced below), not for what's worth suggesting.
    const suggestedReferenceIds = automaticReferenceIds;
    // A character with a few regenerated turnaround passes can easily have a
    // dozen+ accumulated reference images, and this list is every image in
    // the whole project's Media Library, not just this scene's — sort the
    // scene's own suggestions to the front, and see applyReferenceSearchFilter
    // below, which actually hides the rest of the library by default now
    // rather than just visually deprioritizing it.
    const sortedAssets = [...allowedAssets].sort((a, b) => Number(suggestedReferenceIds.includes(b.id)) - Number(suggestedReferenceIds.includes(a.id)));
    const hintMarkup = `<p class="scene-modal-note" id="shotReferenceSearchHint">This scene has no cast, set, or prop images in Media Library yet. This shot isn't limited to this scene's cast, though — search above by name to pull in any character, prop, or set from the whole project.</p>`;
    if (referenceChoices) referenceChoices.innerHTML = allowedAssets.length ? `<div class="reference-grid">${sortedAssets.map((asset) => referenceChoiceTile(asset, (settings.referenceAssetIds || []).includes(asset.id), "shotReferences", true, false, suggestedReferenceIds.includes(asset.id))).join("")}</div>${hintMarkup}` : `<p class="scene-modal-note">No ${esc(capability.references.join(" or "))} references are in Media Library yet.</p>`;
    applyReferenceSearchFilter();
    ...
    if (automaticReferenceIds.length) {
      const availableCount = automaticReferenceIds.length;
      $("#modelReferenceStatus").textContent += limits.image
        ? ` ${availableCount} reference${availableCount === 1 ? "" : "s"} from this scene's cast, sets, and props ${availableCount === 1 ? "is" : "are"} shown below, marked SCENE CAST — check the ones this shot needs. This shot isn't limited to this scene's cast: search by name above to pull in any character, prop, or set from the whole project.`
  ...
  const collectSettings = () => {
    ...
    // Exactly what's checked, nothing more — see the note at
    // suggestedReferenceIds above. Scene-cast/set/prop references are a
    // labeled suggestion in the picker now, not an invisible floor that
    // survives even when every checkbox reads unchecked.
    const manualReferenceIds = [...form.querySelectorAll('input[name="shotReferences"]:checked')].map((input) => input.value).slice(0, limits.image || 0);
    return normalizeReferenceRoles({ ...settings, provider: model.provider, model: model.model, mode: $("#shotMode").value, aspectRatio: $("#shotAspect").value, resolution: $("#shotResolution").value, motion: $("#shotMotion").value, prompt: modelCapability(model.model).output === "Video" ? videoPrompt : imagePrompt, imagePrompt, videoPrompt, promptPackageVersion: 2, negativePrompt: $("#shotNegative").value.trim(), referenceAssetIds: manualReferenceIds, videoReferenceAssetIds: [...form.querySelectorAll('input[name="shotVideoReferences"]:checked')].map((input) => input.value).slice(0, limits.video || 0), audioReferenceAssetIds: [...form.querySelectorAll('input[name="shotAudioReferences"]:checked')].map((input) => input.value).slice(0, limits.audio || 0), startFrameAssetId: supportsStartEndFrames(modelCapability(model.model)) ? settings.startFrameAssetId || "" : "", endFrameAssetId: supportsStartEndFrames(modelCapability(model.model)) ? settings.endFrameAssetId || "" : "", referenceUrls: $("#shotReferenceUrls").value.split(/\r?\n/).map((value) => value.trim()).filter((value) => /^https:\/\//i.test(value)), quality: $("#shotQuality").value, background: $("#shotBackground")?.value || "Auto", referenceStrength: $("#shotReferenceStrength").value, seed: $("#shotSeed").value.trim(), duration: $("#shotDuration").value, cameraInstruction: nextBlueprint.camera, audioInstruction: nextBlueprint.audio, multiShotPrompts: $("#shotMultiPrompts").value.trim(), referenceRole: "guidance", promptEnhancement: $("#shotPromptEnhancement").checked, blueprint: nextBlueprint, outputHistory: Array.isArray(shot.modelSettings?.outputHistory) ? shot.modelSettings.outputHistory : [] });
  };
  ...
  $("#generateShotFrame")?.addEventListener("click", async () => {
    ...
    try {
      ...
      closeModal();
      render();
      if (project.milestones?.firstGeneration) notify("Shot frame ready to review."); else celebrateFirstGeneration();
    } catch (error) {
  ...
  if (isVideo) {
    ...
    if (owningSceneIndex >= 0) {
      ...
      $("#useVideoInStoryboard")?.addEventListener("click", () => {
        ...
        if (owningShot) { owningShot.outputReview = "approved"; owningShot.locked = true; }
...
async function generateSceneImage() {
  ...
  try {
    ...
    // Re-fetch by id, not by re-reading selectedSceneIndex: a render()
    // during the await above replaces project.scenes with new object
    // references (ensureSceneShape rebuilds it every call), and the user
    // could also have navigated to a different scene while this was in
    // flight — id is the only reliable way back to the scene this take was
    // actually generated for.
    const liveScene = project.scenes.find((item) => item.id === scene.id) || scene;
    project.assets = projectAssets(); project.assets.unshift(asset); liveScene.generationPrompt = scenePrompt; liveScene.generationSize = size; liveScene.generationModel = model.model; liveScene.variations = Array.isArray(liveScene.variations) ? liveScene.variations : []; liveScene.variations.unshift({ assetId: asset.id, generation: result.generation, createdAt: new Date().toISOString() }); flashHighlight(asset.id); setDirty(); render();
    if (project.milestones?.firstGeneration) notify(`${model.label} created a new scene take.`); else celebrateFirstGeneration();
...
function openSceneProductionModal(index, preferredAssetId = "") {
  ...
  const previewBlock = preview
    ? `<div class="scene-modal-preview">${preview.kind === "video" ? `<video src="${esc(fileUrl(preview))}" controls></video>` : `<img src="${esc(fileUrl(preview))}" alt="Scene preview" />`}<button type="button" id="downloadScenePreview" class="quiet-button">${esc(t("scene.previewDownload"))}</button></div>`
    : `<div class="scene-modal-preview empty"><span>${esc(t("scene.previewEmpty"))}</span></div>`;
  // Music Video mode — per-section performance & choreography layer (Phase 8).
  const mvSection = project.song && scene.songSectionId ? project.song.sections.find((s) => s.id === scene.songSectionId) : null;
  const mvCast = scene.sectionCast || emptySectionCast();
  const mvLyrics = mvSection ? (project.song.lyrics || []).filter((l) => l.sectionId === mvSection.id).map((l) => l.text) : [];
  const mvAiMovement = (scene.songSectionCreative?.movement || "").trim();
  const roleChecks = (roleKey, selected) => project.characters.length
    ? project.characters.map((ch) => `<label class="scene-cast-choice"><input data-perf-role="${roleKey}" type="checkbox" value="${esc(ch.id)}" ${(selected || []).includes(ch.id) ? "checked" : ""} /> <span>${esc(ch.name)}</span><small>${esc(ch.role)}</small></label>`).join("")
    : `<span class="context-empty">${esc(t("audio.roleChecks.empty"))}</span>`;
  const mvBlock = mvSection ? `<div class="scene-perf-layer"><small>${esc(t("audio.mvBlock.title"))} · ${esc(mvSection.kind)} · ${formatTime(mvSection.start)}–${formatTime(mvSection.end)}</small>`
    + `<label>${esc(t("audio.mvBlock.leadLabel"))}<select id="perfLead"><option value="">${esc(t("audio.mvBlock.noSingleLead"))}</option>${project.characters.map((ch) => `<option value="${esc(ch.id)}" ${mvCast.leadId === ch.id ? "selected" : ""}>${esc(ch.name)}</option>`).join("")}</select></label>`
    + `<div class="scene-cast"><small>${esc(t("audio.mvBlock.backupLabel"))}</small><div>${roleChecks("backup", mvCast.backupIds)}</div></div>`
    + `<div class="scene-cast"><small>${esc(t("audio.mvBlock.dancersLabel"))}</small><div>${roleChecks("dancer", mvCast.dancerIds)}</div></div>`
    + `<label>${esc(t("audio.mvBlock.choreoLabel"))}<textarea id="perfChoreo" rows="3" placeholder="${esc(mvAiMovement || t("audio.mvBlock.choreoPlaceholder"))}">${esc(scene.choreography || "")}</textarea></label>`
    + `<p class="scene-modal-note">${esc(t("audio.mvBlock.note"))}${mvLyrics.length ? ` <br />♪ ${esc(mvLyrics.join(" / "))}` : ""}</p></div>` : "";
  $("#modalRoot").innerHTML = `<div class="modal-backdrop"><form id="productionSceneForm" class="project-modal scene-modal wide-modal"><button type="button" id="closeModal">×</button><p>${esc(t("scene.modal.eyebrowPrefix", { number: String(index + 1).padStart(2, "0") }))}</p><h2>${esc(t("scene.modal.title"))}</h2>${previewBlock}<label>${esc(t("scene.modal.titleLabel"))}<input id="sceneTitle" autofocus value="${esc(scene.title)}" /></label><label>${esc(t("scene.modal.noteLabel"))}<textarea id="sceneNote">${esc(scene.note)}</textarea></label><label>${esc(t("scene.modal.referenceLabel"))}<select id="sceneReference">${assetOptions}</select></label><div class="scene-cast"><small>${esc(t("scene.modal.castLabel"))}</small><div>${castOptions}</div></div><p class="scene-modal-note">${esc(t("scene.modal.castNote"))}</p>${mvBlock}<div><button type="button" id="cancelModal" class="quiet-button">${esc(t("common.cancel"))}</button><button class="save-button">${esc(t("scene.modal.save"))}</button></div></form></div>`;
  $("#closeModal").onclick = closeModal; $("#cancelModal").onclick = closeModal;
  ...
  $("#productionSceneForm").onsubmit = (event) => {
    event.preventDefault();
    scene.title = $("#sceneTitle").value.trim() || `Untitled sequence ${index + 1}`;
    scene.note = $("#sceneNote").value.trim() || "A new beat waiting for its purpose.";
    scene.referenceAssetId = $("#sceneReference").value;
    scene.castIds = [...document.querySelectorAll(".scene-cast-choice input:checked")].filter((input) => !input.dataset.sceneAssetType && !input.dataset.perfRole).map((input) => input.value);
    scene.setIds = [...document.querySelectorAll('[data-scene-asset-type="set"]:checked')].map((input) => input.value);
    scene.propIds = [...document.querySelectorAll('[data-scene-asset-type="prop"]:checked')].map((input) => input.value);
    if (mvSection) {
      const leadId = $("#perfLead")?.value || "";
      const backupIds = [...document.querySelectorAll('[data-perf-role="backup"]:checked')].map((i) => i.value).filter((id) => id !== leadId);
      const dancerIds = [...document.querySelectorAll('[data-perf-role="dancer"]:checked')].map((i) => i.value).filter((id) => id !== leadId && !backupIds.includes(id));
      scene.sectionCast = { leadId, backupIds, dancerIds };
      scene.choreography = $("#perfChoreo")?.value.trim() || "";
      // Fold performers + choreography + lyrics back into the note, shot
      // blueprint and castIds (likeness) for every song scene.
      reseedSongScenes();
    }
    setDirty(); closeModal(); render();
    notify(mvSection ? t("audio.notify.perfUpdated") : t("scene.notify.updated"));
  };
}
...
async function enrichImportedStory() {
  ...
  const button = $("#runAiStoryAnalysis"); if (button) { button.disabled = true; button.textContent = "Reading closer…"; }
  try {
...
async function checkAllScenesDrift() {
  ...
  for (const { scene, index, asset } of candidates) {
    ...
    // Read the title live, not from the pre-loop `scene` snapshot: this is a
    // sequential multi-second loop of API calls, and the scene at this index
    // could have been renamed or reordered by the time this iteration's
    // await resolves.
    if (report) results.push({ sceneIndex: index, sceneTitle: project.scenes[index]?.title || scene.title, report });
  }
...
function openPasteStoryModal(prefill = "") {
  $("#modalRoot").innerHTML = `<div class="modal-backdrop"><section class="project-modal paste-story-modal"><button type="button" id="closeModal">×</button><p>STORY / PASTE SCRIPT</p><h2>Drop your script straight in.</h2><p class="scene-modal-note">Type or paste (Ctrl+V) a script, screenplay, treatment, or outline below. Nothing is committed until you review what was detected. Not sure about the format? <button type="button" id="pasteFormatHelp" class="link-button">See "Format a story" in the guide</button>.</p><textarea id="pasteStoryText" rows="16" placeholder="INT. KITCHEN - NIGHT&#10;&#10;MARA stands at the counter, unmoving...&#10;&#10;MARA&#10;I said I'd be fine.&#10;&#10;Paste or type your script here.">${esc(prefill)}</textarea><div><button type="button" id="submitPastedStory" class="save-button">Parse & import</button><button type="button" id="loadSampleScript" class="quiet-button">Load a sample</button><button type="button" id="cancelModal" class="quiet-button">Cancel</button></div></section></div>`;
  $("#closeModal").onclick = closeModal; $("#cancelModal").onclick = closeModal;
  $("#loadSampleScript").onclick = () => { $("#pasteStoryText").value = SAMPLE_SCRIPT; $("#pasteStoryText").focus(); };
  $("#pasteFormatHelp").onclick = () => { closeModal(); helpTopic = "format"; active = "Help"; render(); };
...
// Bulk still-frame generation — one click for every scene that doesn't have
// a generated frame yet, instead of opening Shot Director once per scene.
// Deliberately STILLS ONLY: video is async/queued per provider (see
// monitorQueuedVideoJob) and safely bulk-queuing many videos across
// providers at once needs its own concurrency/rate-limit design — a
// separate, larger feature, not folded into this first cut. Runs scenes
// SEQUENTIALLY, one provider request in flight at a time — never in
// parallel — so a slow or rate-limited provider isn't hammered, cost stays
// predictable, and a failure on one scene can't corrupt the next one's
// request. Every result lands exactly where a hand-generated one would
// (unreviewed, in Storyboard/Delivery preflight) — bulk generation does not
// skip review.
// Read-only — used for the button's count, so must never create a shot as
// a side effect of merely rendering Storyboard. A scene with no shot yet
// still counts as pending (bulkGenerateAllScenes creates shot #1 for it
// when it actually runs, same as clicking "Generate with AI model" by hand
// already does via generateSceneShot) — excluding it here would silently
// undercount and contradict the button's own "every scene" promise.
function bulkGenerationCandidates() {
  ensureProductionShape();
  // Locked (scene.shots[0].locked — the shot this action would actually
  // fill in or create) is excluded even though a locked-but-empty shot is
  // rare: a director can lock a hand-tuned brief before ever generating it,
  // specifically to keep any bulk pass from touching it.
  return project.scenes.map((scene, sceneIndex) => ({ scene, sceneIndex })).filter(({ scene }) => !sceneGeneratedAsset(scene) && !scene.shots[0]?.locked);
}
function resolveBulkModel(shot) {
  const saved = modelCatalog.find((item) => item.provider === shot?.modelSettings?.provider && item.model === shot?.modelSettings?.model);
  if (saved && saved.status === "live" && modelCapability(saved.model).output === "Image") return saved;
  // Same default a brand-new shot already ships with (defaultShotModelSettings).
  return modelCatalog.find((item) => item.model === "gpt-image-1") || null;
}
async function generateSceneStillHeadless(scene, shot, model) {
  const resolvedStyle = resolvedStyleFor(scene, shot);
  const styleBlocks = stylePromptBlocks(resolvedStyle.style, "image", "storyboard", resolvedStyle.strength);
  const prompt = shot.modelSettings?.imagePrompt?.trim() || productionPromptFor(scene, shot, "image");
  const refIds = linkedReferenceIds(scene, shot);
  const payload = {
    project: { id: project.id, name: project.name, logline: project.logline, premise: project.premise, world: project.world, themes: project.themes, style: project.style, visualDirection: project.visualDirection, characters: project.characters, sets: project.sets, props: project.props },
    scene: { id: scene.id, title: scene.title, note: scene.note, castIds: scene.castIds || [] },
    shot: { id: shot.id, title: shot.title, purpose: shot.purpose, framing: shot.framing, lens: shot.lens, movement: shot.movement, blueprint: shot.blueprint || defaultShotBlueprint() },
    settings: { ...defaultShotModelSettings(), ...shot.modelSettings, provider: model.provider, model: model.model, mode: refIds.length ? "image-edit" : "text-to-image", prompt, imagePrompt: prompt, negativePrompt: shot.modelSettings?.negativePrompt || styleBlocks.negative.join(", "), referenceAssetIds: refIds, styleContext: "storyboard" },
    references: refIds.map(assetById).filter((asset) => asset?.path).map((asset) => ({ id: asset.id, path: asset.path, name: asset.name, kind: asset.kind, role: "image" }))
  };
  const result = await window.storyMakerDesktop.generateShotImage(payload);
  if (!result?.asset) throw new Error("No frame was returned for this shot.");
  const generated = { ...result.asset, id: `shot-output-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, importedAt: new Date().toISOString(), source: model.provider, generation: { ...result.generation, visualDirection: visualDirectionMetadata(scene, shot) } };
  project.assets = projectAssets();
  project.assets.unshift(generated);
  shot.modelSettings = { ...shot.modelSettings, provider: model.provider, model: model.model, prompt, imagePrompt: prompt, outputHistory: [{ id: `job-${Date.now()}`, status: "completed", provider: model.provider, model: model.model, requestedAt: new Date().toISOString(), completedAt: new Date().toISOString(), assetId: generated.id, prompt }, ...(Array.isArray(shot.modelSettings?.outputHistory) ? shot.modelSettings.outputHistory : [])] };
  shot.outputAssetId = generated.id;
  celebrateFirstGeneration();
  return generated;
}
// Shared sequential-stills runner — used by both bulkGenerateAllScenes
// (fills every scene with no output yet) and bulkRerollSectionKind
// (regenerates a chosen kind's EXISTING stills, an intentionally different
// eligibility). One scene at a time, `state.cancelled` checked every
// iteration so a single Stop button can also halt a later video phase in
// the same batch (re-roll runs stills then videos sequentially as two
// phases sharing one cancel flag).
async function runStillsBatch(runnable, dockId, state) {
  let done = 0; let failed = 0;
  for (const item of runnable) {
    if (state.cancelled) break;
    // Re-fetch the scene fresh by id on every iteration — never hold a
    // scene/shot object reference across a render() call. render() runs
    // ensureSceneShape(), which rebuilds project.scenes into brand-new
    // objects every time (never preserves identity, by design elsewhere
    // in this file) — a reference captured before this loop started, or
    // even from a previous iteration's render(), is silently stale the
    // moment the next render() happens, and a mutation against it never
    // reaches the real project state. Confirmed live: without this, only
    // the scene generated last kept its output after the batch.
    const scene = project.scenes.find((candidate) => candidate.id === item.sceneId);
    if (!scene) { failed += 1; continue; }
    updateGenerationOverlay({ id: dockId, kind: "scene batch", stage: `Scene ${done + failed + 1} of ${runnable.length}: ${item.sceneTitle}`, progress: Math.round(((done + failed) / runnable.length) * 100) });
    if (!scene.shots.length) scene.shots.push(newShot(scene, 1)); // same lazy-create generateSceneShot does by hand
    try { await generateSceneStillHeadless(scene, scene.shots[0], item.model); done += 1; }
    catch (error) { failed += 1; console.warn(`Bulk generation failed for "${item.sceneTitle}":`, error?.message || error); }
    setDirty(); render();
  }
  return { done, failed };
}
async function bulkGenerateAllScenes() {
  if (bulkGenerationActive) return;
  if (!window.storyMakerDesktop?.generateShotImage) return notify("Bulk generation is available in the packaged Windows app.");
  const candidates = bulkGenerationCandidates();
  if (!candidates.length) return notify("Every scene already has a generated frame.");
  const priced = candidates.map(({ scene, sceneIndex }) => { const model = resolveBulkModel(scene.shots[0]); const ready = Boolean(model) && modelReady(model); const cost = model ? estimateGenerationCost({ provider: model.provider, model: model.model }, "image") : null; return { sceneId: scene.id, sceneTitle: scene.title, sceneIndex, model, ready, cost }; });
  const runnable = priced.filter((item) => item.ready);
  const blocked = priced.filter((item) => !item.ready);
  if (!runnable.length) return notify(`None of the ${priced.length} remaining scene${priced.length === 1 ? "" : "s"} can render yet — connect a provider in Model Hub first.`);
  const known = runnable.filter((item) => item.cost != null);
  const total = known.reduce((sum, item) => sum + item.cost, 0);
  const costLine = !known.length ? "cost not tracked for this model" : known.length === runnable.length ? `~$${total.toFixed(2)} estimated` : `~$${total.toFixed(2)}+ estimated (${runnable.length - known.length} shot${runnable.length - known.length === 1 ? "" : "s"} without a cost estimate)`;
  const confirmMsg = [
    `Generate ${runnable.length} scene${runnable.length === 1 ? "" : "s"} — ${costLine}.`,
    blocked.length ? `${blocked.length} scene${blocked.length === 1 ? "" : "s"} will be skipped (no connected provider for the assigned model): ${blocked.slice(0, 3).map((item) => item.sceneTitle).join(", ")}${blocked.length > 3 ? "…" : ""}.` : "",
    "Runs one scene at a time so you can stop between scenes, and every result lands unreviewed, same as generating it by hand.",
    "Continue?"
  ].filter(Boolean).join(" ");
  if (!confirm(confirmMsg)) return;
  bulkGenerationActive = true; render();
  const state = { cancelled: false };
  const dockId = openGenerationOverlay("scene batch", { id: "bulk-generate", stage: "Starting the batch" });
  const card = generationOverlayState.get(dockId)?.el?.querySelector(".generation-overlay-card");
  if (card) {
    const cancelBtn = document.createElement("button");
    cancelBtn.type = "button"; cancelBtn.className = "generation-overlay-minimize"; cancelBtn.style.right = "34px"; cancelBtn.title = "Stop after the current scene";
    cancelBtn.textContent = "✕";
    cancelBtn.addEventListener("click", () => { state.cancelled = true; cancelBtn.disabled = true; cancelBtn.title = "Stopping after this scene…"; });
    card.appendChild(cancelBtn);
  }
  let result;
  try { result = await runStillsBatch(runnable, dockId, state); }
  finally { closeGenerationOverlay(dockId); bulkGenerationActive = false; }
  const summary = [`${result.done} scene${result.done === 1 ? "" : "s"} generated`, result.failed ? `${result.failed} failed` : "", blocked.length ? `${blocked.length} skipped (no provider)` : "", state.cancelled ? "stopped early" : ""].filter(Boolean).join(", ");
  notify(`Bulk generation done — ${summary}.`);
  render();
}
// Bulk video generation — animate every scene that has a generated still but
// no video yet. Video providers are already async (submit, then poll), so
// unlike bulk stills this runs a small CONCURRENT worker pool rather than
// one at a time: each job spends nearly all its life waiting on the
// provider, not on this app, so submitting only one at a time would leave
// the pipeline mostly idle for no safety benefit. The cap keeps it well
// short of hammering any provider's rate limit. "Stop" here can only halt
// SUBMITTING new jobs — a job already sent to a provider is already billed
// and can't be recalled, so in-flight jobs always run to their natural
// completion; the confirmation and the cancel button both say this plainly.
const MAX_CONCURRENT_BULK_VIDEOS = 2;
function bulkVideoCandidates() {
  ensureProductionShape();
  // A locked shot is excluded even though today's eligibility (still, no
  // video yet) already implies it isn't approved-with-video — locked is the
  // durable guarantee, not derived from current output, so it still holds
  // if that still's asset is ever cleared some other way.
  return project.scenes.map((scene, sceneIndex) => ({ scene, sceneIndex })).filter(({ scene }) => sceneGeneratedAsset(scene)?.kind === "image" && !scene.shots[0]?.locked);
}
function resolveBulkVideoModel(shot, mode) {
  const saved = modelCatalog.find((item) => item.provider === shot?.modelSettings?.provider && item.model === shot?.modelSettings?.model);
  if (saved && saved.status === "live" && modelCapability(saved.model).output === "Video" && saved.modes.includes(mode)) return saved;
  return defaultVideoModel(mode);
}
// Submits one scene's video and resolves only once that job has fully
// settled — success or failure — unlike a manual "Generate video" click,
// which is fire-and-forget. Awaited by the worker pool below.
function submitSceneVideoHeadless(scene, shot, model, sourceFrame) {
  const prompt = shot.modelSettings?.videoPrompt?.trim() || productionPromptFor(scene, shot, "video");
  // "Animate this frame" is an explicit frame-selection action — the scene's
  // still is deliberately the I2V start frame, same convention as
  // animateSceneFrame/generateSceneVideo (general references are dropped,
  // not carried over from whatever the still happened to use).
  const nextSettings = { ...defaultShotModelSettings(), ...shot.modelSettings, provider: model.provider, model: model.model, mode: "image-to-video", prompt, videoPrompt: prompt, referenceAssetIds: [], videoReferenceAssetIds: [], audioReferenceAssetIds: [], startFrameAssetId: sourceFrame.id, endFrameAssetId: "" };
  shot.modelSettings = nextSettings;
  const payload = {
    project: { id: project.id, name: project.name, logline: project.logline, premise: project.premise, world: project.world, themes: project.themes, style: project.style, visualDirection: project.visualDirection, characters: project.characters, sets: project.sets, props: project.props },
    scene: { id: scene.id, title: scene.title, note: scene.note, castIds: scene.castIds || [] },
    shot: { id: shot.id, title: shot.title, purpose: shot.purpose, framing: shot.framing, lens: shot.lens, movement: shot.movement, blueprint: shot.blueprint || defaultShotBlueprint() },
    settings: { ...nextSettings, styleContext: "storyboard" },
    references: [{ id: sourceFrame.id, path: sourceFrame.path, name: sourceFrame.name, kind: sourceFrame.kind, role: "start-frame" }]
  };
  return window.storyMakerDesktop.submitShotVideo(payload).then((result) => {
    const job = { id: `job-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, status: result.status || "queued", provider: model.provider, model: model.model, requestedAt: new Date().toISOString(), submittedAt: new Date().toISOString(), prompt, providerTaskId: result.taskId, backendJobId: result.job?.id, credits: result.credits, pollAfterSeconds: result.pollAfterSeconds || 10, sourceFrameAssetId: sourceFrame.id, visualDirection: visualDirectionMetadata(scene, shot) };
    shot.modelSettings.outputHistory = [job, ...(Array.isArray(shot.modelSettings.outputHistory) ? shot.modelSettings.outputHistory : [])];
    setDirty();
    job.renderDockId = openGenerationOverlay("video", { id: `bulk-video-${job.id}`, stage: "Queued render will continue while you work elsewhere." });
    return new Promise((resolve) => { monitorQueuedVideoJob(scene, shot, job, model.label, 0, resolve); });
  });
}
// Shared concurrent-video runner — used by both bulkGenerateAllVideos
// (fills every still-only scene) and bulkRerollSectionKind. Finds the
// source frame via storyboardVideoSourceFrame, not sceneGeneratedAsset:
// the latter ranks a scene's video above its still, which is exactly wrong
// for a re-roll of a scene that already HAS a video — it would try to
// animate the video file itself. storyboardVideoSourceFrame always looks
// past any video to the underlying still, correct for both callers (for
// bulkGenerateAllVideos's own scenes, which never have a video yet, the two
// resolve to the same asset). state.cancelled halts new submissions only —
// see submitSceneVideoHeadless's own comment on why an in-flight job can't
// be recalled.
async function runVideosBatch(runnable, dockId, state) {
  let done = 0; let failed = 0; let started = 0;
  const queue = [...runnable];
  const worker = async () => {
    while (queue.length && !state.cancelled) {
      const item = queue.shift();
      started += 1;
      updateGenerationOverlay({ id: dockId, kind: "video batch", stage: `${started} of ${runnable.length} started · ${done} done · ${failed} failed`, progress: Math.round(((done + failed) / runnable.length) * 100) });
      // Re-fetch by id, same reason as runStillsBatch and now
      // monitorQueuedVideoJob itself: never hold a scene/shot reference
      // across a render() call.
      const scene = project.scenes.find((candidate) => candidate.id === item.sceneId);
      const shot = scene?.shots?.[0];
      const sourceFrame = scene && shot ? storyboardVideoSourceFrame(scene, shot) : null;
      if (!scene || !shot || !sourceFrame?.path) { failed += 1; continue; }
      try { const outcome = await submitSceneVideoHeadless(scene, shot, item.model, sourceFrame); if (outcome?.ok) done += 1; else failed += 1; }
      catch (error) { failed += 1; console.warn(`Bulk video failed for "${item.sceneTitle}":`, error?.message || error); }
      updateGenerationOverlay({ id: dockId, kind: "video batch", stage: `${started} of ${runnable.length} started · ${done} done · ${failed} failed`, progress: Math.round(((done + failed) / runnable.length) * 100) });
    }
  };
  await Promise.all(Array.from({ length: Math.min(MAX_CONCURRENT_BULK_VIDEOS, runnable.length) }, worker));
  return { done, failed, started };
}
async function bulkGenerateAllVideos() {
  if (bulkGenerationActive) return;
  if (!window.storyMakerDesktop?.submitShotVideo) return notify("Bulk video generation is available in the packaged Windows app.");
  const candidates = bulkVideoCandidates();
  if (!candidates.length) return notify("Every scene with a frame already has a video, or no scene has a frame to animate yet.");
  const priced = candidates.map(({ scene, sceneIndex }) => {
    if (!scene.shots.length) scene.shots.push(newShot(scene, 1)); // a scene can carry a still via referenceAssetId with no shot yet
    const shot = scene.shots[0];
    const sourceFrame = sceneGeneratedAsset(scene);
    const model = resolveBulkVideoModel(shot, "image-to-video");
    const ready = Boolean(model) && modelReady(model) && Boolean(sourceFrame?.path);
    const cost = model ? estimateGenerationCost({ provider: model.provider, model: model.model }, "video") : null;
    return { sceneId: scene.id, sceneTitle: scene.title, sceneIndex, model, ready, cost };
  });
  const runnable = priced.filter((item) => item.ready);
  const blocked = priced.filter((item) => !item.ready);
  if (!runnable.length) return notify(`None of the ${priced.length} remaining scene${priced.length === 1 ? "" : "s"} can animate yet — connect an image-to-video provider (fal, Kie, or WaveSpeed) in Model Hub first.`);
  const known = runnable.filter((item) => item.cost != null);
  const total = known.reduce((sum, item) => sum + item.cost, 0);
  const costLine = !known.length ? "cost not tracked for this model" : known.length === runnable.length ? `~$${total.toFixed(2)} estimated` : `~$${total.toFixed(2)}+ estimated (${runnable.length - known.length} without a cost estimate)`;
  const confirmMsg = [
    `Animate ${runnable.length} scene${runnable.length === 1 ? "" : "s"} — ${costLine}. Video costs meaningfully more per clip than a still.`,
    blocked.length ? `${blocked.length} scene${blocked.length === 1 ? "" : "s"} will be skipped: ${blocked.slice(0, 3).map((item) => item.sceneTitle).join(", ")}${blocked.length > 3 ? "…" : ""}.` : "",
    `Runs up to ${MAX_CONCURRENT_BULK_VIDEOS} at a time. Stopping only stops new scenes from starting — anything already sent to the provider keeps rendering and will still be billed.`,
    "Continue?"
  ].filter(Boolean).join(" ");
  if (!confirm(confirmMsg)) return;
  bulkGenerationActive = true; render();
  const state = { cancelled: false };
  const dockId = openGenerationOverlay("video batch", { id: "bulk-video", stage: "Starting the batch" });
  const card = generationOverlayState.get(dockId)?.el?.querySelector(".generation-overlay-card");
  if (card) {
    const cancelBtn = document.createElement("button");
    cancelBtn.type = "button"; cancelBtn.className = "generation-overlay-minimize"; cancelBtn.style.right = "34px"; cancelBtn.title = "Stop starting new scenes — anything already sent to the provider keeps rendering";
    cancelBtn.textContent = "✕";
    cancelBtn.addEventListener("click", () => { state.cancelled = true; cancelBtn.disabled = true; cancelBtn.title = "No new scenes will start. Already-rendering scenes finish on their own."; });
    card.appendChild(cancelBtn);
  }
  let result;
  try { result = await runVideosBatch(runnable, dockId, state); }
  finally { closeGenerationOverlay(dockId); bulkGenerationActive = false; }
  const summary = [`${result.done} video${result.done === 1 ? "" : "s"} generated`, result.failed ? `${result.failed} failed` : "", blocked.length ? `${blocked.length} skipped` : "", state.cancelled && result.started < runnable.length ? "stopped early" : ""].filter(Boolean).join(", ");
  notify(`Bulk video generation done — ${summary}.`);
  render();
}
// Batch re-roll by section kind — Music Video mode only. "Redo every
// Chorus": regenerates whichever kind of output each matching, UNLOCKED
// scene currently has (a still gets a new still, a video gets a new
// video — whatever a single scene's own "Generate another take"/"Generate
// another video" button would do), across every section of the chosen
// kind at once. Unlike Generate/Animate everything (which only ever fill
// an empty scene), this deliberately REPLACES a scene's current take —
// the shot lock is what makes that safe to run broadly: an approved or
// hand-locked shot is never a candidate, no matter what kind it is.
function sectionKindsPresent() {
  if (!project.song) return [];
  return [...new Set(project.song.sections.map((section) => section.kind))];
}
// kind: a specific section kind ("Chorus"), or null/"" to match every
// section regardless of kind — used by the feeling-scoped re-roll below,
// since feeling (unlike kind) is one project-wide setting, not a
// per-section property, so there's no per-kind subset to filter by there.
function rerollCandidates(kind) {
  ensureProductionShape();
  if (!project.song) return [];
  return project.scenes.map((scene, sceneIndex) => ({ scene, sceneIndex })).filter(({ scene }) => {
    if (scene.shots[0]?.locked) return false;
    const section = project.song.sections.find((item) => item.id === scene.songSectionId);
    if (!section) return false;
    if (kind && section.kind !== kind) return false;
    return Boolean(sceneGeneratedAsset(scene));
  });
}
// label is what confirm/notify text calls the batch — the section kind for
// a kind-scoped re-roll, or the active feeling's name for a feeling-scoped
// one (bulkRerollForFeeling below); kind itself (used for filtering) stays
// null in the feeling case, matching rerollCandidates' "no kind = every
// section" contract.
async function bulkReroll(kind, label) {
  if (bulkGenerationActive) return;
  if (!label) return notify("Choose what to re-roll first.");
  const candidates = rerollCandidates(kind);
  if (!candidates.length) return notify(`No unlocked ${label} scene has an existing take to re-roll.`);
  const priced = candidates.map(({ scene, sceneIndex }) => {
    const shot = scene.shots[0];
    const currentAsset = sceneGeneratedAsset(scene);
    const isVideo = currentAsset?.kind === "video";
    const model = isVideo ? resolveBulkVideoModel(shot, "image-to-video") : resolveBulkModel(shot);
    const sourceFrame = isVideo ? storyboardVideoSourceFrame(scene, shot) : null;
    const ready = Boolean(model) && modelReady(model) && (!isVideo || Boolean(sourceFrame?.path));
    const cost = model ? estimateGenerationCost({ provider: model.provider, model: model.model }, isVideo ? "video" : "image") : null;
    return { sceneId: scene.id, sceneTitle: scene.title, sceneIndex, isVideo, model, ready, cost };
  });
  const runnable = priced.filter((item) => item.ready);
  const blocked = priced.filter((item) => !item.ready);
  if (!runnable.length) return notify(`None of the ${priced.length} ${label} scene${priced.length === 1 ? "" : "s"} can re-roll yet — connect the assigned provider in Model Hub first.`);
  const stillJobs = runnable.filter((item) => !item.isVideo);
  const videoJobs = runnable.filter((item) => item.isVideo);
  const known = runnable.filter((item) => item.cost != null);
  const total = known.reduce((sum, item) => sum + item.cost, 0);
  const costLine = !known.length ? "cost not tracked for these models" : known.length === runnable.length ? `~$${total.toFixed(2)} estimated` : `~$${total.toFixed(2)}+ estimated (${runnable.length - known.length} without a cost estimate)`;
  const confirmMsg = [
    `Re-roll ${runnable.length} ${label} scene${runnable.length === 1 ? "" : "s"} (${stillJobs.length} still${stillJobs.length === 1 ? "" : "s"}, ${videoJobs.length} video${videoJobs.length === 1 ? "" : "s"}) — ${costLine}.`,
    "Each gets a brand-new take, replacing what's current — the old one stays in that shot's history, just no longer the one shown.",
    blocked.length ? `${blocked.length} ${label} scene${blocked.length === 1 ? "" : "s"} will be skipped: ${blocked.slice(0, 3).map((item) => item.sceneTitle).join(", ")}${blocked.length > 3 ? "…" : ""}.` : "",
    "Locked and approved shots are never included, whatever their kind.",
    "Continue?"
  ].filter(Boolean).join(" ");
  if (!confirm(confirmMsg)) return;
  bulkGenerationActive = true; render();
  const state = { cancelled: false };
  const dockId = openGenerationOverlay(`${label} re-roll`, { id: "bulk-reroll", stage: "Starting the batch" });
  const card = generationOverlayState.get(dockId)?.el?.querySelector(".generation-overlay-card");
  if (card) {
    const cancelBtn = document.createElement("button");
    cancelBtn.type = "button"; cancelBtn.className = "generation-overlay-minimize"; cancelBtn.style.right = "34px"; cancelBtn.title = "Stop after the current item — any video already sent to a provider keeps rendering";
    cancelBtn.textContent = "✕";
    cancelBtn.addEventListener("click", () => { state.cancelled = true; cancelBtn.disabled = true; cancelBtn.title = "Stopping…"; });
    card.appendChild(cancelBtn);
  }
  let stillsResult = { done: 0, failed: 0 }; let videoResult = { done: 0, failed: 0, started: 0 };
  try {
    if (stillJobs.length) stillsResult = await runStillsBatch(stillJobs, dockId, state);
    if (videoJobs.length) videoResult = await runVideosBatch(videoJobs, dockId, state);
  } finally { closeGenerationOverlay(dockId); bulkGenerationActive = false; }
  const done = stillsResult.done + videoResult.done; const failed = stillsResult.failed + videoResult.failed;
  const summary = [`${done} re-rolled`, failed ? `${failed} failed` : "", blocked.length ? `${blocked.length} skipped` : "", state.cancelled ? "stopped early" : ""].filter(Boolean).join(", ");
  notify(`${label} re-roll done — ${summary}.`);
  render();
}
function bulkRerollSectionKind(kind) { return bulkReroll(kind, kind); }
// The companion to changing Feeling: reseedSongScenes() (called by the
// Feeling select's own change handler) updates every section's TEXT brief
// immediately, but a frame or video already generated under the OLD
// feeling doesn't repaint itself — it just silently drifts out of sync
// with the newly chosen creative direction until something regenerates it.
// This is that "something," scoped to every eligible scene at once (no
// kind filter — feeling has no per-section subset to narrow to; see
// rerollCandidates).
function bulkRerollForFeeling() {
  if (!project.songFeeling) return notify("Choose a feeling first.");
  return bulkReroll(null, feelingLabel(project.songFeeling));
}
...
async function saveProject(saveAs = false, closing = false) {
  ...
  filePath = result.filePath; setDirty(false); render();
  if (project.milestones?.firstSave) notify("Project saved safely."); else celebrateFirstSave();
  if (closing) window.storyMakerDesktop?.closeAfterSave();
}
...
// "My own look" — the no-influence option in the Director-inspired scope.
// Not a Style DNA; just reverts the project to no locked visual direction,
// the same state a brand-new project starts in.
function clearProjectStyleDna() {
  ensureProductionShape();
  project.style = ""; project.visualDirection.projectStyle = null;
  setDirty(); render(); notify("Visual direction cleared — using this project's own look.");
}
...
async function verifyProvider(provider, button) {
  ...
  const original = button?.textContent; if (button) { button.disabled = true; button.textContent = "Verifying…"; }
...
function bind() {
  ...
  document.querySelectorAll("[data-nav]").forEach((button) => button.addEventListener("click", () => { active = button.dataset.nav; render(); if (active === "Delivery") { refreshDeliveryCapabilities(); refreshLicenseState(); } if (active === "Settings") refreshLicenseState(); if (active === "Model Hub") refreshProviders(); }));
  ...
  $("#directMusicVideo")?.addEventListener("click", directAsMusicVideo);
  $("#directMusicVideoAI")?.addEventListener("click", directMusicVideoWithAI);
  $("#editSections")?.addEventListener("click", openSectionEditor);
  $("#directMusicVideoStoryboard")?.addEventListener("click", directMusicVideoWithAI);
  $("#directMusicVideoHome")?.addEventListener("click", () => { active = "Audio Studio"; render(); directAsMusicVideo(); });
  $("#songFeelingSelect")?.addEventListener("change", (event) => { project.songFeeling = event.target.value; reseedSongScenes(); setDirty(); render(); notify(project.songFeeling ? t("audio.notify.feelingSet", { feeling: feelingLabel(project.songFeeling) }) : t("audio.notify.feelingReset")); });
  $("#songLyricsInput")?.addEventListener("change", (event) => { project.songLyrics = event.target.value; reseedSongScenes(); setDirty(); render(); notify(t("audio.notify.lyricsSpread", { count: (project.song?.lyrics || []).length })); });
  ...
  $("#exportLyrics")?.addEventListener("click", exportLyrics);
  $("#renderVisualPreview")?.addEventListener("click", renderVisualPreview);
  $("#exportEditTimeline")?.addEventListener("click", exportEditTimeline);
  $("#sendEditToResolve")?.addEventListener("click", sendEditToResolve);
  ...
  document.querySelectorAll("[data-send-to-resolve]").forEach((button) => button.addEventListener("click", () => { const [sceneIndex, shotIndex] = button.dataset.sendToResolve.split(":").map(Number); sendShotToResolve(sceneIndex, shotIndex, button); }));
  ...
  $("#bulkGenerateScenes")?.addEventListener("click", bulkGenerateAllScenes);
  $("#bulkGenerateVideos")?.addEventListener("click", bulkGenerateAllVideos);
  $("#rerollSectionKind")?.addEventListener("click", () => bulkRerollSectionKind($("#rerollKindSelect")?.value || ""));
  $("#rerollForFeeling")?.addEventListener("click", bulkRerollForFeeling);
  ...
  $("#localeSelect")?.addEventListener("change", (event) => { locale = setLocale(event.target.value); localStorage.setItem("storymaker-locale", locale); render(); notify(t("settings.language.confirm", { language: SUPPORTED_LOCALES.find((entry) => entry.id === locale)?.label || locale })); });
  ...
  document.querySelectorAll("[data-remove-character]").forEach((button) => button.addEventListener("click", () => {
    ...
    // Was a blocking confirm() dialog; now an 8s Undo toast (see withUndo) —
    // reversible instead of just double-checked, and no interrupting modal
    // for a whole production profile (appearance, wardrobe, voice,
    // objective, movement, continuity rules) plus every reference tag.
    withUndo(`"${character.name}" removed.`, () => {
  ...
  document.querySelectorAll("[data-timeline-shot-duration]").forEach((field) => field.addEventListener("change", () => { const [sceneIndex, shotIndex] = field.dataset.timelineShotDuration.split(":").map(Number); const scene = project.scenes[sceneIndex]; const shot = scene?.shots?.[shotIndex]; if (!shot) return; const songSeconds = songSectionSeconds(scene); if (songSeconds != null) { const parsed = Number.parseFloat(field.value); shot.duration = Number.isFinite(parsed) && parsed > 0 ? String(Math.min(parsed, songSeconds)) : String(songSeconds); } else { shot.duration = String(numericDuration(field.value)); } setDirty(); render(); }));
  ...
  $("#refreshLocalRuntimes")?.addEventListener("click", async (event) => { event.currentTarget.disabled = true; event.currentTarget.textContent = "Refreshing…"; await refreshProviders(); notify("Local engine status refreshed."); });
  $("#refreshLocalEngine")?.addEventListener("click", (event) => { event.currentTarget.disabled = true; event.currentTarget.textContent = "Refreshing…"; refreshLocalEngine(true); });
  $("#startLocalEngine")?.addEventListener("click", async (event) => {
    event.currentTarget.disabled = true; event.currentTarget.textContent = "Starting…";
    try { localEngineState = { ...localEngineState, ...(await window.storyMakerDesktop.startLocalEngine()) }; notify("Local engine is running."); }
    catch (error) { notify(error?.message || "The local engine could not start."); }
    render();
  });
  $("#stopLocalEngine")?.addEventListener("click", async () => {
    try { localEngineState = { ...localEngineState, ...(await window.storyMakerDesktop.stopLocalEngine()) }; notify("Local engine stopped."); } catch (error) { notify(error?.message || "Could not stop the engine."); }
    render();
  });
  document.querySelectorAll("[data-engine-download]").forEach((button) => button.addEventListener("click", async () => {
    const entryId = button.dataset.engineDownload;
    button.disabled = true;
    try { await window.storyMakerDesktop.downloadLocalModel(entryId); notify("Model downloaded and verified."); }
    catch (error) { notify(error?.message || "The model download failed."); }
    refreshLocalEngine(true);
  }));
  $("[data-engine-cancel]")?.addEventListener("click", () => { window.storyMakerDesktop?.cancelLocalModelDownload?.(); notify("Cancelling the download…"); });
  $("#openDiagnosticsLog")?.addEventListener("click", async () => { if (!window.storyMakerDesktop?.openDiagnosticsLog) return notify("The diagnostics log is available in the packaged Windows app."); const result = await window.storyMakerDesktop.openDiagnosticsLog(); if (result?.empty) notify("No diagnostics recorded yet on this PC — opened its folder instead."); });
  $("#telemetryEnabled")?.addEventListener("change", async (event) => {
    if (!window.storyMakerDesktop?.setTelemetryEnabled) { event.target.checked = telemetryState.enabled; return notify("Crash reporting is available in the packaged Windows app."); }
    const enabled = event.target.checked;
    const result = await window.storyMakerDesktop.setTelemetryEnabled(enabled);
    telemetryState = { ...telemetryState, enabled: Boolean(result?.enabled) };
    notify(telemetryState.enabled ? "Crash and error reports will now be sent automatically." : "Crash and error reporting is off.");
  });
  $("#manageLicense")?.addEventListener("click", () => openLicenseDialog());
  $("#licenseBannerActivate")?.addEventListener("click", () => openLicenseDialog());
  $("#checkForUpdates")?.addEventListener("click", async () => {
    if (!window.storyMakerDesktop?.checkForUpdates) return notify("Update checks are available in the packaged Windows app.");
    updateState = { ...updateState, status: "checking" }; render();
    const result = await window.storyMakerDesktop.checkForUpdates();
    // Set state directly from this call's own result rather than waiting on
    // the separate onUpdateStatus event — electron-updater fires
    // "update-available"/"update-not-available" as a side effect of the
    // same checkForUpdates() call in the main process, sent over a
    // different IPC channel than this invoke's reply, with no guaranteed
    // arrival order between the two. Relying on the event alone left a real
    // window where this promise resolves with a version already in hand
    // but the UI stays stuck on "Checking…" until — if ever — the event
    // catches up. onUpdateStatus still matters for the background check
    // ~8s after launch, which has no corresponding click result to read.
    if (!result?.ok) updateState = { ...updateState, status: "error", message: result?.message || "The update check failed." };
    else if (result.version) updateState = { ...updateState, status: "available", version: result.version };
    else updateState = { ...updateState, status: "up-to-date" };
    render();
  });
  $("#downloadUpdateBtn")?.addEventListener("click", async () => {
    updateState = { ...updateState, status: "downloading", percent: 0 }; render();
    const result = await window.storyMakerDesktop?.downloadUpdate?.();
    if (!result?.ok) { updateState = { ...updateState, status: "error", message: result?.message || "The update could not be downloaded." }; render(); }
  });
  $("#installUpdateBtn")?.addEventListener("click", () => { window.storyMakerDesktop?.installUpdate?.(); });
  ...
  if (active === "Storyboard") {
    ...
    if (beatline && !$(".storyboard-visual-notice")) {
      ...
      notice.innerHTML = `<div><strong>${esc(direction.notSet ? "Visual direction needs a decision" : `Visual direction · ${direction.name}`)}</strong><span>${esc(direction.notSet ? "Set the project look before generation so every scene inherits the same lighting, camera, material, and negative-constraint rules." : "Every scene inherits this look until you deliberately override it in Shot Planner.")}</span></div><button type="button" class="${direction.notSet ? "save-button" : "quiet-button"}">${direction.notSet ? "Set visual direction" : "Review look"}</button>`;
  ...
  $("#helpTrySample")?.addEventListener("click", () => { active = "Story Bible"; render(); openPasteStoryModal(SAMPLE_SCRIPT); });
  $("#helpCopySkeleton")?.addEventListener("click", async () => {
    try { await navigator.clipboard.writeText(SCRIPT_SKELETON); notify("Skeleton copied — paste it into the script box or your own editor."); }
    catch { notify("Couldn't copy automatically — select the skeleton text and copy it manually."); }
  });
  ...
  document.querySelectorAll("[data-clear-project-style]").forEach((button) => button.addEventListener("click", clearProjectStyleDna));
  ...
  document.querySelectorAll("[data-delete-dna]").forEach((button) => button.addEventListener("click", () => { const dna = styleDnaById(button.dataset.deleteDna); withUndo(`"${dna?.name || "Style"}" deleted.`, () => { deleteStyleDna(button.dataset.deleteDna); render(); }); }));
...
window.storyMakerDesktop?.getAppVersion?.().then((version) => { appVersion = String(version || ""); if (active === "Settings") render(); }).catch(() => {});
window.storyMakerDesktop?.telemetryStatus?.().then((status) => { telemetryState = { configured: Boolean(status?.configured), enabled: Boolean(status?.enabled) }; if (active === "Settings") render(); }).catch(() => {});
async function refreshLocalEngine(rerender) {
  if (!window.storyMakerDesktop?.localEngineStatus) return;
  try { localEngineState = { ...localEngineState, ...(await window.storyMakerDesktop.localEngineStatus()) }; }
  catch { /* keep last-known */ }
  if (rerender && active === "Model Hub") render();
}
refreshLocalEngine(false).then(() => { if (active === "Model Hub") render(); });
// Live download progress: patch state and, if the user is looking at Model
// Hub, re-render just enough to move the bar. Throttled by the main process
// (it only emits every ~8 MB), so a plain render() here is fine.
window.storyMakerDesktop?.onLocalEngineProgress?.((payload) => {
  if (!payload || payload.phase === "idle") { localEngineState = { ...localEngineState, download: null }; }
  else { localEngineState = { ...localEngineState, download: payload }; }
  if (active === "Model Hub") render();
});
window.storyMakerDesktop?.licenseStatus?.().then((status) => { licenseState = { ...licenseState, ...status }; render(); }).catch(() => {});
// electron-main.js checks for updates ~8s after launch on its own — this
// only listens for whatever it reports, it never triggers a check itself,
// matching the same "notify, never act without you" pattern the rest of
// this app's local/provider status checks already follow.
window.storyMakerDesktop?.onUpdateStatus?.((status) => {
  updateState = { ...updateState, status: status?.status || updateState.status, version: status?.version || updateState.version, percent: status?.percent ?? updateState.percent, message: status?.message || "" };
  if (status?.status === "available") notify(`Storymaker ${status.version} is available — open Settings to download it.`);
  if (status?.status === "downloaded") notify(`Storymaker ${status.version} downloaded — restart from Settings to install it.`);
  if (active === "Settings") render();
});

// Most of what a user experiences as "the app just broke" is a renderer-
// side JS exception in a click/event handler, not a main-process crash —
// by default that only reaches DevTools' console, which nobody has open.
// Forwarded to the same local diagnostics log the main process already
// writes generation failures to (redacted there too), so a real crash
// leaves an artifact instead of just "it stopped working." No third-party
// account needed — see the matching note on the main-process handlers in
// electron-main.js for why this stays local-only for now.
window.addEventListener("error", (event) => {
  window.storyMakerDesktop?.logRendererError?.({ message: String(event?.message || ""), source: String(event?.filename || ""), line: event?.lineno || 0, col: event?.colno || 0, stack: String(event?.error?.stack || "").slice(0, 4000) })?.catch?.(() => {});
});
window.addEventListener("unhandledrejection", (event) => {
  const reason = event?.reason;
  window.storyMakerDesktop?.logRendererError?.({ message: String(reason?.message || reason || ""), stack: String(reason?.stack || "").slice(0, 4000), type: "unhandledrejection" })?.catch?.(() => {});
});
render();
