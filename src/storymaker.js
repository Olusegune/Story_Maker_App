  ...
  return `<div class="studio-shell nav-${navMode} experience-${experienceMode}" data-theme="${theme}">
    ...
    <main class="studio-main">
      <header class="studio-bar">
        ...
        <div class="bar-actions"><button class="quiet-button" id="quickOpen">${esc(t("shell.open"))}</button><button class="save-button" id="saveProject">${esc(t("shell.save"))}<span>${dirty ? "•" : ""}</span></button></div>
      </header>
...
function styleDnaWorkspace() {
  ...
  const card = (dna) => `<article class="visual-style-card ${active?.id === dna.id ? "active" : ""}"><button class="visual-style-card-main" data-style-detail="${esc(dna.id)}"><div class="visual-style-card-image"><img src="${esc(stylePreviewUrl(dna))}" alt="" /></div><span>${esc((dna.categoryIds || ["visual direction"])[0])}</span><h3>${esc(dna.name)}</h3><p>${esc(dna.shortDescription || styleSummary(dna))}</p><small>${esc(dna.source)} · v${dna.version}</small></button><div><button class="quiet-button" data-toggle-style-favorite="${esc(dna.id)}" aria-label="Toggle favorite">${favoriteIds.includes(dna.id) ? "★" : "☆"}</button><button class="save-button" data-apply-project-style="${esc(dna.id)}">Apply</button></div></article>`;
...
function mountLaunchSplash() {
  ...
  // assistive tech, per the note above) sized generously enough to cover
  // where the pill sits on both splash images (confirmed by inspecting
  // each PNG directly — they share one 1672×941 template, the button
  // lands in very slightly different spots between the two, hence the
  // padding beyond either one's exact bounds). Percentage-positioned, not
