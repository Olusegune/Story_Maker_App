...
// Path confinement for local-file IPC handlers (media:read-bytes,
// source:run-ocr, media:upscale, media:save-copy) that accept a path from
// the renderer. Without this, a path sitting in project JSON — which can
// come from opening a `.storymaker` file someone else sent, not just this
// user's own picks — would be trusted blindly, letting a crafted project
// file point at an arbitrary local file (e.g. outside any project folder)
// and have it read and, for OCR/upscale, uploaded to a cloud provider the
// moment the user clicks an ordinary-looking button. Every path actually
// chosen by the user this session (via dialog.showOpenDialog in
// media:import/source:import) is recorded here; generated-media and the
// current project's own staged `.assets` folder (populated only by this
// app's own stageProjectAssets on save) are always allowed. A path loaded
// from project JSON that was never legitimately authorized this way is
// rejected — the user can always re-import to authorize it again.
const authorizedMediaPaths = new Set();
let currentProjectAssetsDir = "";
const authorizeMediaPath = (p) => { const resolved = path.resolve(String(p || "")); if (resolved) authorizedMediaPaths.add(resolved); };
const noteCurrentProjectFile = (filePath) => { const p = String(filePath || ""); currentProjectAssetsDir = p ? path.join(path.dirname(p), `${path.basename(p, path.extname(p))}.assets`) : ""; };
const assertMediaPathAuthorized = (p) => {
  const resolved = path.resolve(String(p || ""));
  const generatedDir = path.join(app.getPath("userData"), "generated-media");
  if (resolved.startsWith(generatedDir + path.sep)) return resolved;
  if (currentProjectAssetsDir && resolved.startsWith(path.resolve(currentProjectAssetsDir) + path.sep)) return resolved;
  if (authorizedMediaPaths.has(resolved)) return resolved;
  throw new Error("This file wasn't chosen through Storymaker in this session — re-import it before continuing.");
};
...
// Model id gemini-2.5-flash (2026-09-14): this and the two other
// generateContent call sites below (drift-check, reference-description)
// all hardcoded gemini-2.0-flash, which turned out to be retired — a live
// call returned 404 "model gemini-2.0-flash is no longer [available];
// use models/gemini-2.5-flash", caught while testing the reference-
// description feature with a real connected key. Every Gemini text/vision
// call in this file was silently failing until this fix, not just the one
// being tested.
async function requestGeminiCompletion(instructions, userContent, apiKey, timeoutMs = 90000) {
  ...
  try {
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${encodeURIComponent(apiKey)}`, {
      ...
      // thinkingConfig: { thinkingBudget: 0 } — gemini-2.5-flash "thinks"
      // by default, and that reasoning is billed against the SAME output
      // token budget as the actual answer. None of this file's three
      // generateContent call sites (this one, drift-check, reference-
      // description) need chain-of-thought for what they're asked to do;
      // live-caught on reference-description, where a real response came
      // back truncated mid-sentence with plenty of max_output_tokens
      // headroom left unused — the tokens went to invisible thinking, not
      // the visible answer. Disabling it here removes that risk (and the
      // added latency/cost) for every caller of this shared completion path.
      body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: `${instructions}\n\n${userContent}` }] }], generationConfig: { responseMimeType: "application/json", temperature: 0.3, thinkingConfig: { thinkingBudget: 0 } } })
    });
...
async function requestImageOcr(payload) {
  const filePath = assertMediaPathAuthorized(payload?.filePath);
...
async function upscaleProjectMedia(payload) {
  ...
  const filePath = assertMediaPathAuthorized(payload?.path); const kind = String(payload?.kind || mediaKind(filePath)); const factor = Math.max(1, Math.min(4, Number(payload?.factor) || 2));
...
// Defense-in-depth against a hypothetical future XSS (a missed esc() call,
// or a prompt-injection payload from imported/AI-generated text reflected
// into a template). contextIsolation + nodeIntegration:false already stop
// a script injection from reaching Node — this stops it from doing much of
// anything else. Every real provider call happens in the main process
// (confirmed: no fetch() anywhere in src/), so the renderer itself never
// needs to reach an external host directly — connect-src 'self' is enough,
// not the provider allowlist a naive CSP here might reach for. style-src
// needs 'unsafe-inline' for the app's many inline style="" attributes
// (gradients, computed positioning); script-src does not, since the built
// app has no inline <script> blocks.
const CONTENT_SECURITY_POLICY = "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' file: data:; media-src 'self' file:; connect-src 'self'; object-src 'none'; base-uri 'none'; form-action 'none'";
function createWindow() {
  ...
  // No in-app navigation or new-window flow exists (no window.open/<a href>
  // to an external URL anywhere in src/) — these are pure defense-in-depth
  // so an injected payload can't repurpose the app window into a phishing
  // page or spawn an unsandboxed popup.
...
async function readImageAsBase64(assetPath, assetUrl) {
  ...
  if (localPath) {
    // Same path-confinement every other local-file IPC handler applies (see
    // the note on assertMediaPathAuthorized above) — this was the one vision
    // call site that still trusted a renderer-supplied path unchecked,
    // reachable via drift-check and now also the reference-description
    // helper below, both of which upload the file's bytes to a cloud
    // provider on success.
    const authorized = assertMediaPathAuthorized(localPath);
    if (!fs.existsSync(authorized)) throw new Error(`Asset file not found on disk: ${authorized}`);
    const mime = imageMimeType(authorized);
    if (!mime) throw new Error("Style drift can only be checked against an image, not a video or other file.");
    return { data: fs.readFileSync(authorized).toString("base64"), mime };
  }
  // No branch for a raw http(s) assetUrl: every asset this function is ever
  // legitimately called with (drift-check, reference-description) is one of
  // this project's own staged media, whose previewUrl is always built with
  // pathToFileURL() — grep the write sites in this file and every one is a
  // file: URL, never a genuine remote one. A raw fetch(assetUrl) here would
  // have been an unauthenticated SSRF primitive: a `.storymaker` project
  // file is untrusted JSON (it can arrive from anyone), and a crafted
  // asset.previewUrl pointing at an internal service or cloud metadata
  // endpoint would have been fetched — with the response's bytes handed to
  // an external vision provider on success — the moment a user clicked
  // "Check consistency" or "Describe from reference" on that asset, with no
  // authorization check at all on that path (unlike the local-path branch
  // above). If a genuine hosted-preview case ever needs this, it should
  // authorize the specific URL the same deliberate way authorizeMediaPath
  // does for local paths, not accept any URL unconditionally.
  throw new Error("This asset has no local file to check — re-import or regenerate it first.");
}
...
Drift is 0% when the image perfectly matches the profile, 100% when completely different. Base scores on visual analysis, not assumptions.`;
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${encodeURIComponent(apiKey)}`, {
      ...
      body: JSON.stringify({
        ...
        // thinkingConfig: { thinkingBudget: 0 } — see the note on the
        // gemini-2.5-flash completion call above; drift's JSON report
        // doesn't need chain-of-thought either, and thinking output shares
        // this same max_output_tokens budget.
        generationConfig: { temperature: 0.3, max_output_tokens: 1024, thinkingConfig: { thinkingBudget: 0 } }
      })
    });
...
// Character/Prop/Set Lab prompt helper: describe a reference image so the
// user only has to add the action/pose, not the whole visual description,
// closing the gap the user actually hit — a reference image was uploaded
// but the design-brief field still had to be written by hand. Reuses the
// same readImageAsBase64/vision-provider round trip as drift-check above
// rather than a second image-reading code path. Deliberately instructed
// never to describe a pose, expression-in-the-moment, or background — the
// output is meant to be a reusable identity/object/place description the
// generation prompt-building code already knows how to combine with the
// rest of a shot, not a finished one-off prompt.
const REFERENCE_DESCRIPTION_PROMPTS = {
  character: "Describe this character's physical appearance for reuse as a generation design brief: face shape, hair, build, skin tone, wardrobe, and any distinguishing features. Use specific, concrete visual language, written as flowing prose (not a list). Do NOT include any pose, action, expression-in-the-moment, or background/setting — describe only the reusable identity, as if writing a character design bible entry.",
  prop: "Describe this object for reuse as a generation design brief: material, shape, color, scale, condition, and distinguishing design details. Use specific, concrete visual language, written as flowing prose (not a list). Do NOT include any background, setting, or context — describe only the object itself.",
  set: "Describe this location for reuse as a generation design brief: architecture, materials, lighting mood, color palette, and defining visual features. Use specific, concrete visual language, written as flowing prose (not a list). Do NOT include any characters, props in use, or a specific narrative moment — describe only the reusable environment."
};
async function requestGeminiReferenceDescription(payload, apiKey) {
  const instruction = REFERENCE_DESCRIPTION_PROMPTS[payload?.kind] || REFERENCE_DESCRIPTION_PROMPTS.character;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 30000);
  try {
    const image = await readImageAsBase64(payload?.assetPath, payload?.assetUrl);
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${encodeURIComponent(apiKey)}`, {
      method: "POST",
      signal: controller.signal,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        contents: [{ parts: [{ text: instruction }, { inline_data: { mime_type: image.mime, data: image.data } }] }],
        // thinkingConfig: { thinkingBudget: 0 } — see the note on the
        // gemini-2.5-flash completion call above. Also raised the token
        // ceiling a bit as headroom for a full descriptive paragraph now
        // that none of it is being spent on invisible thinking.
        generationConfig: { temperature: 0.4, max_output_tokens: 600, thinkingConfig: { thinkingBudget: 0 } }
      })
    });
    const raw = await response.text();
    if (!response.ok) throw new Error(errorForProvider("Gemini", response, raw));
    ...
    if (!text) throw new Error("Gemini returned no description for this reference image.");
    return { description: text };
  } catch (error) {
    if (error?.name === "AbortError") throw new Error("Describing the reference took too long. Please retry.");
    throw error;
  } finally {
    clearTimeout(timeout);
  }
}
async function requestOpenAIReferenceDescription(payload, apiKey) {
  const instruction = REFERENCE_DESCRIPTION_PROMPTS[payload?.kind] || REFERENCE_DESCRIPTION_PROMPTS.character;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 30000);
  try {
    const image = await readImageAsBase64(payload?.assetPath, payload?.assetUrl);
    // Same /v1/responses + gpt-5.1 + input_image pattern as the drift-check
    // and OCR vision calls above, rather than a second, unproven code path.
    const response = await fetch("https://api.openai.com/v1/responses", {
      method: "POST",
      signal: controller.signal,
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({
        model: "gpt-5.1",
        store: false,
        input: [{ role: "user", content: [{ type: "input_text", text: instruction }, { type: "input_image", image_url: `data:${image.mime};base64,${image.data}` }] }]
      })
    });
    const raw = await response.text();
    if (!response.ok) throw new Error(errorForProvider("OpenAI", response, raw));
    ...
    if (!text) throw new Error("OpenAI returned no description for this reference image.");
    return { description: text };
  } catch (error) {
    if (error?.name === "AbortError") throw new Error("Describing the reference took too long. Please retry.");
    throw error;
...
async function requestReferenceDescription(payload) {
  const provider = String(payload?.provider || "").trim();
  if (provider !== "google" && provider !== "openai") throw new Error("Describing a reference requires Gemini (google) or OpenAI provider.");
  const assetPath = String(payload?.assetPath || "").trim();
  const assetUrl = String(payload?.assetUrl || "").trim();
  if (!assetPath && !assetUrl) throw new Error("An asset file or URL is required to describe a reference.");
  if (provider === "google") {
    const key = getProviderKey("google");
    if (!key) throw new Error("Connect a Google Gemini key in Model Hub before describing a reference image.");
    return requestGeminiReferenceDescription(payload, key);
  } else {
    const key = getProviderKey("openai");
    if (!key) throw new Error("Connect an OpenAI key in Model Hub before describing a reference image.");
    return requestOpenAIReferenceDescription(payload, key);
  }
}

ipcMain.handle("project:save", async (_event, payload) => { const project = payload?.project || {}; if (!payload?.saveAs && payload?.filePath && fs.existsSync(payload.filePath)) { writeAtomic(payload.filePath, stageProjectAssets(payload.filePath, project)); dirty = false; noteCurrentProjectFile(payload.filePath); return { canceled: false, filePath: payload.filePath }; } const result = await dialog.showSaveDialog(mainWindow, { title: "Save Story Maker Project", defaultPath: payload?.suggestedName || "untitled.storymaker", filters: [{ name: "Story Maker Project", extensions: ["storymaker"] }, { name: "JSON", extensions: ["json"] }] }); if (result.canceled || !result.filePath) return { canceled: true }; writeAtomic(result.filePath, stageProjectAssets(result.filePath, project)); dirty = false; noteCurrentProjectFile(result.filePath); return { canceled: false, filePath: result.filePath }; });
ipcMain.handle("project:open", async () => { const result = await dialog.showOpenDialog(mainWindow, { title: "Open Story Maker Project", properties: ["openFile"], filters: [{ name: "Story Maker Project", extensions: ["storymaker", "json"] }] }); if (result.canceled || !result.filePaths[0]) return { canceled: true }; const filePath = result.filePaths[0]; const opened = readSafe(filePath); noteCurrentProjectFile(filePath); return { canceled: false, filePath, project: opened.value, recovered: opened.recovered }; });
ipcMain.handle("source:import", async () => { const result = await dialog.showOpenDialog(mainWindow, { title: "Import Story Source", properties: ["openFile"], filters: [{ name: "Story source", extensions: ["pdf", "doc", "docx", "txt", "md", "fdx", "fountain", "celtx", "rtf", "odt", "html", "htm", "csv", "png", "jpg", "jpeg", "webp", "tif", "tiff"] }, { name: "All files", extensions: ["*"] }] }); if (result.canceled || !result.filePaths[0]) return { canceled: true }; const filePath = result.filePaths[0]; const info = fs.statSync(filePath); if (info.size > 32 * 1024 * 1024) throw new Error("Choose a source file smaller than 32 MB."); authorizeMediaPath(filePath); const ingested = ingestStoryFile(filePath); return { canceled: false, ...ingested }; });
...
ipcMain.handle("media:read-bytes", async (_event, filePath) => {
  const p = assertMediaPathAuthorized(filePath);
...
ipcMain.handle("media:import", async () => {
  ...
  assets.forEach((asset) => authorizeMediaPath(asset.path));
...
ipcMain.handle("media:save-copy", async (_event, payload) => {
  const sourcePath = assertMediaPathAuthorized(payload?.path);
...
ipcMain.handle("reference:describe", async (_event, payload) => requestReferenceDescription(payload));
